...
class BaseTradingModule(abc.ABC):
    ...
    def auto_self_code(
        ...
        *,
        ...
        try:
            ...
            result = coder.run_for_module(self, context=context, apply=apply)
            ...
            if (
                ...
                and result.get("status") in ("applied", "approved", "validated")
            ):
    ...
    def learn_from_mistakes(self, mistake_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, "apply_adaptive_parameters"):
                tuning = {}
                if any(
                    "overfit" in str(m).lower() or "high" in str(m).lower()
                    for m in mistakes
                ):
                    tuning["weight_multiplier"] = 0.92
                    tuning["confidence_floor"] = 0.68
                if any(
                    "under" in str(m).lower() or "miss" in str(m).lower()
                    for m in mistakes
                ):
    ...
    # =====================================================
    _MUTATION_COOLDOWN_SEC = (
        180.0  # 3 minutes metabolic rate (configurable per deployment)
    )
    ...
    def _can_mutate(self) -> bool:
        ...
        now = time.time()
        if (
            not hasattr(self, "_last_mutation_attempt")
            or self._last_mutation_attempt == 0
        ):
            return True
    ...
    def improve_with_market(
        self, regime_data: Dict[str, Any], market_context: Optional[Dict] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                market_context.get("market_improving", False)
                or regime_data.get("confidence", 0) > 0.65
            ):
    ...
    def interconnect(
        self, target_modules: Optional[List[str]] = None, message: Optional[Dict] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.event_bus.publish(
                "MODULE_INTERCONNECT", payload, source=self.module_name
            )
            ...
            if target_modules:
                for target in target_modules:
                    self.event_bus.publish(
                        f"MODULE_{target.upper()}_MESSAGE",
                        payload,
                        source=self.module_name,
                    )
    ...
    def sync_with_organism(self, organism_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "weights" in organism_state
                and self.module_name in organism_state["weights"]
            ):
    ...
    def coherence_check(self, mutation_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._peer_mutations.append(
            {"from": source, "changes": changes, "ts": time.time()}
        )
        ...
        is_risk_like = any(
            k in my_name for k in ["risk", "guard", "sentinel", "safety"]
        )
    ...
    def full_autonomous_cycle(
        self, coder: Any = None, organism_context: Optional[Dict] = None
    ) -> Dict[str, Any]:
        ...
        results["learn_mistakes"] = self.learn_from_mistakes(
            organism_context.get("mistakes", {})
        )

        # 3. Auto self-code (ALWAYS via central engine + cooldown + shadow gate)
        results["self_code"] = self.auto_self_code(
            coder=coder, context=organism_context, apply=True, auto_approve=True
        )

        # 4. Improve with market
        if "regime" in organism_context:
            results["market_improve"] = self.improve_with_market(
                organism_context.get("regime", {}), market_context=organism_context
            )
