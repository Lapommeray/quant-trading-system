...
class MetaLearner:
    ...
    def __init__(self, algorithm, model_path=None):
        ...
        default_model_path = (
            Path(__file__).resolve().parents[1] / "models" / "meta_strategy.json"
        )
        configured_model_path = model_path or os.environ.get("QTS_META_MODEL_PATH")
        self.model_path = str(
            Path(configured_model_path or default_model_path).expanduser().resolve()
        )
    ...
    def _update_strategy_weights(self):
        ...
        total_expectancy = sum(
            max(0.01, data["expectancy"]) for data in strategy_performance.values()
        )

        if total_expectancy > 0:
            for strategy, data in strategy_performance.items():
                self.strategy_weights[strategy] = (
                    max(0.01, data["expectancy"]) / total_expectancy
                )
            ...
            for key, value in market_state.items():
                if key in trade_market_state:
                    if isinstance(value, (int, float)) and isinstance(
                        trade_market_state[key], (int, float)
                    ):
                        if (
                            abs(value - trade_market_state[key]) / (abs(value) + 1e-10)
                            < 0.2
                        ):
                            similarity += 1
    ...
    def _load_model(self):
        ...
        try:
            ...
            self.strategy_weights = model_data.get(
                "strategy_weights", self.strategy_weights
            )
