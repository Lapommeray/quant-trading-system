...
class DataRing:
    ...
    def __init__(self, symbol: str, size: int = 200000):
        ...
        self.dtype = np.dtype([
            ('ts','f8'),
            ('bid','f4'), ('ask','f4'),
            ('bid_size','f4'), ('ask_size','f4'),
            ('price','f4'), ('qty','f4'),
            ('side','i1')
        ])
        self.buf = np.zeros(size, dtype=self.dtype)
        self.head = 0
        self._lock = threading.Lock()  # only for head update, readers lock-free reading old data
        self.last_update_ts = time.time()

    def push(self, ts: float, bid: float, ask: float, bid_size: float, ask_size: float, price: float, qty: float, side: int = 0):
    ...
    def latest_bid_ask(self):
        ...
        entry = self.buf[idx]
        return float(entry['bid']), float(entry['ask']), float(entry['bid_size']), float(entry['ask_size'])
