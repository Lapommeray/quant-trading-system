...
class ExecutionPlanner:
    ...
    def plan(
        ...
        if side not in ("buy", "sell"):
            return ExecutionPlan(
                Routing.ABORT, side, symbol, "invalid_side", confidence=confidence
            )

        if not market.get("feed_fresh", True):
            return ExecutionPlan(
                Routing.ABORT, side, symbol, "stale_feed", confidence=confidence
            )
        ...
        if bid <= 0 or ask <= 0 or ask <= bid:
            return ExecutionPlan(
                Routing.ABORT, side, symbol, "no_quote", confidence=confidence
            )
        ...
        if spread_bps > self.config.max_spread_bps:
            return ExecutionPlan(
                ...
                side,
                symbol,
                f"spread_too_wide_{spread_bps:.1f}bps",
        ...
        if (
            whale_conf >= self.config.taker_whale_conf
            and ofi_z >= self.config.taker_ofi_z
            and cvd_align
        ):
            mid = (bid + ask) / 2.0
            return ExecutionPlan(
        ...
        if (
            our_share >= self.config.min_queue_share
            and ofi_z >= self.config.maker_keep_ofi_z
        ):
            return ExecutionPlan(
                Routing.MAKER,
                side,
    ...
    def finalize_after_recheck(
        ...
        * OFI dead / timeout -> abort (never get picked off).
        ...
        if ofi_z >= self.config.recheck_escalate_ofi_z and (
            whale_conf >= 0.5 or urgency >= 0.6
        ):
            return ExecutionPlan(
                Routing.TAKER,
                plan.side,
