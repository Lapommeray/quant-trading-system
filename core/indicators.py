...
class HestonVolatility:
    ...
    def heston_objective(self, params, returns):
        ...
        for t in range(1, n):
            v[t] = np.abs(
                v[t - 1]
                + kappa * (theta - v[t - 1]) / 252
                + xi * np.sqrt(v[t - 1] / 252) * returns[t - 1]
            )
            ll += -0.5 * (
                np.log(2 * np.pi) + np.log(v[t] / 252) + returns[t] ** 2 / (v[t] / 252)
            )
        return -ll
    ...
    def calculate(self, close_prices):
        ...
        try:
            ...
            return pd.Series(
                np.sqrt(v0) * np.sqrt(252), index=close_prices.index[-len(returns) :]
            )
        except Exception as e:
            self.logger.error(f"Error calculating Heston volatility: {e}")
            return pd.Series(
                0.2, index=close_prices.index
            )  # Fallback to 20% volatility
        ...
        try:
            ...
            for i in range(self.window, len(prices) - self.lookahead):
                features = [
                    ...
                    / (
                        prices[i - self.window : i].max()
                        - prices[i - self.window : i].min()
                    ),  # price position
                    (rsi_values[i] - rsi_values[i - self.window : i].min())
                    / (
                        rsi_values[i - self.window : i].max()
                        - rsi_values[i - self.window : i].min()
                    ),  # RSI position
                ]
                X.append(features)
            ...
            if len(X) > self.lookahead:
                ...
                return pd.Series(
                    predictions, index=prices.index[self.window : -self.lookahead]
                )
            else:
...
class OrderFlowImbalance:
    ...
    def calculate(self, trades):
        ...
        try:
            ...
            return pd.Series(
                0.0, index=trades.index if hasattr(trades, "index") else []
            )
