...
class MarketMakerIntentDetector(BaseTradingModule):
    ...
    def analyze(self, market_data):
        ...
        if recent_vol > avg_vol * vol_mult and range_pct < range_thr:
            ...
            bid_trend = (
                np.mean(bid_sizes[-5:]) - np.mean(bid_sizes[-20:-5])
                if len(bid_sizes) >= 20
                else 0
            )
            ask_trend = (
                np.mean(ask_sizes[-5:]) - np.mean(ask_sizes[-20:-5])
                if len(ask_sizes) >= 20
                else 0
            )
        ...
        if len(ask_sizes) >= 5:
            avg_ask = (
                np.mean(ask_sizes[-20:-3])
                if len(ask_sizes) >= 20
                else np.mean(ask_sizes[:-3])
            )
            spike = ask_sizes[-3]
            after = ask_sizes[-1]
        ...
        if len(bid_sizes) >= 5:
            avg_bid = (
                np.mean(bid_sizes[-20:-3])
                if len(bid_sizes) >= 20
                else np.mean(bid_sizes[:-3])
            )
            spike = bid_sizes[-3]
            after = bid_sizes[-1]
    ...
    def learn_from_outcome(self, outcome):
        ...
        if pnl < 0:
            self.config["adaptive"]["absorption_vol_mult"] = min(
                5.0, self.config["adaptive"]["absorption_vol_mult"] + 0.2
            )
    ...
    def diagnose(self, context=None):
        ...
        if stats.get("win_rate", 0.5) < 0.48:
            return {
                "issue": "low_win_rate",
                "params": {"absorption_vol_mult": 3.5, "confidence_floor": 0.72},
            }
        return {"issue": "none"}
