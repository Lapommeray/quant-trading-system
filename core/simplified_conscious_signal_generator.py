...
class ConsciousSignalGenerator:
    """
    Simplified version of ConsciousSignalGenerator for testing
    that doesn't rely on qiskit
    """
    ...
    def generate(self, temporal_data):
        ...
        if len(temporal_data) > 0:
            if isinstance(temporal_data, np.ndarray):
                ...
                volatility = (
                    np.std(temporal_data) if len(temporal_data) > 1 else self.volatility
                )
            else:
                scale = 0.5
        ...
        normalized_signals = (
            weighted_signals / np.sum(weighted_signals)
            if np.sum(weighted_signals) > 0
            else weighted_signals
        )
