...
class QuantumFirewall:
    ...
    def __init__(self, algorithm, quarantine_dir=None):
        ...
        configured_quarantine_dir = quarantine_dir or os.environ.get(
            "QTS_QUARANTINE_DIR"
        )
        self.quarantine_dir = str(
            Path(configured_quarantine_dir or default_quarantine_dir)
            .expanduser()
            .resolve()
        )
    ...
    def validate_strategy(self, strategy):
        ...
        """
        self.logger.info(
            f"Validating strategy: {strategy.name if hasattr(strategy, 'name') else 'Unknown'}"
        )
        ...
        checks = [
            {
                ...
                "valid": leverage <= self.max_leverage,
            },
            {
                "name": "drawdown",
                "value": estimated_drawdown,
                "limit": self.max_drawdown,
                "valid": estimated_drawdown < self.max_drawdown,
            },
            {
                "name": "position_size",
                "value": max_position_size,
                "limit": self.max_position,
                "valid": max_position_size < self.max_position,
            },
            {
                "name": "daily_loss",
                "value": daily_loss_limit,
                "limit": self.max_daily_loss,
                "valid": daily_loss_limit <= self.max_daily_loss,
            },
            {
                "name": "trade_frequency",
                "value": trade_frequency,
                "limit": self.max_trade_frequency,
                "valid": trade_frequency <= self.max_trade_frequency,
            },
        ]
        ...
        if not all_valid:
            ...
            self.logger.info(
                f"Strategy validation passed: {validation_result['strategy']}"
            )
        ...
        try:
            with open(self.quarantine_log_path, "a") as f:
                ...
                for check in quarantine_entry["failed_checks"]:
                    f.write(
                        f"  - {check['name']}: {check['value']} (limit: {check['limit']})\n"
                    )
    ...
    def validate_trade(self, trade):
        ...
        checks = [
            {
                "name": "leverage",
                "value": current_leverage + leverage,
                "limit": self.max_leverage,
                "valid": current_leverage + leverage <= self.max_leverage,
            },
            {
                "name": "position_size",
                "value": position_size,
                "limit": self.max_position,
                "valid": position_size <= self.max_position,
            },
        ]
        ...
        validation_result = {
            "timestamp": datetime.now().isoformat(),
            "trade": {"symbol": symbol, "quantity": quantity, "direction": direction},
            "valid": all_valid,
            "checks": checks,
        }
    ...
    def validate_portfolio(self):
        ...
        for kvp in self.algorithm.Portfolio:
            ...
            if security_hold.Invested:
                position_size = (
                    abs(security_hold.HoldingsValue)
                    / self.algorithm.Portfolio.TotalPortfolioValue
                )
        ...
        checks = [
            {
                ...
                "limit": self.max_leverage,
                "valid": current_leverage <= self.max_leverage,
            },
            {
                "name": "position_size",
                "value": max_position_size,
    ...
    def enforce_portfolio_limits(self):
        ...
        if current_leverage > self.max_leverage:
            self.logger.warning(
                f"Leverage too high: {current_leverage} (limit: {self.max_leverage})"
            )
            ...
            for kvp in self.algorithm.Portfolio:
                ...
                if security_hold.Invested:
                    self.algorithm.SetHoldings(
                        security_hold.Symbol, security_hold.Quantity * reduction_factor
                    )

                    self.logger.info(
                        f"Reduced position in {security_hold.Symbol} by factor {reduction_factor}"
                    )
        ...
        for kvp in self.algorithm.Portfolio:
            ...
            if security_hold.Invested:
                ...
                if position_size > self.max_position:
                    self.logger.warning(
                        f"Position too large: {security_hold.Symbol} at {position_size} (limit: {self.max_position})"
                    )

                    new_quantity = security_hold.Quantity * (
                        self.max_position / position_size
                    )
                    self.algorithm.SetHoldings(security_hold.Symbol, new_quantity)

                    self.logger.info(
                        f"Reduced position in {security_hold.Symbol} from {position_size} to {self.max_position}"
                    )
    ...
    def set_risk_limits(
        ...
        self.logger.info(
            f"Risk limits updated: leverage={self.max_leverage}, drawdown={self.max_drawdown}, position={self.max_position}, daily_loss={self.max_daily_loss}, trade_frequency={self.max_trade_frequency}"
        )
