...
def create_test_data(periods=200):
    ...
    for i in range(len(df)):
        df.loc[df.index[i], "high"] = df.loc[df.index[i], "open"] * (
            1 + np.random.uniform(0, 0.02)
        )
        df.loc[df.index[i], "low"] = df.loc[df.index[i], "open"] * (
            1 - np.random.uniform(0, 0.02)
        )
...
def test_base_indicator():
    ...
    assert (
        "liquidity_direction" in signal
    ), "Signal should contain 'liquidity_direction' key"
...
def test_advanced_indicators():
    ...
    assert (
        "ml_rsi_prediction" in signal
    ), "Signal should contain 'ml_rsi_prediction' key"
    assert (
        "order_flow_imbalance" in signal
    ), "Signal should contain 'order_flow_imbalance' key"
...
def test_performance_metrics():
    ...
    assert (
        "total_win_rate_boost" in metrics
    ), "Metrics should contain 'total_win_rate_boost'"
    assert (
        "total_drawdown_reduction" in metrics
    ), "Metrics should contain 'total_drawdown_reduction'"
    assert (
        "accuracy_improvement_percentage" in metrics
    ), "Metrics should contain 'accuracy_improvement_percentage'"
    assert (
        "accuracy_multiplier" in metrics
    ), "Metrics should contain 'accuracy_multiplier'"

    if indicator.advanced_indicators_enabled:
        assert (
            metrics["accuracy_multiplier"] > 1.0
        ), "Advanced indicators should improve accuracy"
        print(
            f"✓ Advanced indicators improve accuracy by {metrics['accuracy_improvement_percentage']:.2f}%"
        )
    else:
        print(
            "⚠ Advanced indicators not available, skipping accuracy improvement check"
        )
...
def test_error_handling():
    ...
    assert (
        "signal" in signal
    ), "Signal should be generated even with invalid column names"
...
def main():
    ...
    if not ENHANCED_INDICATOR_AVAILABLE:
        ...
        print(
            "Please ensure all dependencies are installed and the import path is correct."
        )
        return 1
