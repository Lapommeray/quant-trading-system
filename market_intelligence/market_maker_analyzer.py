...
class MarketMakerAnalyzer:
    ...
    def __init__(
        self, sensitivity=0.75, lookback_periods=20, min_volume_threshold=100000
    ):
        """
    ...
    def update_order_book(self, symbol, order_book_data):
        ...
        if len(self.order_book_snapshots) > self.lookback_periods:
            self.order_book_snapshots = self.order_book_snapshots[
    ...
    def update_dark_pool_data(self, symbol, dark_pool_data):
        ...
        if len(self.dark_pool_data[symbol]) > self.lookback_periods:
            self.dark_pool_data[symbol] = self.dark_pool_data[symbol][
                -self.lookback_periods :
            ]
    ...
    def detect_spoofing(self, symbol):
        ...
        symbol_snapshots = [
            s for s in self.order_book_snapshots if s["symbol"] == symbol
        ]
        ...
        for i in range(1, len(symbol_snapshots)):
            ...
            for price in prev_snapshot.get("bids", {}).keys():
                if price in curr_snapshot.get("bids", {}):
                    ...
                    if (
                        prev_size > self.min_volume_threshold
                        and curr_size < prev_size * 0.5
                    ):
    ...
    def detect_iceberg_orders(self, symbol, trades_data):
        ...
        grouped = df.groupby(["price", "time_window"]).agg(
            {"volume": "sum", "timestamp": "count"}
        )
        ...
        for price, group in grouped.groupby("price"):
            if len(group) >= 3:  # At least 3 time windows with activity at this price
                if group["timestamp"].sum() >= 10:  # At least 10 trades
                    if (
                        group["volume"].sum() > self.min_volume_threshold
                    ):  # Significant volume
    ...
    def detect_layering(self, symbol):
        ...
        bid_prices = sorted(
            [float(p) for p in latest_snapshot.get("bids", {}).keys()], reverse=True
        )
    ...
    def analyze_dark_pool_activity(self, symbol):
        ...
        large_trades = [
            d
            for d in self.dark_pool_data[symbol]
            if d.get("volume", 0) > avg_trade_size * 3
        ]

        large_trade_volume = sum(d.get("volume", 0) for d in large_trades)
        large_trade_percentage = (
            large_trade_volume / total_volume if total_volume > 0 else 0
        )
    ...
    def detect_manipulation(self, symbol):
        ...
        manipulation_detected = (
            spoofing_result["spoofing_detected"]
            or layering_result["layering_detected"]
            or dark_pool_result["unusual_activity"]
        )
        ...
        if manipulation_detected:
            self.logger.warning(
                f"Manipulation detected for {symbol} with {confidence:.2%} confidence"
            )
    ...
    def get_detection_stats(self, symbol=None, lookback_days=7):
        ...
        if symbol:
            detections = [
                d for d in self.detection_history if d.get("symbol") == symbol
            ]
        else:
        ...
        recent_detections = [
            d
            for d in detections
            if d.get("timestamp", datetime.datetime.now()) > cutoff_time
        ]

        total_detections = len(recent_detections)
        spoofing_detections = sum(
            1
            for d in recent_detections
            if d.get("spoofing", {}).get("spoofing_detected", False)
        )
        layering_detections = sum(
            1
            for d in recent_detections
            if d.get("layering", {}).get("layering_detected", False)
        )
        dark_pool_detections = sum(
            1
            for d in recent_detections
            if d.get("dark_pool", {}).get("unusual_activity", False)
        )

        if recent_detections:
            avg_confidence = sum(
                d.get("confidence", 0) for d in recent_detections
