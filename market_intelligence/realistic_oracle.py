...
class RealisticOracle:
    ...
    def __init__(
        self, lookback_periods=30, prediction_horizon=5, confidence_threshold=0.7
    ):
        """
    ...
    def _generate_features(self, price_data):
        ...
        df["upper_shadow"] = df["high"] - df["close"].where(
            df["close"] >= df["open"], df["open"]
        )
        df["lower_shadow"] = (
            df["close"].where(df["close"] <= df["open"], df["open"]) - df["low"]
        )
    ...
    def train(self, price_data):
        ...
        try:
            ...
            self.feature_importance = dict(
                zip(X.columns, self.rf_model.feature_importances_)
            )
    ...
    def predict_next_move(self, symbol, price_data):
        ...
        if not self.is_trained:
            return {
                ...
                + datetime.timedelta(minutes=self.prediction_horizon),
                "certainty": 0.0,
                "direction": "UNKNOWN",
            }
        ...
        try:
            ...
            ensemble_pred = self.target_scaler.inverse_transform(
            ...
            model_agreement = 1 - abs(rf_pred_scaled[0] - gb_pred_scaled[0]) / max(
                abs(rf_pred_scaled[0]), abs(gb_pred_scaled[0])
            )
    ...
    def get_prediction_accuracy(self):
        ...
        for i, prediction in enumerate(self.prediction_history[:-1]):
            ...
            if (
                prediction["price"] is not None
                and self.prediction_history[i + 1]["price"] is not None
            ):
                squared_error = (
                    prediction["price"] - self.prediction_history[i + 1]["price"]
                ) ** 2
