...
class FractalPatternAnalyzer:
    ...
    def add_historical_pattern(self, symbol, price_data, outcome_data=None, label=None):
        ...
        if len(self.historical_patterns[symbol]) > self.max_patterns:
            self.historical_patterns[symbol] = self.historical_patterns[symbol][
                -self.max_patterns :
            ]
    ...
    def find_matching_fractal(self, symbol, current_data, outcome_length=20):
        ...
        """
        if (
            symbol not in self.historical_patterns
            or not self.historical_patterns[symbol]
        ):
            return {
                "match_found": False,
                "confidence": 0.0,
        ...
        if "outcome" in pattern:
            ...
            remaining_pattern = len(pattern["pattern"]) - (
                alignment_offset + len(current_pattern)
            )
        ...
        self.logger.info(
            f"Pattern match found for {symbol}: {result['pattern_id']} with {result['similarity']:.2f} similarity"
        )
    ...
    def get_match_statistics(self, symbol=None, lookback_days=7):
        ...
        recent_matches = [
            m
            for m in matches
            if m.get("timestamp", datetime.datetime.now()) > cutoff_time
        ]
        ...
        avg_similarity = (
            sum(m.get("similarity", 0) for m in recent_matches) / total_matches
        )

        up_predictions = sum(1 for m in recent_matches if m.get("next_move") == "UP")
        down_predictions = sum(
            1 for m in recent_matches if m.get("next_move") == "DOWN"
        )
        unknown_predictions = sum(
            1 for m in recent_matches if m.get("next_move") == "UNKNOWN"
        )
        ...
        try:
            ...
            for symbol, patterns in self.historical_patterns.items():
                ...
                for pattern in patterns:
                    ...
                    if "pattern" in serializable_pattern and isinstance(
                        serializable_pattern["pattern"], np.ndarray
                    ):
                        serializable_pattern["pattern"] = serializable_pattern[
                            "pattern"
                        ].tolist()

                    if (
                        "outcome" in serializable_pattern
                        and "data" in serializable_pattern["outcome"]
                        and isinstance(
                            serializable_pattern["outcome"]["data"], np.ndarray
                        )
                    ):
                        serializable_pattern["outcome"]["data"] = serializable_pattern[
                            "outcome"
                        ]["data"].tolist()

                    if "timestamp" in serializable_pattern and isinstance(
                        serializable_pattern["timestamp"], datetime.datetime
                    ):
                        serializable_pattern["timestamp"] = serializable_pattern[
                            "timestamp"
                        ].isoformat()
