...
class DarkPoolDNAScanner:
    ...
    def _extract_patterns(self, symbol, prints):
        ...
        pattern = {
            ...
            "expiry": (
                datetime.now() + timedelta(seconds=self.settings["pattern_expiry"])
            ).isoformat(),
            ...
            "significance": self._calculate_significance(
                volume_profile, price_levels, venue_distribution, side_imbalance
            ),
        }
    ...
    def _calculate_volume_profile(self, prints):
        ...
        volume_profile = {
            ...
            "peak_time": (
                max(time_buckets.items(), key=lambda x: x[1])[0]
                if time_buckets
                else None
            ),
            "peak_volume": max(time_buckets.values()) if time_buckets else 0,
            "volume_concentration": (
                max(time_buckets.values()) / total_volume if total_volume > 0 else 0
            ),
        }
    ...
    def _calculate_price_levels(self, prints):
        ...
        vwap = (
            sum(price * volume for price, volume in price_volumes) / total_volume
            if total_volume > 0
            else 0
        )
        ...
        price_levels = {
            "vwap": vwap,
            "min_price": (
                min(price for price, _ in price_volumes) if price_volumes else 0
            ),
            "max_price": (
                max(price for price, _ in price_volumes) if price_volumes else 0
            ),
            "price_buckets": price_buckets,
            "peak_bucket": (
                max(price_buckets.items(), key=lambda x: x[1])[0]
                if price_buckets
                else None
            ),
            "peak_volume": max(price_buckets.values()) if price_buckets else 0,
            "price_concentration": (
                max(price_buckets.values()) / total_volume if total_volume > 0 else 0
            ),
        }
    ...
    def _calculate_venue_distribution(self, prints):
        ...
        venue_distribution = {
            ...
            "dominant_venue": (
                max(venue_volumes.items(), key=lambda x: x[1])[0]
                if venue_volumes
                else None
            ),
            "dominant_share": (
                max(venue_volumes.values()) / total_volume if total_volume > 0 else 0
            ),
        }
    ...
    def _calculate_significance(
        self, volume_profile, price_levels, venue_distribution, side_imbalance
    ):
        """
    ...
    def _calculate_liquidity_map(self, symbol, patterns):
        ...
        liquidity_map = {
            ...
            "significance": (
                sum(p.get("significance", 0) for p in patterns) / len(patterns)
                if patterns
                else 0
            ),
        }
