...
class RealDataConnector:
    ...
    def _check_api_keys(self):
        ...
        for source in required_sources:
            if source not in self.api_keys:
                self.logger.warning(
                    f"API key for {source} not provided. Some features will be limited."
                )
    ...
    def get_onchain_crypto_data(self, symbol, metric="transfers", timeframe="1h"):
        ...
        if "glassnode" not in self.api_keys:
            self.logger.warning(
                "Glassnode API key not available. Using simulated data."
            )
        ...
        if cache_key in self.data_cache:
            ...
            if (
                cache_time and (datetime.now() - cache_time).total_seconds() < 300
            ):  # 5 minutes
    ...
    def get_earnings_sentiment(self, symbol):
        ...
        if cache_key in self.data_cache:
            cache_time = self.last_update.get(cache_key)
            if (
                cache_time and (datetime.now() - cache_time).total_seconds() < 86400
            ):  # 24 hours
                return self.data_cache[cache_key]
    ...
    def get_satellite_data(self, location_type, identifier):
        ...
        if "orbital_insight" not in self.api_keys:
            self.logger.warning(
                "Orbital Insight API key not available. Using simulated data."
            )
        ...
        try:
            ...
            params = {"location_type": location_type, "identifier": identifier}
    ...
    def _calculate_spoofing_score(self, data):
        ...
        canceled_orders = [
            order for order in large_orders if order.get("status") == "canceled"
        ]
        ...
        timestamps = [
            datetime.fromisoformat(order.get("timestamp", "2023-01-01T00:00:00"))
    ...
    def _calculate_sentiment_score(self, news_data):
        ...
        for item in news_items:
            ...
            positive_count = sum(
                1 for word in positive_keywords if word in headline or word in summary
            )
            negative_count = sum(
                1 for word in negative_keywords if word in headline or word in summary
            )
    ...
    def _get_simulated_data(self, data_type, identifier):
        ...
        """
        self.logger.warning(
            f"Using simulated {data_type} data for {identifier} - FOR DEMO PURPOSES ONLY"
        )
...
class ComplianceCheck:
    ...
    def pre_trade_check(self, ticker, data_source=None):
        ...
        """
        result = {"allowed": True, "limit_order_size": None, "warnings": []}
        ...
        if self._is_retail_frontrun(ticker):
            ...
            result["warnings"].append(
                "Potential front-running detected, order size limited"
            )

        if data_source and data_source.get("is_simulated", False):
            result["warnings"].append(
                "Using simulated data - not recommended for production"
            )
