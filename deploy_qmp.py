...
def main():
    ...
    compiler = MultiAssetCompiler(
        assets=["BTC", "ETH", "SPX", "GOLD"], data_feeds=data_feeds
    )
        ...
        try:
            ...
            for asset, data in signals.items():
                ritual_decisions[asset] = ritual.execute_ritual(
                    {"ohlcv": data_feeds[asset].get_ohlcv(), "symbol": asset}
                )
            ...
            for asset in signals:
                ...
                signals[asset]["final_decision"] = (
                    "BUY"
                    if "BUY" in signals[asset]["decision"]
                    and "BUY" in ritual_decisions[asset]["decision"]
                    else (
                        "SELL"
                        if "SELL" in signals[asset]["decision"]
                        and "SELL" in ritual_decisions[asset]["decision"]
                        else "HOLD"
                    )
                )
