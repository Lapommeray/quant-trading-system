...
class OnChainVolumeSpoofFilter:
    ...
    def _analyze_volume_patterns(self):
        ...
        for asset_id, data in self.asset_data.items():
            ...
            if "round_number_pct" in tx_patterns:
                pattern_metrics["round_number_score"] = min(
                    1.0,
                    tx_patterns["round_number_pct"]
                    / self.pattern_thresholds["round_number"],
                )

            if "repeating_pct" in tx_patterns:
                pattern_metrics["repeating_score"] = min(
                    1.0,
                    tx_patterns["repeating_pct"] / self.pattern_thresholds["repeating"],
                )

            if "wash_cycle_pct" in tx_patterns:
                pattern_metrics["wash_cycle_score"] = min(
                    1.0,
                    tx_patterns["wash_cycle_pct"]
                    / self.pattern_thresholds["wash_cycle"],
                )

            volume_distribution = data.get("volume_distribution", {})
            if "top10_pct" in volume_distribution:
                pattern_metrics["volume_concentration_score"] = volume_distribution[
                    "top10_pct"
                ]
            ...
            combined_spoof_score = (
                ...
                + pattern_metrics["volume_concentration_score"]
    ...
    def _update_flagged_assets(self):
        ...
        for asset_id, data in self.spoof_signals.items():
            ...
            if (
                signal["type"] in ["STRONG_AVOID", "AVOID"]
                ...
                self.logger.warning(
                    f"Asset flagged: {asset_id} with {signal['type']} signal (score: {signal['spoof_score']:.2f})"
                )
    ...
    def get_trading_signal(self, asset_id):
        ...
        """
        if (
            asset_id not in self.spoof_signals
            or "signal" not in self.spoof_signals[asset_id]
        ):
