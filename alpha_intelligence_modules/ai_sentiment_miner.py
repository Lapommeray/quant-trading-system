...
class AISentimentMiner:
    ...
    def _update_sentiment_data_internal(self):
        ...
        for symbol in self.tracked_symbols:
            ...
            avg_sentiment = np.mean(
                [data["score"] for source, data in self.sentiment_data[symbol].items()]
            )
            avg_volume = np.mean(
                [data["volume"] for source, data in self.sentiment_data[symbol].items()]
            )
    ...
    def _analyze_sentiment(self):
        ...
        for symbol, sources in self.sentiment_data.items():
            ...
            if (
                symbol in self.sentiment_history
                and len(self.sentiment_history[symbol]) > 5
            ):
                recent_sentiments = [
                    point["sentiment"] for point in self.sentiment_history[symbol][-5:]
                ]
    ...
    def get_trading_signal(self, symbol):
        ...
        """
        if (
            symbol not in self.sentiment_signals
            or "signal" not in self.sentiment_signals[symbol]
        ):
