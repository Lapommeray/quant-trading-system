...
def test_quantconnect_integration():
    ...
    try:
        ...
        assert hasattr(
            strategy, "algo_fingerprinter"
        ), "AlgoFingerprinter not initialized"
        assert hasattr(
            strategy, "lstm_predictor"
        ), "LSTMLiquidityPredictor not initialized"
        ...
        test_market_data = {
            ...
            "timestamps": [
                datetime.now() - timedelta(minutes=i) for i in range(150, 0, -1)
            ],
        ...
        print(
            f"✅ EntropyScanner: {entropy_result['signal']} (confidence: {entropy_result['confidence']:.3f})"
        )
        ...
        print(
            f"✅ AlgoFingerprinter: {algo_result['signal']} (confidence: {algo_result['confidence']:.3f})"
        )
        ...
        print(
            f"✅ LSTMLiquidityPredictor: {lstm_result['signal']} (confidence: {lstm_result['confidence']:.3f})"
        )
