    ...
    def _aggregate_results(self, results):
        ...
        for result in results:
            if "signed_return" not in result:
                result["signed_return"] = result.get("signal", 0) * result.get(
                    "price_change", 0
                )
        ...
        avg_profit = (
            np.mean([r["signed_return"] for r in winning_trades])
            if winning_trades
            else 0
        )
        avg_loss = (
            np.mean([r["signed_return"] for r in losing_trades]) if losing_trades else 0
        )
