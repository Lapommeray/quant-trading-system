...
class AuroraGateway:
            ...
            try:
                ...
                if self.algorithm:
                    self.algorithm.Debug(
                        f"Aurora Gateway: Error in {name} module - {e}"
                    )
    ...
    def fuse_signals(self):
        ...
        if signal_counts[final_signal] > 0:
            final_confidence = (
                confidence_sum[final_signal] / signal_counts[final_signal]
            )
        else:
        ...
        if self.algorithm:
            self.algorithm.Debug(
                f"Aurora Gateway: Fused signal {final_signal} with confidence {final_confidence:.2f}"
            )
