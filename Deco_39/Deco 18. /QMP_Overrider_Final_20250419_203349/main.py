...
from market_intelligence.heat_latency_pipeline import HeatLatencyPipeline
...
class QMPOverriderUnified(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        for symbol in self.symbols:
            ...
            regulatory_heat = RegulatoryHeatImprints(log_dir=os.path.join(self.DataFolder, "logs/regulatory_heat"))
            latency_fingerprints = TransactionalLatencyFingerprints(log_dir=os.path.join(self.DataFolder, "logs/latency_fingerprints"))
            heat_latency_pipeline = HeatLatencyPipeline(log_dir=os.path.join(self.DataFolder, "logs/heat_latency_pipeline"))
            ...
            self.symbol_data[symbol] = {
                ...
                "regulatory_heat": regulatory_heat,  # Regulatory Heat Imprints
                "latency_fingerprints": latency_fingerprints,  # Transactional Latency Fingerprints
                "heat_latency_pipeline": heat_latency_pipeline,  # Combined Heat-Latency Pipeline
                "last_signal": None,
                "position_size": 0.0,
    ...
    def CheckSignals(self):
        ...
        for symbol in self.symbols:
            ...
            transaction_data = {
                "timestamps": [self.Time.timestamp() - i * 0.001 for i in range(20)],  # 20 transactions in last 20ms
                "venues": ["NYSE", "DARK", "DARK", "NYSE", "CITADEL", "VIRTU", "NYSE", "DARK", "NYSE", "CITADEL"] * 2
            }
            
            heat_source_data = {
                "sec_filings": 0.7 if symbol in ["BTCUSD", "ETHUSD"] else 0.3,
                "blockchain_tagging": 0.8 if symbol in ["BTCUSD", "ETHUSD"] else 0.1,
                "fine_anomalies": 0.5,
                "whistleblower_signals": 0.2
            }
            
            market_intelligence = self.symbol_data[symbol]["heat_latency_pipeline"].analyze_symbol(
                str(symbol),
                transaction_data,
                heat_source_data
            )
            
            dark_pool_analysis = self.symbol_data[symbol]["heat_latency_pipeline"].detect_dark_pool_leakage_with_heat(
                str(symbol),
                transaction_data["timestamps"],
                transaction_data["venues"]
            )
            
            self.symbol_data[symbol]["qmp"].monitoring_results['market_intelligence'] = {
                'risk_score': market_intelligence.get('risk_score', 0.0),
                'risk_level': market_intelligence.get('risk_level', 'minimal'),
                'heat_score': market_intelligence.get('heat_analysis', {}).get('total_score', 0.0),
                'latency_anomalies': market_intelligence.get('latency_analysis', {}).get('anomaly_count', 0),
                'dark_pool_leakage': dark_pool_analysis.get('leakage_detected', False),
                'combined_score': dark_pool_analysis.get('combined_score', 0.0),
