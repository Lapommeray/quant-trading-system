...
class TestRLRewardTracker:
    ...
    def test_reward_normalization(self):
        ...
        reward_high = tracker.record_step(
            pnl_delta=100.0, realized_ig=1.0, drawdown=0.0
        )
        assert reward_high > 0.9

        tracker.reset()
        reward_low = tracker.record_step(
            pnl_delta=-100.0, realized_ig=0.0, drawdown=1.0
        )
...
class TestTradingEnvironment:
    ...
    def test_observation_bounds(self):
        ...
        assert all(env.observation_space.high <= 1) or any(
...
@pytest.mark.skipif(
    not OPENAI_GYM_AVAILABLE, reason="OpenAI gym is required for SB3 checks"
)
class TestRLEvolver:
    """Tests for RLEvolver"""
    ...
    def test_initialization(self):
        ...
        evolver = RLEvolver(
            checkpoint_path="models/test_checkpoint.zip", burn_in_steps=10
        )
