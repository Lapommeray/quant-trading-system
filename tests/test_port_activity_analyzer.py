...
from advanced_modules.port_activity_analyzer import (
...
class TestPortActivityAnalyzer(unittest.TestCase):
    ...
    def test_normal_operation(self):
        ...
        self.assertIsInstance(
            self.analyzer.assessments["JPN_TKY"].encrypted_score, bytes
        )
    ...
    def test_quantum_failure(self):
        ...
        with patch.object(
            self.analyzer.quantum_engine,
            "encrypt",
            side_effect=Exception("Quantum disruption"),
        ):
            result = self.analyzer.analyze({"TEST_PORT": {"cargo_volatility": 0.5}})
    ...
    def test_protocol_activation(self):
        ...
        with patch.object(
            self.analyzer.quantum_engine, "encrypt", side_effect=Exception("Test")
        ):
            self.analyzer.analyze(
                {"FAIL1": {"cargo_volatility": 0.1}, "FAIL2": {"cargo_volatility": 0.2}}
            )
