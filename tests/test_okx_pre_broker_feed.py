...
def _feed(registry, bus=None):
    return OKXPreBrokerFeed(
        OKXFeedConfig(
            ...
            channels=["trades", "books5", "tickers", "open-interest", "funding-rate", "liquidation-orders"],
...
def test_trade_frame_writes_ring_and_publishes_market_data():
    ...
    feed.handle_frame({
        ...
        "data": [{"instId": "BTC-USDT-SWAP", "tradeId": "1", "px": "109210.0",
                  "sz": "0.5", "side": "buy", "ts": "1722840000100"}],
    })
...
def test_sell_trade_side_is_negative():
    ...
    feed.handle_frame({
        ...
        "data": [{"instId": "ETH-USDT-SWAP", "tradeId": "2", "px": "3500.0",
                  "sz": "10.0", "side": "sell", "ts": "1722840000200"}],
    })
...
def test_whale_flow_and_cluster_detection():
    ...
    for i, px in enumerate(["109210.0", "109212.0", "109215.0"]):
        feed.handle_frame({
            "arg": {"channel": "trades", "instId": "BTC-USDT-SWAP"},
            "data": [{"instId": "BTC-USDT-SWAP", "tradeId": str(i), "px": px,
                      "sz": "0.5", "side": "buy", "ts": f"1722840000{100 + i}00"}],
        })
...
def test_book_frame_populates_best_bid_ask():
    ...
    feed.handle_frame({
        ...
        "data": [{"asks": [["109234.1", "1.02"], ["109235.0", "2.5"]],
...
def test_liquidation_and_funding_frames():
    ...
    feed.handle_frame({
        ...
        "data": [{"instId": "BTC-USDT-SWAP", "side": "sell", "posSide": "long",
                  "bkPx": "108900.0", "sz": "12.5", "type": "filled", "ts": "1722840000400"}],
    })
    ...
    feed.handle_frame({
        ...
        "data": [{"instId": "BTC-USDT-SWAP", "fundingRate": "0.0001",
                  "nextFundingRate": "0.00015", "fundingTime": "1722844800000",
                  "ts": "1722840000500"}],
    })
...
def test_oi_and_ticker_frames():
    ...
    feed.handle_frame({
        ...
        "data": [{"instId": "BTC-USDT-SWAP", "oi": "185234", "oiCcy": "1.69",
                  "ts": "1722840000600"}],
    })
    ...
    feed.handle_frame({
        ...
        "data": [{"instId": "BTC-USDT-SWAP", "last": "109210.0", "open24h": "105000.0",
                  "high24h": "110000.0", "low24h": "104500.0", "vol24h": "45231.0",
                  "ts": "1722840000700"}],
    })
