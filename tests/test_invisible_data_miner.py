...
pytestmark = pytest.mark.skip(
    reason="Requires QuantConnect-style runtime state not available in local CI"
)
...
class TestInvisibleDataMiner(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.valid_signals = {"DarkPoolFlow": 0.92, "IcebergDetect": 0.71}
        cls.invalid_signals = {"BadType": "invalid", "OutOfRange": 1.5}
    ...
    def test_encryption_failure(self, mock_encrypt):
        ...
        self.assertFalse(result)
        self.assertEqual(
            self.miner.encrypted_blobs["TestSignal"], b"STEALTH_FAILOVER_XMSS"
        )
        self.assertIn("ACTIVATING DARK FAILOVER PROTOCOL", str(cm.output))

    def test_failover_counting(self):
        with patch.object(
            self.miner.encryption_engine, "encrypt", side_effect=Exception("Test")
        ):
