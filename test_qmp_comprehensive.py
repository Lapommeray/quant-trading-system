...
def test_comprehensive_engine():
    ...
    print(
        f"   - Data range: {history_data['1m'].index[0]} to {history_data['1m'].index[-1]}"
    )
    ...
    for symbol in symbols:
        ...
        print(
            f"   {symbol}: {result['final_signal']} (conf: {result['confidence']:.3f}, time: {end_time - start_time:.3f}s)"
        )
    ...
    successful_modules = sum(
        1 for result in module_test_results.values() if result.startswith("✅")
    )
    ...
    print(
        f"   ✅ Signal generation: {'Working' if any(r['signal'] for r in symbol_results.values()) else 'No signals generated'}"
    )
    print(
        f"   ✅ Performance: Average {np.mean([r['time'] for r in symbol_results.values()]):.3f}s per signal"
    )
