...
class BlackSwanDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized BlackSwanDetector with kurtosis threshold: {kurtosis_threshold}"
        )
    ...
    def _load_volcanic_data(self):
            ...
            "active_volcanoes": [
                {
                    ...
                    "location": "Hawaii",
                    "vei": 1,
                    "last_eruption": "2023-05-01",
                },
                {
                    "name": "Etna",
                    "location": "Italy",
                    "vei": 2,
                    "last_eruption": "2023-06-15",
                },
                {
                    "name": "Fuego",
                    "location": "Guatemala",
                    "vei": 3,
                    "last_eruption": "2023-03-10",
                },
                {
                    "name": "Merapi",
                    "location": "Indonesia",
                    "vei": 3,
                    "last_eruption": "2023-04-22",
                },
                {
                    "name": "Taal",
                    "location": "Philippines",
                    "vei": 2,
                    "last_eruption": "2022-12-05",
                },
            ],
            "historical_impacts": {
        ...
        logger.info(
            f"Loaded volcanic data with {len(volcanic_data['active_volcanoes'])} active volcanoes"
        )
    ...
    def detect(self, price_series, window=20, check_geo_events=True):
        ...
        is_black_swan = (
            kurt > self.kurtosis_threshold or sigma_move > self.volatility_multiplier
        )

        confidence = min(
            1.0,
            max(
                kurt / self.kurtosis_threshold, sigma_move / self.volatility_multiplier
            ),
        )
            ...
            for volcano in self.volcanic_data["active_volcanoes"]:
                ...
                if vei >= 3:
                    ...
                    if threshold_key in self.volcanic_data["risk_thresholds"]:
                        geo_risk_factor += self.volcanic_data["risk_thresholds"][
                            threshold_key
                        ]

            if geo_risk_factor > 0:
                logger.info(
                    f"Volcanic activity detected: +{geo_risk_factor:.2f} risk factor"
                )
        ...
        if check_geo_events and self.monitor_solar_flares:
            ...
            if solar_status["flare_detected"]:
                ...
                logger.warning(
                    f"Solar flare detected: {solar_status['flare_class']} "
        ...
        if total_risk_factor > 0:
            ...
            if sigma_move > adjusted_threshold:
                ...
                confidence = max(
                    confidence,
                    total_risk_factor + (sigma_move / self.volatility_multiplier),
                )

                logger.warning(
                    f"Black swan threshold adjusted due to external factors: "
        ...
        if is_black_swan:
            ...
            logger.warning(
                f"BLACK SWAN DETECTED: {sigma_move:.1f}σ move, "
...
def main():
    ...
    parser.add_argument(
        "--kurtosis", type=float, default=8.0, help="Kurtosis threshold for detection"
    )
    ...
    detector = BlackSwanDetector(
        kurtosis_threshold=args.kurtosis, volatility_multiplier=args.volatility
    )
