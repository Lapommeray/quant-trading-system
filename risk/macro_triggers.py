...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("macro_triggers.log")],
)
    ...
    def _calculate_spreads(self) -> None:
        ...
        try:
            for short_tenor, long_tenor in self.key_spreads:
                ...
                if (
                    self.yield_data[short_tenor]["current"] is not None
                    and self.yield_data[long_tenor]["current"] is not None
                ):

                    spread = (
                        self.yield_data[long_tenor]["current"]
                        - self.yield_data[short_tenor]["current"]
                    )
                    ...
                    self.spreads[spread_key]["history"].append(
                        {"timestamp": timestamp, "spread": spread}
                    )

                    if len(self.spreads[spread_key]["history"]) > 100:
                        self.spreads[spread_key]["history"] = self.spreads[spread_key][
                            "history"
                        ][-100:]
                ...
                if self.spreads[spread_key]["current"] is not None:
                    ...
                    if is_inverted:
                        if not self.spreads[spread_key]["inversion"]:
                            ...
                            inversion_start = self.spreads[spread_key][
                                "inversion_start"
                            ]
                            duration = (
                                datetime.now() - inversion_start
                            ).total_seconds() / 86400  # days

                            self.spreads[spread_key]["inversion_days"] = duration

                            if self.inversion_history[spread_key]:
                                current_inversion = self.inversion_history[spread_key][
                                    -1
                                ]
                        ...
                        if self.spreads[spread_key]["inversion"]:
                            ...
                            if self.inversion_history[spread_key]:
                                ...
                                logger.info(
                                    f"Yield curve inversion resolved: {spread_key}, duration: {current_inversion['duration_days']:.2f} days"
                                )
    ...
    def _generate_exit_signals(self) -> None:
        ...
        try:
            for short_tenor, long_tenor in self.key_spreads:
                ...
                if (
                    self.spreads[spread_key]["inversion"]
                    and self.spreads[spread_key]["inversion_days"]
                    >= self.inversion_threshold_days
                ):
                    ...
                    if spread_key not in self.active_exit_signals:
                        signal = {
                            ...
                            "inversion_days": self.spreads[spread_key][
                                "inversion_days"
                            ],
                            "timestamp": datetime.now(),
                            "status": "active",
                        ...
                        logger.warning(
                            f"Bond exit signal generated: {spread_key}, inversion: {self.spreads[spread_key]['inversion_days']:.2f} days"
                        )
...
class MacroRiskMonitor:
    ...
    def __init__(self, check_interval_hours: int = 6, risk_threshold: float = 0.7):
        """
    ...
    def _update_risk_indicators(self) -> None:
        ...
        try:
            ...
            inversion_count = sum(
                1 for status in inversion_status.values() if status["inverted"]
            )
            max_duration = max(
                (status["duration_days"] for status in inversion_status.values()),
                default=0,
            )

            yield_curve_risk = min(
                1.0,
                (inversion_count / len(inversion_status)) * 0.5
                + (max_duration / 30) * 0.5,
            )
            ...
            vix_risk = min(
                1.0, max(0.0, vix_value / self.risk_indicators["vix"]["threshold"])
            )
            ...
            credit_risk = min(
                ...
                max(
                    0.0,
                    credit_spread / self.risk_indicators["credit_spreads"]["threshold"],
                ),
            )
            ...
            liquidity_risk = min(
                ...
                max(
                    0.0,
                    liquidity_value / self.risk_indicators["liquidity"]["threshold"],
                ),
            )
        ...
        try:
            if self.overall_risk >= self.risk_threshold:
                if "high_risk" not in self.active_risk_signals:
                    ...
                    logger.warning(
                        f"High risk signal generated: {self.overall_risk:.4f}"
                    )
    ...
    def get_active_risk_signals(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        yield_curve_signals = self.risk_indicators["yield_curve"][
            "monitor"
        ].get_active_exit_signals()
