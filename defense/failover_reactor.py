...
class FailoverSystem:
    ...
    def _activate_backup(self, module_name):
        ...
        self.logger.warning(
            f"Activating backup for module {module_name}: {backup_path}"
        )
        ...
        try:
            ...
            self.logger.error(
                f"Error activating backup for module {module_name}: {str(e)}"
            )
            return False
    ...
    def _deactivate_backup(self, module_name):
        ...
        try:
            ...
            self.logger.error(
                f"Error deactivating backup for module {module_name}: {str(e)}"
            )
            return False
