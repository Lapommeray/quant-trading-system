...
class MomentumRideEngine:
    ...
    def _detect_parabolic_events(self, symbol):
        ...
        if len(returns) >= self.volatility_window:
            ...
            is_parabolic = (
                z_score > self.std_dev_threshold
            ...
            if is_parabolic:
                if not self.parabolic_events[symbol]["is_parabolic"]:
                    ...
                    self.parabolic_events[symbol]["magnitude"] = max(
                        self.parabolic_events[symbol]["magnitude"], z_score
                    )
                    self.parabolic_events[symbol]["volume_surge"] = max(
                        self.parabolic_events[symbol]["volume_surge"], volume_ratio
                    )
            else:
                self.parabolic_events[symbol] = {
                    "is_parabolic": False,
    ...
    def _manage_active_rides(self, symbol, current_price, current_time):
        ...
        if symbol in self.active_rides:
            ...
            ).total_seconds() / 60.0  # Duration in minutes
            ...
            if (
                not self.parabolic_events[symbol]["is_parabolic"]
                and ride["return"] > 0.1
            ):
                should_exit = True
            ...
            self.parabolic_events[symbol]["is_parabolic"]
            and self.momentum_scores[symbol]["combined"] > 0
        ):
    ...
    def _generate_signal(self, symbol):
        ...
        """
        if (
            symbol not in self.momentum_scores
            or self.momentum_scores[symbol]["combined"] == 0.0
        ):
            return {
                "direction": "NEUTRAL",
                "confidence": 0.0,
        ...
        if is_parabolic and short_score > 0.5:
            return {
                ...
                "confidence": min(
                    1.0, self.parabolic_events[symbol]["magnitude"] / 5.0
                ),
