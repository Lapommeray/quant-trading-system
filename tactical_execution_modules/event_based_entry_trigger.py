    ...
    def _predict_surprises(self):
        ...
        for event_type, sentiment in self.sentiment_data.items():
            ...
            surprise_direction = (
                ...
                else "NEGATIVE" if avg_sentiment < 0.45 else "NEUTRAL"
            )
            ...
            confidence = min(
                1.0, (0.5 + abs(sentiment_change) * 2.0) * (num_articles / 5.0)
            )
                ...
                if prediction["confidence"] > 0.6 and event_importance > 0.6:
                    ...
                    if trigger_id not in self.active_triggers:
                        ...
                        self.active_triggers[trigger_id] = {
                            ...
                            "status": (
                                "PENDING" if trigger_time > current_time else "ACTIVE"
                            ),
                        }
            ...
            if trigger["status"] == "ACTIVE":
                ...
                if trigger_time <= current_time:
                    signal = {
                        ...
                        "direction": self._convert_direction_to_signal(
                            trigger["direction"]
                        ),
