    ...
    def is_aligned(self, direction=None):
        ...
        if self.algorithm:
            self.algorithm.Debug(
                f"Ritual Lock: {'Aligned' if is_aligned else 'Misaligned'}"
            )
            self.algorithm.Debug(
                f"Moon Phase: {self.moon_phase:.2f}, Mercury Retrograde: {self.mercury_retrograde}"
            )
            self.algorithm.Debug(
                f"Geomagnetic Storm: {self.geomagnetic_storm}, Solar Activity: {self.solar_activity:.1f}"
            )
