...
FALLBACK_SIGNAL_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "mt5_signals"
)
...
class MT5BridgeConfig:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.signal_dir = (
            configured_dir if configured_dir else self._get_default_signal_dir()
        )
        ...
        self.separate_timeframe_files = config.get(
    ...
    def _check_circuit_breaker(self) -> bool:
        ...
        # Prune old entries
        self._recent_signal_times = [t for t in self._recent_signal_times if t > cutoff]
...
def init_bridge(config: Optional[Dict[str, Any]] = None) -> MT5BridgeConfig:
    ...
    if _bridge_config.enabled:
        try:
            ...
            logger.info(
                f"MT5 Bridge initialized. Signal directory: {_bridge_config.signal_dir}"
            )
...
def write_signal_atomic(signal_dict: Dict[str, Any]) -> bool:
    ...
    if config.separate_timeframe_files and timeframe:
        signal_file = os.path.join(
            config.signal_dir, f"{safe_symbol}_{timeframe}_signal.json"
        )
    else:
    ...
    with _write_lock:
        try:
            ...
            fd, temp_path = tempfile.mkstemp(
                suffix=".json", prefix=f"{safe_symbol}_", dir=config.signal_dir
            )
...
def write_signal_output(signal_dict: Dict[str, Any]) -> bool:
    ...
    with _write_lock:
        try:
            ...
            # Write to temporary file first
            fd, temp_path = tempfile.mkstemp(
                suffix=".json", prefix="signal_output_", dir=config.signal_dir
            )
