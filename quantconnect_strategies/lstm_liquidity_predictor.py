...
class LSTMLiquidityPredictor:
    ...
    def _simplified_lstm_prediction(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            vols = (
                np.array(volumes[-self.sequence_length :])
                if len(volumes) >= self.sequence_length
                else np.ones(self.sequence_length)
            )
            ...
            volume_ratio = (
                (np.mean(vols[-3:]) / np.mean(vols[-self.sequence_length :]))
                if len(vols) >= 3
                else 1.0
            )
