...
class AlgoFingerprinter:
        ...
        try:
            if self.dbscan and SKLEARN_AVAILABLE:
                ...
                if len(labels) > 0 and labels[-1] != -1:
                    ...
                    cluster_ratio = (
                        unique_labels / len(labels) if len(labels) > 0 else 0
                    )

                    last_feature = features[-1]
                    signal = (
                        "SELL"
                        if last_feature[0] > np.median([f[0] for f in features])
                        else "BUY"
                    )
    ...
    def _simple_pattern_detection(self, features: List[List[float]]) -> Dict[str, Any]:
        ...
        if abs(volume_trend) > np.std(volumes) and abs(price_trend) > np.std(
            price_devs
        ):
