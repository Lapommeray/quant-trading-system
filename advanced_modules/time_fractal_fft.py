...
class TimeFractalFFT:
    ...
    def _find_dominant_cycles(
        ...
        valid_mask = (cycle_lengths >= self.config.min_cycle_length) & (
            cycle_lengths <= self.config.max_cycle_length
        )
        ...
        peaks, properties = find_peaks(
            valid_power, height=np.max(valid_power) * self.config.significance_threshold
        )
    ...
    def _analyze_phases(
        self, fft_result, frequencies: np.ndarray, dominant_cycles: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _predict_turning_point(
        self, dominant_cycles: List[Dict], phase_info: Dict
    ...
    def _calculate_confidence(
        self, dominant_cycles: List[Dict], cycle_strength: float
    ) -> float:
        ...
        cycle_consistency = len(
            [c for c in dominant_cycles if c["relative_strength"] > 0.3]
        )
        consistency_factor = min(1.0, cycle_consistency / 3.0)

        confidence = (
            strongest_cycle_power * 0.6
            + cycle_strength * 0.3
    ...
    def get_cycle_summary(self) -> Dict[str, Any]:
        ...
        for history in self.cycle_history[-5:]:
            if history["dominant_cycles"]:
                dominant_cycle_lengths.append(
                    history["dominant_cycles"][0]["cycle_length"]
                )

        stable_cycles = (
            len(set(np.round(dominant_cycle_lengths))) <= 2
            if dominant_cycle_lengths
            else False
        )
        ...
        return {
            ...
            "dominant_cycle_length": (
                dominant_cycle_lengths[-1] if dominant_cycle_lengths else None
            ),
    ...
    def predict_price_direction(self, prices: np.ndarray) -> Dict[str, Any]:
        ...
        confidence = min(
            0.9, cycle_analysis["confidence"] * turning_point["confidence"]
        )
