...
class SelfDestructProtocol:
    ...
    def __init__(self, algorithm):
        ...
        self.thresholds = {
            ...
            "consecutive_losses": {"warning": 3, "danger": 5, "critical": 7},
            "win_rate": {
                "warning": 0.4,  # 40% win rate
                "danger": 0.3,  # 30% win rate
                "critical": 0.2,  # 20% win rate
            },
            "profit_factor": {"warning": 0.9, "danger": 0.7, "critical": 0.5},
        }
    ...
    def update_performance(self, strategy_name, module_name, symbol, trade_result):
        ...
        self._update_performance_record(
            self.strategy_performance[strategy_name], is_win, profit_loss
        )

        self._update_performance_record(
            self.module_performance[module_name], is_win, profit_loss
        )

        self._update_performance_record(
            self.symbol_performance[symbol_str], is_win, profit_loss
        )
    ...
    def _update_performance_record(self, record, is_win, profit_loss):
        ...
        record["win_rate"] = (
            record["wins"] / record["total_trades"]
        ...
        if record["profit_loss"] < record["max_profit"]:
            current_drawdown = (
                (record["max_profit"] - record["profit_loss"]) / record["max_profit"]
    ...
    def _check_self_destruct_conditions(self, strategy_name, module_name, symbol_str):
        ...
        if strategy_name not in self.isolated_strategies:
            ...
            if self._should_isolate(strategy_record):
                ...
                self.logger.warning(
                    f"Strategy {strategy_name} isolated due to poor performance"
                )
                isolated = True
                ...
                self.isolation_log.append(
                    {
                        ...
                        "metrics": {
                            k: v
                            for k, v in strategy_record.items()
                            if k != "trade_history"
                        },
                    }
                )
        ...
        if module_name not in self.isolated_modules:
            ...
            if self._should_isolate(module_record):
                ...
                self.logger.warning(
                    f"Module {module_name} isolated due to poor performance"
                )
                isolated = True
                ...
                self.isolation_log.append(
                    {
                        ...
                        "metrics": {
                            k: v
                            for k, v in module_record.items()
                            if k != "trade_history"
                        },
                    }
                )
        ...
        if symbol_str not in self.isolated_symbols:
            ...
            if self._should_isolate(symbol_record):
                ...
                self.logger.warning(
                    f"Symbol {symbol_str} isolated due to poor performance"
                )
                isolated = True
                ...
                self.isolation_log.append(
                    {
                        ...
                        "metrics": {
                            k: v
                            for k, v in symbol_record.items()
                            if k != "trade_history"
                        },
                    }
                )
    ...
    def _should_isolate(self, record):
        ...
        if (
            ...
            >= self.thresholds["consecutive_losses"]["critical"]
        ):
            return True

        if (
            record["win_rate"] <= self.thresholds["win_rate"]["critical"]
            and record["total_trades"] >= 10
        ):
            return True

        if (
            record["profit_factor"] <= self.thresholds["profit_factor"]["critical"]
            and record["total_trades"] >= 10
        ):
            return True
    ...
    def _get_isolation_reason(self, record):
        ...
        if (
            ...
            reasons.append(
                f"Too many consecutive losses ({record['consecutive_losses']})"
            )
        ...
        for strategy_name, isolation_info in list(self.isolated_strategies.items()):
            if self._can_recover(isolation_info):
                ...
                self.strategy_performance[strategy_name] = (
                    self._create_performance_record()
                )
                    ...
                    self.recovery_log.append(
                        {
                            ...
                            "isolation_duration": (
                                current_time - isolation_info["isolation_time"]
                            ).total_seconds()
                            / 3600,
    ...
    def _can_recover(self, isolation_info):
        ...
        if isolation_info["recovery_attempts"] > 0:
            required_idle_time = self.recovery_conditions["min_idle_time"] * (
                2 ** isolation_info["recovery_attempts"]
            )
    ...
    def get_performance_metrics(
        self, strategy_name=None, module_name=None, symbol=None
    ):
        """
        ...
        if strategy_name:
            if strategy_name in self.strategy_performance:
                metrics["strategy"] = {
                    k: v
                    for k, v in self.strategy_performance[strategy_name].items()
                    if k != "trade_history"
                }
        ...
        if module_name:
            if module_name in self.module_performance:
                metrics["module"] = {
                    k: v
                    for k, v in self.module_performance[module_name].items()
                    if k != "trade_history"
                }
        ...
        if symbol:
            ...
            if symbol_str in self.symbol_performance:
                metrics["symbol"] = {
                    k: v
                    for k, v in self.symbol_performance[symbol_str].items()
                    if k != "trade_history"
                }
    ...
    def get_isolation_status(self):
        ...
        return {
            ...
            "total_isolated": len(self.isolated_strategies)
            + len(self.isolated_modules)
            + len(self.isolated_symbols),
        }
    ...
    def record_trade_result(self, symbol, result, trade_data=None):
        ...
        trade_result = {
            ...
            "entry_time": trade_data.get(
                "entry_time", self.algorithm.Time - timedelta(minutes=15)
            ),
    ...
    def check_isolation_criteria(self):
        ...
        for strategy_name, record in self.strategy_performance.items():
            if strategy_name not in self.isolated_strategies and self._should_isolate(
                record
            ):
        ...
        for module_name, record in self.module_performance.items():
            if module_name not in self.isolated_modules and self._should_isolate(
                record
            ):
                reason = self._get_isolation_reason(record)
                updates.append(
                ...
                updates.append(
                    {
                        ...
                        "target_type": "strategy",
                        "isolation_duration": (
                            self.algorithm.Time - isolation_info["isolation_time"]
                        ).total_seconds()
                        / 3600,
                    }
                )
