...
class MetaAdaptiveAI:
    ...
    def __init__(self, algorithm, symbol=None):
        ...
        self.logger = logging.getLogger(
            f"MetaAdaptiveAI_{symbol}" if symbol else "MetaAdaptiveAI"
        )
        ...
        self.model_path = os.path.join(
            ...
            "data",
            (
                f"meta_adaptive_model_{symbol}.pkl"
                if symbol
                else "meta_adaptive_model.pkl"
            ),
        )
        ...
        algorithm.Debug(
            f"Meta-Adaptive AI module initialized for {symbol}"
            if symbol
            else "Meta-Adaptive AI module initialized"
        )
    ...
    def predict(self, features):
        ...
        if (
            len(available_features) < len(feature_list) * 0.7
        ):  # Need at least 70% of features
            return {"signal": "NEUTRAL", "confidence": 0.0, "model": None}
        ...
        try:
            ...
            if len(probas) >= 3:  # Multi-class (BUY, SELL, NEUTRAL)
                ...
                if confidence > 0.5 + (self.confidence_threshold - 0.5) / 2:
                    ...
                    signal = "NEUTRAL"
                    confidence = (
                        1.0 - abs(confidence - 0.5) * 2
                    )  # Rescale confidence for NEUTRAL
        ...
        if best_model:
            ...
            if best_model in ["forest", "boost"]:
                self.feature_importance = dict(
                    zip(
                        available_features, self.models[best_model].feature_importances_
                    )
                )
    ...
    def add_training_sample(self, features, target):
        ...
        feature_list = self.feature_sets[self.current_feature_set]
        available_features = {
            f: features.get(f, 0) for f in feature_list if f in features
        }
        ...
        if self.training_data is None or len(self.training_data) == 0:
            ...
            self.training_data = pd.concat(
                [self.training_data, sample_df], ignore_index=True
            )
    ...
    def _check_evolution(self):
        ...
        if (
            self.last_evolution_check is not None
            and current_time - self.last_evolution_check < self.evolution_check_interval
        ):
            return False
        ...
        for model_name, history in self.performance_history.items():
            if len(history) >= 5:
                recent_performance[model_name] = (
                    sum(h["accuracy"] for h in history[-5:]) / 5
                )
