...
class SentimentEnergyCouplingEngine:
                ...
                for i in range(len(df)):
                    if df["volatility"].iloc[i] > high_vol_threshold:
                        if current_zone is None:
                            ...
                            current_zone["max_volatility"] = max(
                                current_zone["max_volatility"],
                                float(df["volatility"].iloc[i]),
                            )
                            current_zone["price_range"][0] = min(
                                current_zone["price_range"][0], float(df["low"].iloc[i])
                            )
                            current_zone["price_range"][1] = max(
                                current_zone["price_range"][1],
                                float(df["high"].iloc[i]),
                            )
    ...
    def _couple_sentiment_with_volatility(
        self, sentiment_data: Dict[str, Any], volatility_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if (
            not sentiment_data
            or "sentiment_spikes" not in sentiment_data
            or not sentiment_data["sentiment_spikes"]
        ):
            return {"coupling_points": [], "confidence": 0.0}

        if (
            not volatility_data
            or "volatility_zones" not in volatility_data
            or not volatility_data["volatility_zones"]
        ):
            return {"coupling_points": [], "confidence": 0.0}
        ...
        for spike in sentiment_spikes:
            spike["datetime"] = datetime.fromisoformat(
                spike["timestamp"].replace("Z", "+00:00")
            )

        for zone in volatility_zones:
            zone["start_datetime"] = datetime.fromisoformat(
                zone["start_time"].replace("Z", "+00:00")
            )
            zone["end_datetime"] = datetime.fromisoformat(
                zone["end_time"].replace("Z", "+00:00")
            )
        ...
        coupling_points = sorted(
            coupling_points, key=lambda x: x["coupling_strength"], reverse=True
        )
        ...
        return {"coupling_points": coupling_points, "confidence": float(confidence)}
    ...
    def update_volatility_zones(self, symbol: str) -> None:
        ...
        if (
            symbol in self.volatility_zones
            and current_time - self.last_update < self.update_interval
        ):
            return
    ...
    def couple_sentiment_energy(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            coupling_results = self._couple_sentiment_with_volatility(
                self.sentiment_data[symbol], self.volatility_zones[symbol]
            )
    ...
    def generate_trading_signal(
        ...
        try:
            ...
            for point in coupling_points:
                spike_time = datetime.fromisoformat(
                    point["sentiment_spike"]["timestamp"].replace("Z", "+00:00")
                )
            ...
            if confidence >= self.confidence_threshold:
                return {
                    ...
                    "sentiment_direction": strongest_point["sentiment_spike"][
                        "direction"
                    ],
                    "time_to_volatility_hours": float(
                        strongest_point["time_diff_hours"]
                    ),
                    "timestamp": datetime.now().isoformat(),
                }
            else:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "sentiment_detection_accuracy": float(
                self.performance["sentiment_detection_accuracy"]
            ),
            "volatility_prediction_accuracy": float(
                self.performance["volatility_prediction_accuracy"]
            ),
