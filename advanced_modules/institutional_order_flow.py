...
class OrderFlowImbalanceDetector:
    ...
    def classify_tick(
        self, price: float, volume: float, bid: float, ask: float, prev_price: float
    ) -> OrderFlowTick:
    ...
    def process_tick(
        self, price: float, volume: float, bid: float, ask: float, timestamp: float = 0
    ...
    def _detect_imbalance(self) -> Optional[ImbalanceSignal]:
        ...
        sell_volume = sum(
            ...
            for t in recent_ticks
            if t.direction
            in [FlowDirection.AGGRESSIVE_SELL, FlowDirection.PASSIVE_SELL]
        )
    ...
    def _same_side(self, d1: FlowDirection, d2: FlowDirection) -> bool:
        ...
        return (d1 in buy_side and d2 in buy_side) or (
            d1 in sell_side and d2 in sell_side
        )
...
class AbsorptionClusterDetector:
    ...
    def _create_absorption_cluster(
        self, timestamp: float, price: float, volume: float
    ) -> AbsorptionCluster:
        ...
        logger.info(
            f"Absorption cluster detected at {price:.4f}, direction: {direction}"
        )
        return cluster
    ...
    def _create_exhaustion_cluster(
        self, timestamp: float, price: float, volume: float
    ) -> AbsorptionCluster:
        ...
        self.clusters.append(cluster)
        logger.info(
            f"Exhaustion cluster detected at {price:.4f}, reversal direction: {direction}"
        )
        return cluster
...
class VolumeProfileAnalyzer:
    ...
    def _calculate_value_area(
        self, volume_profile: np.ndarray, bins: np.ndarray
    ) -> Tuple[float, float]:
    ...
    def _find_high_volume_nodes(
        self, volume_profile: np.ndarray, bins: np.ndarray, threshold_pct: float = 0.8
    ) -> List[float]:
    ...
    def _find_low_volume_nodes(
        self, volume_profile: np.ndarray, bins: np.ndarray, threshold_pct: float = 0.2
    ) -> List[float]:
...
class InstitutionalOrderFlowAnalyzer:
        ...
        if "ohlcv" in market_data:
            ...
            if isinstance(ohlcv, pd.DataFrame) and len(ohlcv) > 0:
                ...
                volumes = (
                    ...
                    else ohlcv["Volume"].values
                )
                results["volume_profile"] = self.volume_analyzer.calculate_profile(
                    prices, volumes
                )
    ...
    def _calculate_institutional_score(self, results: Dict) -> float:
        ...
        if results["absorption_clusters"]:
            cluster_strength = np.mean(
                [c["strength"] for c in results["absorption_clusters"]]
            )
    ...
    def _generate_signal(self, results: Dict) -> Tuple[Optional[str], float]:
        ...
        if buy_count > sell_count:
            direction = "BUY"
            confidence = np.mean(
                [c for s, c in zip(signals, confidences) if s == "BUY"]
            )
        elif sell_count > buy_count:
            direction = "SELL"
            confidence = np.mean(
                [c for s, c in zip(signals, confidences) if s == "SELL"]
            )
        else:
            return None, 0.0
...
def main():
    ...
    if results["absorption_clusters"]:
        ...
        for cluster in results["absorption_clusters"]:
            print(
                f"  Price: {cluster['price_level']:.2f}, Direction: {cluster['direction']}"
            )
