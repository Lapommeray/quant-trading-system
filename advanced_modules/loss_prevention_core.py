...
try:
    from .bayesian_market_state import (
        BayesianMarketState,
        BeliefState,
        CommitmentAccountingSystem,
    )
...
class OnlineDrawdownPredictor:
    ...
    def _extract_features(self) -> Optional[Dict[str, float]]:
        ...
        return {
            ...
            "drawdown_trend": (
                drawdowns[-1] - drawdowns[-5] if len(drawdowns) >= 5 else 0
            ),
            "max_recent_drawdown": max(drawdowns[-10:]),
            "volatility": (
                np.std(returns[-20:]) if len(returns) >= 20 else np.std(returns)
            ),
    ...
    def predict(self, horizon_hours: float = 1.0) -> Tuple[float, float]:
        ...
        trend = (
            (current - self.drawdown_history[-5]) / 5
...
""",
    ...
    def analyze_weakness(
        self, performance_data: Dict, market_conditions: Dict
    ) -> List[str]:
        ...
        if (
            ...
            > market_conditions.get("avg_volatility", 0.01) * 1.5
        ):
        ...
        if (
            ...
            > market_conditions.get("avg_spread", 0.001) * 2
        ):
...
class LossPreventionCore:
    ...
    def _transition_state(self, new_state: TradingState, forecast: DrawdownForecast):
        ...
        logger.warning(
            f"Trading state transition: {old_state.value} -> {new_state.value}"
        )
    ...
    def force_state(self, state: TradingState, reason: str = "Manual override"):
        ...
        logger.warning(
            f"Forced state change: {old_state.value} -> {state.value} ({reason})"
        )
...
class InformationGainTradeEvaluator:
    ...
    def evaluate_trade(
        self,
        signal: Dict[str, Any],
        loss_prevention_core: Optional[LossPreventionCore] = None,
        ...
        if ig > self.ig_threshold and belief.confidence < self.confidence_threshold:
            ...
            result_signal["size"] = (
                signal.get("size", 0.1) * self.exploration_size_factor
            )
        ...
        if (
            belief.p_accept < self.min_p_accept
            and belief.confidence > self.confidence_threshold
        ):
            result_signal["mode"] = "rejected"
            result_signal["reason"] = (
                f"Confident rejection: p_accept={belief.p_accept:.3f}"
            )
            self.metrics["rejections"] += 1
        ...
        if self.commitment_accounting:
            can_commit = self.commitment_accounting.can_commit(
                result_signal, belief.p_accept
            )
            if not can_commit:
                result_signal["mode"] = "rejected"
