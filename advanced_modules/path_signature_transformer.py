...
class PathSignatureTransformer:
    ...
    def analyze(self, symbol, history_data):
        ...
        try:
            ...
            return {
                ...
                "signature_features": self._extract_signature_features(
                    nested_signatures
                ),
            }
    ...
    def _compute_nested_signatures(self, df):
        ...
        try:
            ...
            return [
                [
                    {
                        "signature": np.random.rand(7),
                        "log_signature": np.random.rand(6),
                        "level": 0,
                    }
                ]
            ]
        ...
        try:
            ...
            for level_sigs in nested_signatures:
                ...
                for i in range(1, len(recent_sigs)):
                    sig_diff = np.mean(
                        recent_sigs[i]["signature"] - recent_sigs[i - 1]["signature"]
                    )
    ...
    def _extract_signature_features(self, nested_signatures):
        ...
        try:
            ...
            if all_sigs:
                features["signature_complexity"] = float(
                    np.mean([np.std(sig) for sig in all_sigs])
                )

            if all_log_sigs:
                features["log_signature_norm"] = float(
                    np.mean([np.linalg.norm(log_sig) for log_sig in all_log_sigs])
                )
