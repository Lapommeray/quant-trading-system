...
class ShadowSpreadResonator:
    ...
    def __init__(self, algorithm=None):
        ...
        self.update_interval = timedelta(
            seconds=30
        )  # More frequent updates for micro anomalies
    ...
    def _calculate_micro_spread_metrics(
        ...
        volume_weighted_bid = (
            sum(float(bid[0]) * float(bid[1]) for bid in bids[:10]) / bid_volume_sum
            if bid_volume_sum > 0
            else best_bid
        )
        volume_weighted_ask = (
            sum(float(ask[0]) * float(ask[1]) for ask in asks[:10]) / ask_volume_sum
            if ask_volume_sum > 0
            else best_ask
        )
        ...
        if len(trades) >= 2:
            ...
            buy_volume = sum(
                vol for vol, side in zip(trade_volumes, trade_sides) if side == "buy"
            )
            sell_volume = sum(
                vol for vol, side in zip(trade_volumes, trade_sides) if side == "sell"
            )
            ...
            for i in range(1, len(trades)):
                if (
                    trades[i]["price"] >= best_ask
                    and trades[i - 1]["price"] <= best_bid
                ) or (
                    trades[i]["price"] <= best_bid
                    and trades[i - 1]["price"] >= best_ask
                ):
                    spread_crossings += 1
            ...
            for trade in trades:
                if trade["side"] == "buy":
                    effective_spread = (
                        (float(trade["price"]) - mid_price) / mid_price * 100
                    )
                else:
                    effective_spread = (
                        (mid_price - float(trade["price"])) / mid_price * 100
                    )

                effective_spreads.append(effective_spread)

            avg_effective_spread = (
                np.mean(effective_spreads) if effective_spreads else 0
            )

            spread_efficiency = (
                avg_effective_spread / spread_pct if spread_pct > 0 else 0
            )

            if len(trades) >= 20:
                if "datetime" in trades[0]:
                    trade_times = [
                        datetime.fromisoformat(trade["datetime"].replace("Z", "+00:00"))
                        for trade in trades
                    ...
                    if len(trade_times) >= 2:
                        ...
                        time_variability = (
                            np.std(time_diffs) / avg_time_between_trades
                            if avg_time_between_trades > 0
                            else 0
                        )
                    else:
                ...
                for i in range(2, len(trade_prices)):
                    if (
                        trade_prices[i] > trade_prices[i - 1]
                        and trade_prices[i - 1] < trade_prices[i - 2]
                    ) or (
                        trade_prices[i] < trade_prices[i - 1]
                        and trade_prices[i - 1] > trade_prices[i - 2]
                    ):
                ...
                price_reversal_rate = (
                    price_reversals / (len(trade_prices) - 2)
    ...
    def _detect_spread_anomalies(
        ...
        if not current_metrics or not historical_metrics:
            return {"anomalies_detected": 0, "confidence": 0.0, "direction": "NEUTRAL"}

        historical_values = {}
        for metric in current_metrics:
            historical_values[metric] = [
                h[metric] for h in historical_metrics if metric in h
            ]
        ...
        if anomalies:
            ...
            if "depth_imbalance" in anomalies:
                direction_signals.append(
                    1 if anomalies["depth_imbalance"]["direction"] == "up" else -1
                )

            if "trade_flow_imbalance" in anomalies:
                direction_signals.append(
                    1 if anomalies["trade_flow_imbalance"]["direction"] == "up" else -1
                )

            if "spread_efficiency" in anomalies:
                direction_signals.append(
                    1 if anomalies["spread_efficiency"]["direction"] == "up" else -1
                )
    ...
    def update_spread_metrics(self, symbol: str) -> None:
        ...
        if symbol not in self.spread_anomalies:
            self.spread_anomalies[symbol] = {
                ...
                - self.update_interval,  # Force update first time
                "anomalies": {},
            }

        if (
            current_time - self.spread_anomalies[symbol]["last_update"]
        ...
        if len(self.spread_anomalies[symbol]["historical_metrics"]) > max_history:
            self.spread_anomalies[symbol]["historical_metrics"] = self.spread_anomalies[
                symbol
            ]["historical_metrics"][-max_history:]
    ...
    def detect_resonance(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            anomalies = self._detect_spread_anomalies(
                symbol, current_metrics, previous_metrics
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "anomaly_detection_accuracy": float(
                self.performance["anomaly_detection_accuracy"]
            ),
