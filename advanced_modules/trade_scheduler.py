...
class UtilityFrontierScheduler:
    ...
    def __init__(
        ...
        logger.info(
            f"UtilityFrontierScheduler initialized: "
    ...
    def compute_utility(self, signal: Signal) -> ScheduledSignal:
        ...
        return ScheduledSignal(
            signal=signal, u_total=u_total, ig=ig, ucb_bonus=ucb, components=components
        )
    ...
    def add_candidate(self, signal: Signal) -> float:
        ...
        heapq.heappush(self.candidate_queue, (-scheduled.u_total, scheduled))
    ...
    def record_outcome(self, signal_id: str, realized_profit: float):
        ...
        if n > 0:
            self.metrics["avg_u_total"] = (
                self.metrics["avg_u_total"] * (n - 1) + realized_profit
            ) / n
    ...
    def update_lambdas(
        ...
        logger.info(
            f"Updated lambdas: IG={self.lambda_ig}, "
    ...
    def compute_regret(
        self, best_possible_utility: float, chosen_utility: float
    ) -> float:
        """
    ...
    def rank_candidates(
        self, candidates: List[Signal], top_k: int = 10
...
def demo():
    ...
    scheduler = UtilityFrontierScheduler(lambda_ig=0.2, lambda_risk=1.0, lambda_ucb=0.1)
    ...
    for signal in candidates:
        ...
        print(
            f"| {signal.id:8s} | {signal.expected_profit:9.2f} | "
