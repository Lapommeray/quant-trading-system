...
class LiquidityEventHorizonMapper:
    ...
    def _fetch_liquidation_data(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(1, len(trades)):
                ...
                price_change = (
                    abs(current_trade["price"] - prev_trade["price"])
                    / prev_trade["price"]
                )

                if (
                    current_trade["amount"] > np.mean([t["amount"] for t in trades]) * 3
                    and price_change > 0.001
                ):
                    liquidation = {
    ...
    def _detect_liquidity_zones(
        self, order_book: Dict[str, Any], liquidations: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        bid_peaks = self._find_peaks(
            bid_volume_profile, threshold=np.mean(bid_volume_profile) * 2
        )
        ask_peaks = self._find_peaks(
            ask_volume_profile, threshold=np.mean(ask_volume_profile) * 2
        )
            ...
            if peak_idx < len(bid_prices):
                zone = {
                    ...
                    "distance_from_vwap": float(
                        abs(bid_prices[peak_idx] - vwap_bids) / vwap_bids
                    ),
                }
        ...
        for peak_idx in ask_peaks:
            if peak_idx < len(ask_prices):
                zone = {
                    ...
                    "distance_from_vwap": float(
                        abs(ask_prices[peak_idx] - vwap_asks) / vwap_asks
                    ),
                }
        ...
        if liquidations:
            ...
            clusters = self._cluster_liquidations(
                liquidation_prices, liquidation_volumes
            )
            ...
            for cluster in clusters:
                ...
                zone = {
                    ...
                    "type": "liquidation",
                    "strength": float(
                        total_volume / np.mean(liquidation_volumes)
                        if liquidation_volumes
                        else 1.0
                    ),
                    "distance_from_vwap": float(
                        abs(avg_price - vwap_bids) / vwap_bids
                        if avg_price < vwap_bids
                        else abs(avg_price - vwap_asks) / vwap_asks
                    ),
                }
        ...
        liquidity_zones = sorted(
            liquidity_zones, key=lambda x: x["strength"], reverse=True
        )
    ...
    def _calculate_gravitational_pull(
        self, current_price: float, liquidity_zones: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for zone in liquidity_zones:
            ...
            if distance_pct > 0:
                ...
                pull = (
                    zone["strength"] * 100
                )  # Very high pull for zones at current price

            direction = "up" if zone["price"] > current_price else "down"

            time_to_impact = (
                distance_pct * 100 / (pull + 1e-10)
            )  # Rough estimate in arbitrary units
        ...
        gravitational_pulls = sorted(
            gravitational_pulls, key=lambda x: x["pull"], reverse=True
        )

        return gravitational_pulls

    def _detect_event_horizons(
        self, current_price: float, gravitational_pulls: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if net_up_pull > net_down_pull * 1.5:  # Significant upward bias
            ...
            dominant_pulls = gravitational_pulls[
                :3
            ]  # Top 3 pulls regardless of direction
        ...
        for pull in dominant_pulls:
            ...
            if pull["zone_type"] == "liquidation":
                confidence = min(
                    base_confidence + 0.1, 0.99
                )  # Boost for liquidation zones
            else:
    ...
    def update_liquidity_zones(self, symbol: str) -> None:
        ...
        if (
            symbol in self.liquidity_zones
        ...
        self.liquidity_zones[symbol] = {
            ...
            "zones": zones,
            "current_price": (
                float((order_book["bids"][0][0] + order_book["asks"][0][0]) / 2)
        ...
        self.logger.info(
            f"Updated liquidity zones for {symbol}: {len(zones)} zones detected"
        )
    ...
    def map_event_horizons(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            gravitational_pulls = self._calculate_gravitational_pull(
                current_price, zones
            )
            ...
            event_horizons = self._detect_event_horizons(
                current_price, gravitational_pulls
            )
    ...
    def map_liquidity(self, symbol: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            strongest_horizon = max(
                mapping["event_horizons"], key=lambda x: x["confidence"]
            )
            ...
            if (
                strongest_horizon["direction"] == "up"
                and confidence >= self.confidence_threshold
            ):
                signal = "BUY"
            elif (
                strongest_horizon["direction"] == "down"
                and confidence >= self.confidence_threshold
            ):
                signal = "SELL"
            ...
            if confidence >= self.confidence_threshold and signal in ["BUY", "SELL"]:
                return {
                    ...
                    "price_distance_pct": float(
                        abs(strongest_horizon["price"] - current_price)
                        / current_price
                        * 100
                    ),
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "zone_detection_accuracy": float(
                self.performance["zone_detection_accuracy"]
            ),
