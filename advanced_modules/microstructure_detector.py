...
class AbsorptionDetector:
    ...
    def update(
        self, volume: float, price_delta: float, depth_change: float = 0.0
    ) -> Tuple[str, float]:
        """
    ...
    def detect(self) -> Tuple[str, float]:
        ...
        recent_price_move = (
            np.abs(np.mean(prices[-5:])) if len(prices) >= 5 else abs(prices[-1])
        )
...
class InventoryFlipDetector:
    ...
    def detect(self) -> Tuple[str, str]:
        ...
        if (
            max_recent > self.flip_ratio_threshold
            and min_recent < 1 / self.flip_ratio_threshold
        ):
...
class DepthPressureAnalyzer:
    ...
    def analyze(self, snapshot: OrderBookSnapshot) -> Dict[str, float]:
        ...
        for i, (price, vol) in enumerate(
            zip(snapshot.bid_prices[: self.levels], snapshot.bid_volumes[: self.levels])
        ):
            distance = mid - price
        ...
        self.ask_pressure = 0.0
        for i, (price, vol) in enumerate(
            zip(snapshot.ask_prices[: self.levels], snapshot.ask_volumes[: self.levels])
        ):
            distance = price - mid
            weight = self.decay_factor**i
...
class MicrostructureDetectors:
    ...
    def update(self, event_data: Dict[str, Any]) -> MicrostructureFlags:
        ...
        pressure_metrics = {
            ...
            "ask_pressure": 0.0,
            "depth_imbalance": 0.0,
            "implied_inventory_direction": 0.0,
        }
...
def demo():
    ...
    print(
        f"Spoof detected: {flags.spoof_detected}, confidence: {flags.spoof_confidence:.2f}"
    )
    ...
    print(
        f"Absorption detected: {flags.absorption_detected}, confidence: {flags.absorption_confidence:.2f}"
    )
    print(f"Bayesian penalty: {flags.get_bayesian_penalty():.3f}")
    ...
    print(
        f"Inventory flip detected: {flags.inventory_flip_detected}, direction: {flags.flip_direction}"
    )
