...
def simulate_heston_paths(
    initial_state, times, kappa=1.0, theta=0.04, xi=0.2, rho=-0.7, dt=0.01
):
    """
...
class HestonModel(torch.nn.Module):
    ...
    def calibrate(
        self, market_data, initial_params=None, learning_rate=0.01, epochs=100
    ):
        """
        ...
        try:
            ...
            implied_vols = torch.tensor(
                market_data["implied_vol"].values, dtype=torch.float32
            )
            ...
            for epoch in range(epochs):
                initial_state = torch.tensor(
                    [[prices[0], implied_vols[0] ** 2]], dtype=torch.float32
                )
    ...
    def price_options(
        self, spot, strike, maturity, risk_free_rate=0.0, num_paths=10000
    ):
        """
        ...
        try:
            initial_state = torch.tensor(
                [[spot, self.theta]], dtype=torch.float32
            ).repeat(num_paths, 1)

            initial_state[:, 1] = torch.clamp(
                initial_state[:, 1] * (1 + 0.1 * torch.randn(num_paths)), min=1e-6
            )
            ...
            return {"call": call_price.item(), "put": put_price.item()}
        except Exception as e:
