...
class LimitOrderBook:
    ...
    def simulate_limit_order(self, price, volume, is_buy):
        ...
        if is_buy:
            ...
            average_price = (
                total_cost / executed_volume if executed_volume > 0 else price
            )
            ...
            average_price = (
                total_revenue / executed_volume if executed_volume > 0 else price
            )
            return executed_volume, average_price
...
class EnhancedLimitOrderBook(LimitOrderBook):
    ...
    def calculate_vpin(self, bucket_volume=10000, window=50):
        ...
        """
        if (
            not self.enable_institutional_features
            or len(self.buy_volumes) < window
            or len(self.sell_volumes) < window
        ):
            return np.nan
            ...
            snapshot.update(
                {
                    ...
                    "price_impact": (
                        np.mean(self.price_impact[-10:])
                        if len(self.price_impact) >= 10
                        else None
                    ),
                }
            )
