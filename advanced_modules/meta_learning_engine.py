    ...
    def __init__(self, algorithm):
        ...
        if TORCH_AVAILABLE:
            ...
            self.opt = torch.optim.SGD(
                self.fund_model.parameters(), lr=self.learning_rate
            )
        else:
            self.fund_model = torch.nn.Linear(self.input_size, self.output_size)
    ...
    def _extract_features(self, df):
        ...
        features = np.array(
            [
                ...
                np.mean(df["Volume"].values[-5:]) / np.mean(df["Volume"].values[-10:-5])
                - 1,
    ...
    def _meta_learn_prediction(self, features):
        ...
        try:
            if TORCH_AVAILABLE and hasattr(higher, "innerloop_ctx"):
                with higher.innerloop_ctx(self.fund_model, self.opt) as (
                    fmodel,
                    diffopt,
                ):
