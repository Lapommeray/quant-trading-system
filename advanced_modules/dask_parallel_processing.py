...
class DaskParallelProcessor:
    ...
    def parallel_optimization(self, objective_func, parameter_grid):
        ...
        if self.client is None:
            self.logger.warning(
                "Falling back to sequential processing for optimization"
            )
            results = []
            for params in parameter_grid:
                result = objective_func(**params)
                results.append({"parameters": params, "result": result})
    ...
    def parallel_dataframe_apply(self, df, func, axis=1, **kwargs):
        ...
        if self.client is None:
            self.logger.warning(
                "Falling back to sequential processing for DataFrame apply"
            )
        ...
        for chunk in chunks:
            future = self.client.submit(
                lambda df: df.apply(func, axis=axis, **kwargs), chunk
            )
            futures.append(future)
