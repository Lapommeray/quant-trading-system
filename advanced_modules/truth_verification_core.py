...
class TruthVerificationCore:
    ...
    def _extract_truth_patterns(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "ohlcv" in market_data and "1h" in market_data["ohlcv"]:
            ...
            if len(ohlcv) >= 24:
                ...
                truth_patterns["volume_profile"] = {
                    ...
                    "volume_cv": (
                        float(volume_std / volume_mean) if volume_mean > 0 else 0
                    ),
                }
        ...
        if "order_book" in market_data:
            ...
            if "bids" in order_book and "asks" in order_book:
                ...
                if bids and asks:
                    spread = (
                        (asks[0][0] - bids[0][0]) / bids[0][0] if bids[0][0] > 0 else 0
                    )
        ...
        if "ohlcv" in market_data and "1h" in market_data["ohlcv"]:
            ...
            if len(ohlcv) >= 24:
                ...
                if returns:
                    ...
                    return_kurtosis = (
                        stats.kurtosis(returns) if len(returns) >= 4 else 0
                    )
    ...
    def _detect_manipulation(
        self, truth_patterns: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if (
            "volume_profile" in truth_patterns
            ...
            if len(ohlcv) >= 3:
                ...
                if recent_mean > volume_truth["volume_mean"] * 3:
                    manipulation_signals["volume_spike"] = {
                        "severity": float(
                            min(recent_mean / volume_truth["volume_mean"] / 3, 1.0)
                        ),
                    ...
                    manipulation_signals["volume_dry_up"] = {
                        "severity": float(
                            min(
                                (volume_truth["volume_mean"] / recent_mean - 1) / 2, 1.0
                            )
                        ),
                ...
                if bids and asks:
                    ...
                    if spread > order_book_truth["spread"] * 3:
                        manipulation_signals["spread_widening"] = {
                            "severity": float(
                                min(spread / order_book_truth["spread"] / 3, 1.0)
                            ),
                        ...
                        manipulation_signals["order_book_imbalance"] = {
                            "severity": (
                                float(
                                    min(
                                        abs(imbalance)
                                        / abs(order_book_truth["imbalance"])
                                        / 3,
                                        1.0,
                                    )
                                )
                                if abs(order_book_truth["imbalance"]) > 0
                                else 0.5
                            ),
        ...
        if (
            "price_action" in truth_patterns
            and "ohlcv" in market_data
            and "1h" in market_data["ohlcv"]
        ):
                    ...
                    for ret in recent_returns:
                        if abs(ret) > price_truth["return_std"] * 3:
                            manipulation_signals["price_anomaly"] = {
                                "severity": float(
                                    min(abs(ret) / price_truth["return_std"] / 3, 1.0)
                                ),
        ...
        if "trade_flow" in truth_patterns and "trades" in market_data:
            ...
            if trades:
                recent_sizes = [
                    trade["amount"] for trade in trades[-10:] if "amount" in trade
                ]
                    ...
                    for size in recent_sizes:
                        if size > trade_truth["size_max"] * 2:
                            manipulation_signals["large_trade"] = {
                                "severity": float(
                                    min(size / trade_truth["size_max"] / 2, 1.0)
                                ),
    ...
    def _verify_trading_signal(
        self, signal: Dict[str, Any], manipulation_signals: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if manipulation_signals:
            ...
            severity_avg = (
                severity_sum / len(manipulation_signals) if manipulation_signals else 0
            )
            ...
            if manipulated:
                confidence = (
                    1.0 - (1.0 - severity_avg) * 0.5
                )  # Higher severity = higher confidence in manipulation
            else:
    ...
    def detect_market_manipulation(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            manipulation_signals = self._detect_manipulation(
                self.truth_patterns[symbol]["patterns"], market_data
            )
    ...
    def verify_signal(self, symbol: str, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            manipulation_signals = self.manipulation_signals.get(symbol, {}).get(
                "signals", {}
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "manipulation_detection_accuracy": float(
                self.performance["manipulation_detection_accuracy"]
            ),
            "false_signal_detection_rate": float(
                self.performance["false_signal_detection_rate"]
            ),
