...
class DarkPoolDNADecoder:
    ...
    def _extract_trade_features(self, df):
        ...
        try:
            ...
            for i in range(1, len(prices)):
                ...
                trades.append(
                    {"size": size, "time_gap": time_gap, "price_impact": price_impact}
                )

            return trades
        except Exception:
            return [
                {"size": 1.0, "time_gap": 1.0, "price_impact": 0.001} for _ in range(10)
            ]
    ...
    def _decode_algo_dna(self, trades):
        ...
        try:
            ...
            X = np.array(
                [[t["size"], t["time_gap"], t["price_impact"]] for t in trades]
            )
    ...
    def _analyze_algo_patterns(self, clusters, trades):
        ...
        try:
            ...
            for cluster_id in unique_clusters:
                ...
                cluster_trades = [
                    trades[i] for i in range(len(trades)) if cluster_mask[i]
                ]
                ...
                if cluster_trades:
                    ...
                    direction = (
                        1
                        if avg_impact > np.median([t["price_impact"] for t in trades])
                        else -1
                    )
            ...
            if cluster_directions:
                weighted_direction = np.average(
                    cluster_directions, weights=cluster_sizes
                )
                pattern_strength = (
                    1.0 - np.std(cluster_directions) / 2.0
                    if len(cluster_directions) > 1
                    else 0.8
                )
                cluster_consistency = (
                    len(unique_clusters) / len(trades) if len(trades) > 0 else 0.0
                )
            else:
