...
class QuantumExecutionOptimizer:
    ...
    def analyze(self, symbol, history_data):
        ...
        try:
            ...
            execution_quality = self._evaluate_execution_quality(
                optimal_execution, orders
            )
    ...
    def _quantum_slice_optimization(self, orders):
        ...
        try:
            if QISKIT_AVAILABLE:
                ...
                solution = self._solve_qubo_classical(
                    orders["sizes"], orders["avg_impact"]
                )
    ...
    def _evaluate_execution_quality(self, execution, orders):
        ...
        try:
            ...
            total_impact = sum(
                orders["impacts"][i]
                for i in execution["slices"]
                if i < len(orders["impacts"])
            )
            avg_impact = (
                total_impact / len(execution["slices"]) if execution["slices"] else 1.0
            )
