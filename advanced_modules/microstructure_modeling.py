...
class MicrostructureModeling:
    ...
    def simulate_limit_order_book(
        ...
        for i in range(1, n_steps):
            imbalance = (bid_volumes[i - 1] - ask_volumes[i - 1]) / (
                bid_volumes[i - 1] + ask_volumes[i - 1]
            )
            ...
            recent_vol = (
                np.std(np.diff(mid_prices[max(0, i - 100) : i])) if i > 100 else spread
            )
            dynamic_spread = max(
                float(self.tick_size), float(spread * (1 + recent_vol))
            )
            ...
            bid_volumes[i] = max(
                0, bid_volumes[i - 1] * 0.99 + np.random.exponential(100)
            )
            ask_volumes[i] = max(
                0, ask_volumes[i - 1] * 0.99 + np.random.exponential(100)
            )
    ...
    def analyze_order_book_imbalance(
        self, bids: Dict[float, float], asks: Dict[float, float], levels: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        if not bids or not asks:
            ...
            return {"imbalance": 0.0, "pressure": "neutral", "confidence": 0.5}
        ...
        weighted_bid_volume = sum(
            bids[price] * (1 - (mid_price - price) / mid_price) for price in bid_prices
        )
        weighted_ask_volume = sum(
            asks[price] * (1 - (price - mid_price) / mid_price) for price in ask_prices
        )
    ...
    def calculate_kyle_lambda(
        self, prices: np.ndarray, volumes: np.ndarray, window_size: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        if len(prices) != len(volumes):
            ...
            return {"kyle_lambda": 0.0, "confidence": 0.5, "r_squared": 0.0}

        if len(prices) < window_size + 1:
            logger.warning(
                f"Insufficient data for Kyle's lambda calculation. Need at least {window_size+1} points"
            )
            return {"kyle_lambda": 0.0, "confidence": 0.5, "r_squared": 0.0}
        ...
        for i in range(len(price_changes) - window_size + 1):
            ...
            if np.sum(window_signed_volumes**2) > 0:
                lambda_est = np.sum(
                    window_price_changes * window_signed_volumes
                ) / np.sum(window_signed_volumes**2)

                predictions = lambda_est * window_signed_volumes
                ss_total = np.sum(
                    (window_price_changes - np.mean(window_price_changes)) ** 2
                )
    ...
    def optimal_execution_strategy(
        ...
        risk_cost = (
            ...
            * risk_aversion
            * (vol_adjusted**2)
            * sum(remaining_shares[i] ** 2 * tau for i in range(n_periods))
        )
    ...
    def detect_spoofing_patterns(
        self, order_history: List[Dict[str, Any]], price_history: np.ndarray
    ) -> Dict[str, Any]:
        """
        ...
        if not order_history or len(order_history) < 10:
            ...
            return {"spoofing_detected": False, "confidence": 0.0, "pattern": None}

        large_orders = [
            order
            for order in order_history
            if order.get("volume", 0)
            > np.percentile([o.get("volume", 0) for o in order_history], 90)
        ]
        ...
        for i, order in enumerate(large_orders):
            if order.get("action") == "add":
                ...
                for j in range(i + 1, len(large_orders)):
                    if (
                        large_orders[j].get("action") == "cancel"
                        and large_orders[j].get("order_id") == order_id
                    ):

                        time_diff = large_orders[j].get("timestamp", 0) - order.get(
                            "timestamp", 0
                        )
                ...
                if i > 0 and price_history[i - 1] == cancel_time:
                    ...
                    if (cancel["order"].get("side") == "buy" and price_change > 0) or (
                        ...
                        price_movements.append(
                            {"cancellation": cancel, "price_change": price_change}
                        )
