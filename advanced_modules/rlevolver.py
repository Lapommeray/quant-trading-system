                ...
                class MockBox:
                    def __init__(self, low, high, shape, dtype):
                        self.low = (
                            ...
                            else np.full(shape, low)
                        )
                        self.high = (
                            np.array(high)
                            if hasattr(high, "__iter__")
                            else np.full(shape, high)
                        )
...
class RLObservation:
    ...
    def from_belief_state(
        ...
        return cls(
            ...
            mm_inventory_flip=(
                1.0 if mm_flags.get("inventory_flip_detected", False) else 0.0
            ),
...
class RLAction:
    ...
    def to_task_description(self) -> str:
        ...
        if abs(self.feature_weight_multiplier - 1.0) > 0.05:
            parts.append(
                f"adjust feature weights to {self.feature_weight_multiplier:.2f}x"
            )
        if abs(self.signal_confidence_multiplier - 1.0) > 0.05:
            parts.append(
                f"adjust signal confidence to {self.signal_confidence_multiplier:.2f}x"
            )
...
class RewardLoggingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.log_interval == 0:
            ...
            self.rewards.append(
                self.locals.get("rewards", [0])[0] if "rewards" in self.locals else 0
            )
        return True
...
class RLEvolver:
    ...
    def __init__(
        ...
        if REWARD_TRACKER_AVAILABLE:
            self.reward_tracker = RLRewardTracker(
                lambda_ig=lambda_ig, lambda_risk=lambda_risk
            )
        else:
            self.reward_tracker = None

        self.env = TradingEnvironment(
            reward_tracker=self.reward_tracker, step_interval_sec=step_interval_sec
        )

        self.policy_kwargs = policy_kwargs or {"net_arch": [256, 128, 64]}
        ...
        logger.info(
            f"RLEvolver initialized: checkpoint={checkpoint_path}, burn_in={burn_in_steps}"
        )
    ...
    def predict(
        self, observation: Optional[np.ndarray] = None, deterministic: bool = False
    ) -> RLAction:
        """
...
def demo():
    ...
    if not GYM_AVAILABLE or not SB3_AVAILABLE:
        ...
        evolver = RLEvolver(
            checkpoint_path="models/test_checkpoint.zip", burn_in_steps=100
        )
        ...
        evolver = RLEvolver(
            checkpoint_path="models/demo_checkpoint.zip", burn_in_steps=50
        )
