...
def volume_imbalance_func(bid_vol, ask_vol):
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        result = np.where(total > 1e-10, (bid_vol - ask_vol) / total, 0.0)
    return result
...
def momentum_func(current, previous):
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        result = np.where(
            np.abs(previous) > 1e-10, (current - previous) / np.abs(previous), 0.0
        )
    return result


if GPLEARN_AVAILABLE:
    micro_price = make_function(function=micro_price_func, name="micro_price", arity=4)

    volume_imbalance = make_function(
        function=volume_imbalance_func, name="vol_imbal", arity=2
    )

    weighted_mid = make_function(function=weighted_mid_func, name="wmid", arity=4)

    spread_ratio = make_function(
        function=spread_ratio_func, name="spread_ratio", arity=2
    )

    momentum = make_function(function=momentum_func, name="momentum", arity=2)

    protected_div = make_function(function=safe_div, name="pdiv", arity=2)

    protected_log = make_function(function=safe_log, name="plog", arity=1)

    protected_sqrt = make_function(function=safe_sqrt, name="psqrt", arity=1)
...
class FeatureEvolver:
    ...
    def evolve_features(
        ...
        include_trading_functions: bool = True,
    ) -> np.ndarray:
        """
        ...
        logger.info(
            f"Starting feature evolution with {self.generations} generations..."
        )
    ...
    def evolve_predictor(
        self, X: np.ndarray, y: np.ndarray, include_trading_functions: bool = True
    ) -> "SymbolicRegressor":
        """
    ...
    def get_best_features(self, n: int = 5) -> List[EvolvedFeature]:
        ...
        sorted_features = sorted(
            self.evolved_features, key=lambda f: f.fitness, reverse=True
        )
...
class AutonomousFeatureEvolver:
    ...
    def run_evolution_cycle(
        self, df, target_col: str = "returns"
    ) -> List[EvolvedFeature]:
        """
            ...
            try:
                ...
                for feature in evolver.evolved_features:
                    if feature.fitness >= self.min_fitness_threshold:
                        feature.name = (
                            f"auto_evolved_{self.cycle_count}_{len(new_features)}"
                        )
        ...
        logger.info(
            f"Cycle {self.cycle_count} complete: {len(new_features)} new features"
        )
    ...
    def _update_best_features(self, top_n: int = 10):
        ...
        all_features = sorted(
            self.evolved_features_history, key=lambda f: f.fitness, reverse=True
        )
...
def run_autonomous_evolution(base_dir: str = None) -> Dict[str, Any]:
    ...
    imbalance = (data["bid_vol"] - data["ask_vol"]) / (
        data["bid_vol"] + data["ask_vol"] + 1e-10
    )
...
def demo():
    ...
    if GPLEARN_AVAILABLE:
        ...
        evolver = FeatureEvolver(generations=5, population_size=500, n_components=3)
    else:
