...
def fetch_live_ohlcv(
    symbol: str = "BTC-USD", period: str = "1y", interval: str = "1d"
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        logger.info(
            f"Fetching live OHLCV for {symbol} period={period} interval={interval} via yfinance"
        )
        ...
        if len(normalized) < 4:  # at least OHLC
            ...
            try:
                from quant_trading_system.data_feeds.yfinance_feed import (
                    get_price_history,
                )
        ...
        logger.info(
            f"Live OHLCV fetched: {len(df)} rows from {df.index[0]} to {df.index[-1]}"
        )
        return df[cols] if all(c in df.columns for c in cols) else df

    except Exception as exc:
        logger.warning(
            f"Live fetch failed ({exc}); using deterministic synthetic fallback seeded by OMNIUM_INVARIANT_SEED"
        )

        # Deterministic synthetic fallback — seeded, reproducible, 252 trading days
        rng = np.random.RandomState(OMNIUM_DETERMINISTIC_SEED)
        dates = pd.date_range(
            end=pd.Timestamp.now(tz="UTC"), periods=252, freq="B"
        )  # business days
        ...
        logger.info(
            f"Synthetic deterministic OHLCV generated: {len(df)} rows, seed={OMNIUM_DETERMINISTIC_SEED}"
        )
        return df
...
class EnhancedBacktester:
    ...
    def add_data(self, data, name=None):
        ...
        self.cerebro["data_feeds"].append({"data": data, "name": name})
    ...
    def add_analyzer(self, analyzer_type, **kwargs):
        ...
        self.cerebro["analyzers"].append({"type": analyzer_type, "params": kwargs})

    def set_live_data(self, ohlcv_df: pd.DataFrame):
        """Attach real OHLCV for live-grounded backtest."""
        self._live_ohlcv = ohlcv_df
        self._data_source = (
            "live" if ohlcv_df is not None and not ohlcv_df.empty else "simulated"
        )
    ...
    def run_backtest(self, ohlcv_df: pd.DataFrame = None):
        ...
        if live_df is not None and not live_df.empty:
            ...
            self.logger.info(
                f"Live Data Injection: using real OHLCV {len(live_df)} rows, source=live"
            )
        ...
        self.logger.info(
            f"Backtest completed in {execution_time:.2f} seconds [source={self._data_source}]"
        )
    ...
    def _generate_trades_from_ohlcv(self, ohlcv_df: pd.DataFrame):
        ...
        for i in range(len(df) - 1):
            ...
            trade = {
                ...
                "pnl": pnl,
                "return": (
                    pnl / (entry_price * size) if entry_price * size != 0 else 0.0
                ),
                "sma_20": sma,
                "close": entry_price,
            }
        ...
        if not trades:
            self.logger.warning(
                "No trades generated from OHLCV; falling back to simulated"
            )
            return self._generate_simulated_trades()
            ...
            for data_idx in range(num_data_feeds):
                ...
                for i in range(num_trades):
                    entry_time = datetime.now() - timedelta(
                        days=int(self.rng.randint(1, 30))
                    )
    ...
    def _calculate_metrics(self):
        ...
        profit_factor = (
            ...
            if losing_trades > 0
            and sum([t["pnl"] for t in self.trades if t["pnl"] <= 0]) != 0
            else float("inf")
        )
