...
class EmotionHarvestAI:
    ...
    def _calculate_order_flow_imbalance(self, ticks: List[Dict[str, Any]]) -> float:
        ...
        buy_volume = sum(
            tick["amount"]
            for tick in ticks
            if "amount" in tick and "side" in tick and tick["side"] == "buy"
        )
        sell_volume = sum(
            tick["amount"]
            for tick in ticks
            if "amount" in tick and "side" in tick and tick["side"] == "sell"
        )
    ...
    def _detect_microbursts(
        self, ticks: List[Dict[str, Any]], timeframe_ms: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i in range(window_size, len(ticks), window_size // 2):
            ...
            if volatility > 0.05 and imbalance > 0.3 and volatility_acceleration > 0:
                ...
                volatility > 0.1 and imbalance > 0.5 and volatility_acceleration > 0.05
            ):
                emotion = "euphoria"
            ...
            for i in range(len(df) - 1):
                ...
                for j in range(1, num_ticks - 1):
                    if j != high_idx and j != low_idx:
                        prices[j] = (
                            prices[j - 1] * 0.4 + prices[j] * 0.2 + prices[j + 1] * 0.4
                        )
    ...
    def detect_emotions(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "symbol": symbol,
                "emotions": [
                    {"emotion": e, "strength": float(sum(s) / len(s))}
    ...
    def harvest_emotions(
        ...
        try:
            ...
            if (
                emotions["dominant_emotion"] == "panic"
                and emotions["emotion_strength"] >= self.panic_threshold
            ):
                signal = "BUY"  # Buy the panic
                confidence = emotions["confidence"] * 1.1  # Boost confidence for panic
            elif (
                emotions["dominant_emotion"] == "fear"
                and emotions["emotion_strength"] >= self.fear_threshold
            ):
                signal = "BUY"  # Buy the fear
                confidence = emotions["confidence"]
            elif (
                emotions["dominant_emotion"] == "euphoria"
                and emotions["emotion_strength"] >= self.euphoria_threshold
            ):
                signal = "SELL"  # Sell the euphoria
                confidence = (
                    emotions["confidence"] * 1.1
                )  # Boost confidence for euphoria
            elif (
                emotions["dominant_emotion"] == "greed"
                and emotions["emotion_strength"] >= self.greed_threshold
            ):
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "emotion_detection_accuracy": float(
                self.performance["emotion_detection_accuracy"]
            ),
