...
class InverseTimeEchoes:
    ...
    def _calculate_market_depth(self, df: pd.DataFrame) -> pd.Series:
        ...
        for i in range(20, len(df)):
            window = df.iloc[i - 20 : i]
            price_range = (window["high"].max() - window["low"].min()) / window[
                "close"
            ].iloc[-1]
        ...
        for scale in [8, 16, 32, 64]:
            ...
            for i, coeff in enumerate(coeffs):
                peaks, _ = signal.find_peaks(
                    np.abs(coeff), height=np.std(coeff) * 2, distance=scale
                )

                for peak_idx in peaks:
                    if (
                        peak_idx < self.min_pattern_length
                        or peak_idx > len(price_series) - self.min_pattern_length
                    ):
                        continue
                    ...
                    if (
                        peak_idx - pattern_length < 0
                        or peak_idx + pattern_length >= len(price_series)
                    ):
                        continue

                    pattern = price_series[
                        peak_idx - pattern_length : peak_idx + pattern_length
                    ]

                    pattern_norm = (pattern - np.mean(pattern)) / (
                        np.std(pattern) + 1e-10
                    )
                    ...
                    matches.append(
                        {
                            ...
                            "match_timestamp": (
                                df.index[i] if i < len(df.index) else None
                            ),
    ...
    def _predict_future_movement(
        self, df: pd.DataFrame, matches: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for match in high_conf_matches:
            ...
            if match["is_future"]:
                ...
                if pattern_idx + (match_idx - pattern_idx) < len(df):
                    future_price = df["close"].iloc[
                        pattern_idx + (match_idx - pattern_idx)
                    ]
        ...
        if total_weight > 0:
            ...
            prediction_values = [
                (
                    df["close"].iloc[m["match_idx"] + m["time_distance"]]
                    / df["close"].iloc[m["match_idx"]]
                )
                - 1
            ...
            if len(prediction_values) >= 3:
                ...
                consistency = 1.0 / (
                    1.0 + prediction_std * 10
                )  # Higher consistency for lower std
    ...
    def update_echo_patterns(self, symbol: str) -> None:
        ...
        self.logger.info(
            f"Updated echo patterns for {symbol}: {len(patterns)} patterns, {len(matches)} matches"
        )
        ...
        try:
            ...
            if confidence >= self.confidence_threshold and signal in ["BUY", "SELL"]:
                ...
                return {
                    ...
                    "confidence": float(confidence),
                    "prediction": (
                        float(echoes["prediction"]) if "prediction" in echoes else 0.0
                    ),
                    "timestamp": datetime.now().isoformat(),
                }
