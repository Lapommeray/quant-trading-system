...
class EnhancedRiskManagement:
    ...
    def calculate_portfolio_metrics(self, trades):
        ...
        try:
            ...
            returns = np.array(
                [trade["return"] for trade in trades if "return" in trade]
            )
            ...
            sortino_ratio = (
                np.mean(excess_returns) * np.sqrt(252) / downside_deviation
    ...
    def apply_position_sizing(self, trades, portfolio_metrics=None):
        ...
        try:
            ...
            for trade in trades:
                ...
                if (
                    "adjusted_var" in portfolio_metrics
                    and portfolio_metrics["adjusted_var"] != 0
                ):
                    risk_factor = min(
                        1.0, 0.02 / abs(portfolio_metrics["adjusted_var"])
                    )
    ...
    def apply_stop_loss_take_profit(
        self, trades, stop_loss_pct=0.02, take_profit_pct=0.05
    ):
        """
    ...
    def calculate_risk_adjusted_returns(self, returns):
        ...
        try:
            ...
            calmar_ratio = (
                np.mean(excess_returns) * 252 / max_drawdown
                if max_drawdown > 0
                else 0.0
            )
            ...
            threshold = 0.0
            omega_ratio = (
                np.sum(returns[returns > threshold] - threshold)
                / abs(np.sum(returns[returns < threshold] - threshold))
                if np.sum(returns[returns < threshold] - threshold) != 0
                else float("inf")
            )
