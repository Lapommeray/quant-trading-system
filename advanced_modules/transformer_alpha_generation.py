...
class TransformerAlphaGeneration:
    ...
    def prepare_data(
        self, data, asset, feature_columns=None, target_column="returns", seq_length=20
    ):
        """
    ...
    def train_model(
        ...
        X, y = self.prepare_data(
            data, asset, feature_columns, target_column, seq_length
        )
    ...
    def predict(self, data, asset):
        ...
        try:
            ...
            X = torch.tensor(last_sequence, dtype=torch.float32).unsqueeze(
                1
    ...
    def generate_signals(self, data, asset, threshold=0.0):
        ...
        try:
            ...
            for i in range(seq_length, len(data)):
                ...
                X = torch.tensor(sequence, dtype=torch.float32).unsqueeze(
                    1
                )  # [seq_len, 1, input_size]
                X = X.to(self.device)
                ...
                signals.append(
                    {"date": data.index[i], "prediction": prediction, "signal": signal}
                )
    ...
    def load_model(self, asset, file_path):
        ...
        try:
            ...
            model = TimeSeriesTransformer(
                input_size=len(checkpoint["feature_columns"]), output_size=1
            ).to(self.device)
