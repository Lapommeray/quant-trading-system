...
class QuantumLiquiditySignatureReader:
    ...
    def _calculate_order_book_fingerprint(
        ...
        spread = (
            float(ask_prices[0] - bid_prices[0])
        ...
        bid_clustering = (
            float(np.std(bid_price_diffs)) if len(bid_price_diffs) > 0 else 0.0
        )
        ask_clustering = (
            float(np.std(ask_price_diffs)) if len(ask_price_diffs) > 0 else 0.0
        )

        bid_volume_distribution = (
            float(np.std(bid_volumes) / (np.mean(bid_volumes) + 1e-10))
            if len(bid_volumes) > 0
            else 0.0
        )
        ask_volume_distribution = (
            float(np.std(ask_volumes) / (np.mean(ask_volumes) + 1e-10))
            if len(ask_volumes) > 0
            else 0.0
        )

        large_bid_threshold = (
            np.percentile(bid_volumes, 90)
            if len(bid_volumes) > 10
            else np.max(bid_volumes)
        )
        large_ask_threshold = (
            np.percentile(ask_volumes, 90)
            if len(ask_volumes) > 10
            else np.max(ask_volumes)
        )

        large_bid_presence = (
            float(np.sum(bid_volumes > large_bid_threshold) / len(bid_volumes))
            if len(bid_volumes) > 0
            else 0.0
        )
        large_ask_presence = (
            float(np.sum(ask_volumes > large_ask_threshold) / len(ask_volumes))
            if len(ask_volumes) > 0
            else 0.0
        )
    ...
    def _match_fingerprint(self, fingerprint: Dict[str, Any]) -> Tuple[str, float]:
        ...
        for whale_id, signatures in self.signature_database.items():
            for signature in signatures:
                ...
                spread_diff = abs(fingerprint["spread"] - signature["spread"]) / (
                    signature["spread"] + 1e-10
                )
                volume_imbalance_diff = abs(
                    fingerprint["volume_imbalance"] - signature["volume_imbalance"]
                )
                bid_clustering_diff = abs(
                    fingerprint["bid_clustering"] - signature["bid_clustering"]
                ) / (signature["bid_clustering"] + 1e-10)
                ask_clustering_diff = abs(
                    fingerprint["ask_clustering"] - signature["ask_clustering"]
                ) / (signature["ask_clustering"] + 1e-10)

                diff_score = (
                    spread_diff * 0.3
                    + volume_imbalance_diff * 0.3
                    + bid_clustering_diff * 0.2
                    + ask_clustering_diff * 0.2
                )
    ...
    def _predict_whale_action(
        self, whale_id: str, fingerprint: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if current_imbalance > 0.3 and current_spread < np.mean(spreads) * 0.8:
            ...
            confidence = min(
                0.6 + current_spread / (np.mean(spreads) + 1e-10) * 0.2, 0.99
            )
        else:
            action = "neutral"
            confidence = 0.5
    ...
    def update_signature_database(self, symbols: List[str]) -> None:
        ...
        for symbol in symbols:
            ...
            if match_score > 0.8:
                ...
                if len(self.signature_database[whale_id]) > 20:
                    self.signature_database[whale_id] = self.signature_database[
                        whale_id
                    ][-20:]
            ...
            self.active_whales[symbol] = {
                "whale_id": (
                    whale_id
                    if match_score > 0.8
                    else f"whale_{len(self.signature_database)}"
                ),
        ...
        self.logger.info(
            f"Updated signature database with {len(symbols)} symbols. Total unique whales: {len(self.signature_database)}"
        )
    ...
    def read_signature(
        ...
        try:
            ...
            if signature["prediction"]["action"] == "accumulating":
                ...
                confidence = (
                    signature["prediction"]["confidence"] * signature["match_score"]
                )
                ...
                confidence = (
                    signature["prediction"]["confidence"]
                    * signature["match_score"]
                    * 0.8
                )  # Lower confidence for trap reversal
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        """
        return {
            "signature_detection_accuracy": float(
                self.performance["signature_detection_accuracy"]
            ),
            "prediction_accuracy": float(self.performance["prediction_accuracy"]),
            "unique_signatures_detected": int(
                self.performance["unique_signatures_detected"]
            ),
