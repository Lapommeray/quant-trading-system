...
class BTCOffchainMonitor:
    ...
    def check_transfers(self, current_time):
        ...
        self.transfer_history.append({"time": current_time, "amount": transfer_amount})
        ...
        if alert_triggered:
            self.algo.Debug(
                f"BTCOffchainMonitor: Large transfer detected! {transfer_amount} BTC"
            )
        ...
        return {
            ...
            "confidence": (
                min(transfer_amount / (self.alert_threshold * 2), 1.0)
                if alert_triggered
                else 0.0
            ),
