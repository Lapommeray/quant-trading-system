...
class PortActivityAnalyzer:
    ...
    def analyze(self, current_time=None):
        ...
        for port in self.monitored_ports:
            if (
                port not in self.last_check_time
                or current_time - self.last_check_time[port] >= self.check_interval
            ):
                should_analyze = True
        ...
        for port, info in self.monitored_ports.items():
            ...
            if disruption_detected:
                confidence = min(
                    info["importance"] * (spectral_value - info["threshold"]) / 5 + 0.5,
                    1.0,
                )
            else:
                confidence = 0.0
            ...
            if len(self.detection_history[port]) > self.max_history_size:
                self.detection_history[port] = self.detection_history[port][
            ...
            if disruption_detected:
                self.algo.Debug(
                    f"PortActivityAnalyzer: Disruption detected at {port}! "
        ...
        return {
            ...
            "direction": (
                "BUY" if overall_disruption else None
            ),  # Supply chain disruptions often lead to price increases
        }
        ...
        for port in self.monitored_ports:
            if port in self.detection_history and self.detection_history[port]:
                ...
                if len(recent) >= 3:
                    ...
                    trend = np.polyfit(range(len(spectral_values)), spectral_values, 1)[
                        0
                    ]
                    trend_direction = (
                        "increasing"
