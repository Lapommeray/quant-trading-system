...
class StressDetector:
    ...
    def detect(self, symbol, current_time=None):
        ...
        if extreme_stress:
            confidence = min(
                (stress_score - self.extreme_stress_threshold) * 5 + 0.7, 1.0
            )
        elif moderate_stress:
            confidence = min(
                (stress_score - self.moderate_stress_threshold) * 3 + 0.4, 0.7
            )
        else:
            confidence = 0.0
        ...
        if len(self.detection_history[symbol_str]) > self.max_history_size:
            self.detection_history[symbol_str] = self.detection_history[symbol_str][
                -self.max_history_size :
            ]

        if extreme_stress:
            self.algo.Debug(
                f"StressDetector: Extreme stress detected in {symbol_str} earnings call! "
                + f"Score: {stress_score:.2f}"
            )
        elif moderate_stress:
            self.algo.Debug(
                f"StressDetector: Moderate stress detected in {symbol_str} earnings call. "
                + f"Score: {stress_score:.2f}"
            )
        ...
        return {
            ...
            "confidence": confidence,
            "direction": (
                "SELL" if extreme_stress else ("SELL" if moderate_stress else None)
            ),
        }
    ...
    def _update_earnings_calendar(self, current_time):
        ...
        for i in range(30):
            ...
            if date not in self.earnings_calendar:
                ...
                earnings_symbols = np.random.choice(
                    self.sp500_symbols, num_earnings, replace=False
                )
