...
class FedJetMonitor:
    ...
    def check_movements(self, current_time):
        ...
        self.movement_history.append({"time": current_time, "movements": movements})
        ...
        significant_destinations = [
            dest for dest in movements if dest in self.key_destinations
        ]
        ...
        if zurich_travel or multiple_cb_visits:
            self.algo.Debug(
                f"FedJetMonitor: Significant travel detected! Destinations: {significant_destinations}"
            )
        ...
        return {
            ...
            "confidence": (
                0.7 if zurich_travel else (0.6 if multiple_cb_visits else 0.0)
            ),
            "direction": (
                "BUY" if zurich_travel else ("SELL" if multiple_cb_visits else None)
            ),
        }
    ...
    def analyze_pattern(self):
        ...
        recent_zurich = any(
            "ZRH" in entry["movements"] for entry in self.movement_history[-3:]
        )
