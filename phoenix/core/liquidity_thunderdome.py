...
try:
    ...
    TF_AVAILABLE = False
    logging.warning(
        "TensorFlow not available. Using classical fallback for RL operations."
    )
    ...
    def _build_models(self):
        ...
        self.actor = Sequential(
            [
                ...
                Dense(
                    self.action_dim, activation="tanh"
                ),  # tanh for bounded actions [-1, 1]
            ]
        )
        ...
        self.training_history.append(
            {
                ...
                "critic_loss": float(
                    self.critic.optimizer.get_gradients()[0].numpy().mean()
                ),
...
class MirrorAgent:
    ...
    def mirror(self, state):
        ...
        if not TF_AVAILABLE or self.model is None:
            if len(self.observations) > 0:
                similarities = [
                    np.sum(np.square(obs_state - state))
                    for obs_state, _ in self.observations
                ]
    ...
    def train(self):
        ...
        self.training_history.append({"loss": float(loss), "timestamp": time.time()})
    ...
    def predict_detection(self, state):
        ...
        if not TF_AVAILABLE or self.model is None:
            if len(self.surveillance_patterns) > 0:
                similarities = [
                    np.sum(np.square(obs_state - state))
                    for obs_state, _ in self.surveillance_patterns
                ]
                most_similar_idx = np.argmin(similarities)
...
class LiquidityThunderdome:
    ...
    def _reset_environment(self):
        ...
        self.current_step = 0
        self.current_state = np.random.normal(
            0, 1, self.state_dim
    ...
    def _run_battle(self):
        ...
        while not self.current_done and self.current_step < self.max_steps:
            ...
            stealth_reward = (
                1.0 - detection_prob
            )  # Higher reward for lower detection probability
            ...
            self.aggressor.remember(
                self.current_state, aggressor_action, reward, next_state, done
            )

            detection = (
                np.random.random() < detection_prob
        ...
        battle_result = {
            ...
            "average_reward": (
                self.current_reward / self.current_step if self.current_step > 0 else 0
            ),
    ...
    def get_status(self):
        ...
        return {
            ...
            "average_reward": (
                np.mean(
                    [battle["total_reward"] for battle in self.battle_history[-100:]]
                )
                if self.battle_history
                else 0
            ),
            "average_detection_prob": (
                np.mean(
                    [
                        battle["final_detection_prob"]
                        for battle in self.battle_history[-100:]
                    ]
                )
                if self.battle_history
                else 0
