...
class DarkPoolConnector:
    ...
    def __init__(self, pools=None):
        ...
        self.logger.info(
            f"DarkPoolConnector initialized with pools: {', '.join(self.pools)}"
        )
    ...
    def get_best_pools(self, asset, size, stealth_priority=0.5):
        ...
        for pool in self.pools:
            ...
            combined_score = (execution_score * (1.0 - stealth_priority)) + (
                stealth_score * stealth_priority
            )
...
class PhoenixChannel:
            ...
            try:
                ...
                for asset in list(self.canceled_orders.keys()):
                    self.canceled_orders[asset] = [
                        order
                        for order in self.canceled_orders[asset]
    ...
    def get_status(self):
        ...
        return {
            ...
            "total_canceled_orders": sum(
                [len(orders) for orders in self.canceled_orders.values()]
            ),
