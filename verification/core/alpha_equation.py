...
class AlphaEquation:
    ...
    def calculate_alpha(self, trades_df):
        ...
        edge_frequency = (
            len(winning_trades) / len(trades_df) if len(trades_df) > 0 else 0
        )
        error_frequency = (
        ...
        profit_factor = (
            (edge_frequency * edge_size) / (error_frequency * error_cost)
            if (error_frequency * error_cost) > 0
            else float("inf")
        )
    ...
    def calculate_alpha_by_period(self, trades_df, period="month"):
        ...
        if period == "day":
            ...
            trades_df["period"] = (
                trades_df["timestamp"].dt.year.astype(str)
                + "-Q"
                + (trades_df["timestamp"].dt.quarter).astype(str)
            )
        elif period == "year":
            trades_df["period"] = trades_df["timestamp"].dt.year
        else:
            raise ValueError(
                "Invalid period. Choose from 'day', 'week', 'month', 'quarter', 'year'"
            )
