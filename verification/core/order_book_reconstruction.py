...
class OrderBookReconstructor:
    ...
    def get_order_book(self):
        ...
        bids = [
            {"price": -price, "volume": self.bid_volumes[-price]}
            for price in sorted(self.bids)
        ]
        asks = [
            {"price": price, "volume": self.ask_volumes[price]}
    ...
    def calculate_market_impact(self, side, volume):
        ...
        impact_amount = (
            execution["avg_price"] - mid_price
            if side == "BUY"
            else mid_price - execution["avg_price"]
        )
    ...
    def get_liquidity_metrics(self):
        ...
        near_bid_volume = sum(
            self.bid_volumes[price]
            for price in self.bid_volumes
            if price >= mid_price * 0.995
        )
        near_ask_volume = sum(
            self.ask_volumes[price]
            for price in self.ask_volumes
            if price <= mid_price * 1.005
        )

        liquidity_score = (near_bid_volume + near_ask_volume) / mid_price

        imbalance = (total_ask_volume - total_bid_volume) / (
