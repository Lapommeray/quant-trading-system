...
class RetailSentimentAnalyzer:
    ...
    def fetch_sentiment_data(self, symbol):
        ...
        bullish_percentage = min(
            0.95, max(0.05, 0.5 + base_sentiment * 0.4 * trend_factor)
        )
        ...
        urgency_metrics = {
            ...
            "rocket_emoji_count": (
                int(max(0, sentiment_magnitude * 20 - 5)) if base_sentiment > 0 else 0
            ),
            "bear_emoji_count": (
                int(max(0, sentiment_magnitude * 15 - 5)) if base_sentiment < 0 else 0
            ),
            "urgent_terms_frequency": min(0.7, 0.1 + sentiment_magnitude * 0.4),
        }

        bullish_phrases = self._generate_phrases(
            symbol, is_bullish=True, sentiment_magnitude=sentiment_magnitude
        )
        bearish_phrases = self._generate_phrases(
            symbol, is_bullish=False, sentiment_magnitude=sentiment_magnitude
        )
        ...
        self.sentiment_data[symbol] = {
            ...
            "common_phrases": {"bullish": bullish_phrases, "bearish": bearish_phrases},
    ...
    def _generate_phrases(self, symbol, is_bullish, sentiment_magnitude):
        ...
        num_phrases = max(
            1, min(len(base_phrases[is_bullish]), int(sentiment_magnitude * 5))
        )
        ...
        for phrase in selected_phrases:
            ...
            phrases.append({"text": phrase, "count": count})
    ...
    def analyze_sentiment(self, symbol):
        ...
        if sentiment > 0.7:
            ...
            message = (
                "Extreme bullish retail sentiment detected - contrarian sell signal"
            )
        elif sentiment < -0.7:
            direction = "BUY"
            confidence = min(0.9, (abs(sentiment) - 0.7) * 3)
            message = (
                "Extreme bearish retail sentiment detected - contrarian buy signal"
            )
        elif sentiment > 0.4:
            direction = "SELL"
            confidence = min(0.6, (sentiment - 0.4) * 3)
            message = (
                "Moderately bullish retail sentiment - weak contrarian sell signal"
            )
        elif sentiment < -0.4:
            direction = "BUY"
        ...
        urgency_score = (
            urgency["exclamation_frequency"]
            + urgency["caps_percentage"]
            + urgency["urgent_terms_frequency"]
        ) / 3
