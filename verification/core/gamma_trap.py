...
class GammaTrap:
    ...
    def fetch_options_data(self, symbol):
        ...
        """
        if (
            symbol in self.options_data
            and symbol in self.last_update
            and datetime.now() - self.last_update[symbol] < self.update_frequency
        ):
        ...
        for i in range(3):  # Next 3 months
            month_end = datetime(
                now.year,
                now.month + i if now.month + i <= 12 else now.month + i - 12,
        ...
        for expiry in expiries:
            ...
            for strike in strike_range:
                ...
                call_delta = self._calculate_synthetic_delta(
                    base_price, strike, days_to_expiry, iv, is_call=True
                )
                call_gamma = self._calculate_synthetic_gamma(
                    base_price, strike, days_to_expiry, iv
                )
                call_open_interest = int(
                    np.random.exponential(1000)
                    * math.exp(-0.01 * abs(strike - base_price))
                )

                put_delta = self._calculate_synthetic_delta(
                    base_price, strike, days_to_expiry, iv, is_call=False
                )
                ...
                call_gamma_exposure = (
                    dealer_call_position * call_gamma * 100 * base_price
                )
    ...
    def _calculate_synthetic_delta(
        self, price, strike, days_to_expiry, iv, is_call=True
    ):
    ...
    def _calculate_synthetic_gamma(self, price, strike, days_to_expiry, iv):
        ...
        gamma = math.exp(
            -((math.log(moneyness)) ** 2) / (2 * (iv * time_factor) ** 2)
        ) / (price * iv * time_factor * math.sqrt(2 * math.pi))
    ...
    def analyze_gamma_hedging(self, symbol="SPX", current_price=None):
        ...
        call_oi_by_strike = {
            opt["strike"]: opt["call"]["open_interest"] for opt in options_chain
        }
        put_oi_by_strike = {
            opt["strike"]: opt["put"]["open_interest"] for opt in options_chain
        }

        top_call_strikes = sorted(
            call_oi_by_strike.keys(), key=lambda k: call_oi_by_strike[k], reverse=True
        )[:3]
        top_put_strikes = sorted(
            put_oi_by_strike.keys(), key=lambda k: put_oi_by_strike[k], reverse=True
        )[:3]

        if gamma_exposure < self.hedging_thresholds["extreme"]:
            direction = (
                "BUY" if current_price < options_data["price"] * 0.99 else "SELL"
            )
            confidence = min(
                0.9, abs(gamma_exposure / self.hedging_thresholds["extreme"])
            )
            message = (
                "Extreme negative gamma exposure: dealers will amplify market moves"
            )
        elif gamma_exposure < self.hedging_thresholds["low"]:
            direction = (
                "BUY" if current_price < options_data["price"] * 0.995 else "SELL"
            )
            ...
            direction = (
                "SELL" if current_price < options_data["price"] * 0.99 else "BUY"
            )
        ...
        return {
            ...
            "gamma_exposure_normalized": gamma_exposure
            / abs(self.hedging_thresholds["extreme"]),
    ...
    def get_gamma_report(self, symbol="SPX"):
        ...
        for expiry, chain in options_data["options_chains"].items():
            expiry_gamma = sum(
                opt["call"]["gamma_exposure"] + opt["put"]["gamma_exposure"]
                for opt in chain
            )
        ...
        report = {
            ...
            "gamma_exposure_normalized": self.gamma_exposure.get(symbol, 0)
            / abs(self.hedging_thresholds["extreme"]),
