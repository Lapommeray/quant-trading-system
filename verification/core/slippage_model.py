...
class SlippageModel:
    ...
    def generate_costs_log(self, output_file="costs.log"):
        ...
        with open(output_file, "w") as f:
            ...
            f.write(
                f"Worst slippage: {stats['worst_slippage_bps']:.2f} bps ({stats['worst_slippage_symbol']})\n"
            )
            ...
            for trade in stats["timestamps"]:
                f.write(
                    f"[{trade['timestamp']}] {trade['symbol']} {trade['direction']} {trade['volume']} @ "
...
def slippage_simulator(
    ...
    model.log_trade(
        symbol, raw_price, adjusted_price, volume, direction, actual_latency
    )
