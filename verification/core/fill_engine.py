...
class FillEngine:
    def __init__(
        self, slippage_enabled=True, latency_ms=200, order_book_simulation=True
    ):
        """
        ...
        self.slippage_model = SlippageModel(
            enable_slippage=slippage_enabled, latency_ms=latency_ms
        )
    ...
    def execute_order(self, symbol, direction, price, volume, timestamp=None):
        ...
        current_price = self._get_current_price(
            symbol, price, direction, actual_latency
        )
        ...
        if self.order_book_simulation:
            fill_price = self._simulate_order_book(
                symbol, fill_price, volume, direction
            )
    ...
    def _simulate_order_book(self, symbol, price, volume, direction):
        ...
        market_impact_pct = (
            volume / liquidity
        ) ** 0.5 * 0.0001  # Square root function to model diminishing impact
