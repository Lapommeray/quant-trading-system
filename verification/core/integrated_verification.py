...
class IntegratedVerification:
    ...
    def analyze_symbol(self, symbol, current_price, high_price=None, low_price=None):
        ...
        if self.modules_enabled["market_regime"]:
            self.market_regime.update_price_memory(
        ...
        if self.modules_enabled["market_regime"]:
            ...
            if regime["regime"] in ["crisis", "pre_crisis"]:
                if combined_signal["confidence"] > 0.85:
                    ...
                    results["trading_message"] = (
                        f"Trading allowed despite {regime['regime']} regime - very strong signal"
                    )
                else:
                    results["trading_allowed"] = False
                    results["trading_message"] = (
                        f"Trading halted - {regime['regime']} regime with insufficient signal strength"
                    )
            else:
                results["trading_allowed"] = True
                results["trading_message"] = (
                    f"Trading allowed - {regime['regime']} regime"
                )
        else:
            results["trading_allowed"] = True
            results["trading_message"] = (
                "Trading allowed - market regime detection disabled"
            )
    ...
    def _combine_signals(self, signals):
        ...
        signal_count = len(
            [s for s in signals.values() if s.get("direction") is not None]
        )
        signal_agreement = (
            max(buy_votes, sell_votes) / total_votes if total_votes > 0 else 0
        )
    ...
    def generate_verification_report(self, trades_df, symbol, output_dir="./reports"):
        ...
        if self.modules_enabled["neural_pattern"]:
            neural_path = os.path.join(
                output_dir, f"{symbol}_neural_pattern_report.json"
            )
        ...
        if self.modules_enabled["market_regime"]:
            regime_path = os.path.join(
                output_dir, f"{symbol}_market_regime_report.json"
            )
        ...
        summary = {
            ...
            "alpha_summary": (
                alpha_report["overall_alpha"] if self.modules_enabled["alpha"] else None
            ),
        }
        ...
        print(
            f"Performance metrics: Win Rate={win_rate:.2%}, Profit Factor={profit_factor:.2f}, Max Drawdown={max_drawdown:.2%}"
        )
