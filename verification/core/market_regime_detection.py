...
class MarketRegimeDetection:
    def __init__(self):
        ...
        self.circuit_breaker_cooldown = (
            0  # Cooldown period after circuit breaker activation
        )
        self.regime_types = [
    ...
    def _detect_regime(self, symbol):
        ...
        symbol_data = [
            point for point in self.price_memory if point["symbol"] == symbol
        ]
        ...
        short_term_volatility = (
            np.std(returns[-5:]) * math.sqrt(252) if len(returns) >= 5 else volatility
        )
        ...
        for price in prices[1:]:
            ...
            current_drawdown = (
                (rolling_max - price) / rolling_max if rolling_max > 0 else 0
            )
        ...
        if (
            rapid_change or volatility_ratio > 3.5 or atr_ratio > 3.5
        ):  # Much higher thresholds
            print(
                f"⚠️ EXTREME VOLATILITY WARNING: Very high volatility for {symbol}, but trading still allowed"
            )
        elif (
            volatility
            > self.volatility_thresholds.get(
                asset_class, self.volatility_thresholds["default"]
            )["crisis"]
            * 0.9
        ):
            print(
                f"⚠️ VOLATILITY WARNING: Elevated volatility levels for {symbol}, but trading still allowed"
            )
        elif drawdown > 0.049:  # Just below 5% threshold
            print(
                f"⚠️ DRAWDOWN WARNING: Significant drawdown of {drawdown:.2%} for {symbol}, but trading still allowed"
            )
            ...
            if self.circuit_breaker_cooldown == 0:
                ...
                if (
                    ...
                    * 0.7
                    and drawdown < 0.02
                    and volatility_ratio < 1.3
                    and atr_ratio < 1.3
                ):
                    self.circuit_breaker_active = False
                    print(
                        f"✅ CIRCUIT BREAKER DEACTIVATED: Market conditions have normalized for {symbol}"
                    )
                else:
                    self.circuit_breaker_cooldown = 5  # Reduced extension
                    print(
                        f"⚠️ CIRCUIT BREAKER EXTENDED: Market conditions still not normalized for {symbol}"
                    )
    ...
    def _classify_regime(self, metrics):
        ...
        if (
            metrics["volatility"]
            > thresholds["crisis"] * 0.8  # 80% of crisis threshold triggers crisis
            or metrics["drawdown"] > 0.04  # 4% drawdown triggers crisis
            or metrics["max_drawdown"] > 0.045  # 4.5% max drawdown triggers crisis
            or metrics["volatility_ratio"]
            > 2.0  # Moderate threshold for volatility spike
            or (metrics["rapid_change"] and metrics["drawdown"] > 0.03)
        ):  # Rapid price change with significant drawdown
            regime["regime"] = "crisis"
            regime["confidence"] = min(
                0.99, max(0.9, metrics["volatility"] / (thresholds["crisis"] * 0.8))
            )
        ...
        if (
            metrics["volatility"]
            > thresholds["pre_crisis"]
            * 0.8  # 80% of pre-crisis threshold triggers pre-crisis
            or metrics["drawdown"] > 0.03  # 3% drawdown triggers pre-crisis
            or metrics["atr_pct"] > 0.01  # 1% ATR as percentage of price
            or metrics["volatility_ratio"] > 1.8  # Moderate sensitivity
            or metrics["atr_ratio"] > 1.8  # Moderate sensitivity
            or metrics["max_drawdown"] > 0.035
        ):  # 3.5% max drawdown triggers pre-crisis
            regime["regime"] = "pre_crisis"
            regime["confidence"] = min(
                0.99, max(0.8, metrics["volatility"] / (thresholds["pre_crisis"] * 0.8))
            )
            regime["message"] = (
                "Pre-crisis conditions detected - reduced position sizing"
            )
            return regime
        ...
        if len(recent_returns) >= 10:
            recent_volatility = np.std(
                np.diff(recent_returns) / recent_returns[:-1]
            ) * math.sqrt(252)
            if (
                recent_volatility > 0.1
                and recent_volatility > metrics["volatility"] * 1.2
            ):  # More sensitive (was 0.2/1.5)
                regime["regime"] = "pre_crisis"
                regime["confidence"] = min(0.95, max(0.7, recent_volatility / 0.1))
                regime["message"] = (
                    "Rapidly increasing volatility detected - caution advised"
                )
                return regime

        if (
            metrics["volatility"] > thresholds["high_volatility"] * 0.9
        ):  # More sensitive (was 0.12 fixed)
            regime["regime"] = "high_volatility"
            regime["confidence"] = min(
                0.9,
                max(0.7, metrics["volatility"] / (thresholds["high_volatility"] * 0.9)),
            )
            regime["message"] = (
                "High volatility regime detected - reduce position sizes"
            )
            return regime

        if (
            metrics["volatility"] < thresholds["high_volatility"] * 0.5
        ):  # Relative to threshold (was 0.08 fixed)
            regime["regime"] = "low_volatility"
            regime["confidence"] = min(
                0.85,
                max(0.6, (thresholds["high_volatility"] * 0.5) / metrics["volatility"]),
            )
            regime["message"] = "Low volatility regime detected"
            return regime

        if (
            metrics["price_change_pct"] > 0.03  # More sensitive (was 0.05)
            and metrics.get("ma20") > metrics.get("ma50", 0)
            and metrics["drawdown"] < 0.005
        ):  # Added drawdown check
            regime["regime"] = "trending_up"
            regime["confidence"] = min(
                0.8, max(0.6, metrics["price_change_pct"] / 0.03)
            )
        ...
        if (
            metrics["price_change_pct"] < -0.03  # More sensitive (was -0.05)
            and metrics.get("ma20") < metrics.get("ma50", float("inf"))
            and metrics["drawdown"] < 0.005
        ):  # Added drawdown check
            regime["regime"] = "trending_down"
            regime["confidence"] = min(
                0.8, max(0.6, abs(metrics["price_change_pct"]) / 0.03)
            )
            regime["message"] = "Downtrend regime detected"
            return regime

        if (
            metrics["drawdown"] < 0.005  # More sensitive (was 0.03)
            and metrics["price_change_pct"] > 0.03  # More sensitive (was 0.05)
            and metrics.get("ma20", 0) > metrics.get("ma50", 0)
            and metrics["volatility"] < thresholds["high_volatility"] * 0.7
        ):  # Added volatility check
            regime["regime"] = "recovery"
            regime["confidence"] = min(
                0.75, max(0.6, metrics["price_change_pct"] / 0.03)
            )
    ...
    def should_trade(self, symbol, strategy_type="all"):
        ...
        if (
            regime["regime"] == "crisis"
            or drawdown > 0.04
            or max_drawdown > 0.045
            or volatility > 0.05
        ):
            self.circuit_breaker_active = True
            self.circuit_breaker_cooldown = 20  # Moderate cooldown period
            print(
                f"⚠️ CIRCUIT BREAKER ACTIVATED: {regime['regime']} regime, drawdown={drawdown:.4f}, max_drawdown={max_drawdown:.4f}, volatility={volatility:.4f}"
            )
        ...
        if (
            ...
            print(
                f"🛑 TRADING HALTED: {regime['regime']} regime, drawdown={drawdown:.4f}, max_drawdown={max_drawdown:.4f}, volatility={volatility:.4f}"
            )
            return (
                False,
                f"TRADING HALTED: Risk thresholds exceeded - no new positions",
                0.0,
            )
        ...
        if self.circuit_breaker_active:
            ...
            return (
                True,
                f"Circuit breaker active - trading with microscopic 0.01% position size",
                position_sizing,
            )
        ...
        if volatility > 0.01 or drawdown > 0.005:
            ...
            return (
                True,
                f"Elevated risk - trading with minimal 0.05% position size",
                position_sizing,
            )
        ...
        if regime["regime"] == "high_volatility" or regime["regime"] == "trending_down":
            ...
            return (
                True,
                f"High volatility or downtrend - trading with tiny 0.1% position size",
                position_sizing,
            )

        if regime["regime"] == "normal":
            position_sizing = (
                0.005  # 0.5% position size for normal conditions (reduced from 3%)
            )
            return (
                True,
                f"Normal market conditions - very conservative 0.5% position sizing",
                position_sizing,
            )

        if regime["regime"] == "low_volatility" and regime["regime"] == "trending_up":
            position_sizing = (
                0.01  # 1% position size for ideal conditions (reduced from 5%)
            )
            return (
                True,
                f"Ideal conditions - conservative 1% position sizing",
                position_sizing,
            )
        ...
        return (
            True,
            f"Default trading conditions - microscopic 0.05% position size",
            0.0005,
        )
    ...
    def get_regime_report(self, symbol):
        ...
        mean_reversion_allowed, mean_reversion_reason, mean_reversion_sizing = (
            self.should_trade(symbol, "mean_reversion")
        )
        volatility_allowed, volatility_reason, volatility_sizing = self.should_trade(
            symbol, "volatility"
        )
