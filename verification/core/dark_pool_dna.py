...
class DarkPoolDNA:
    ...
    def _generate_order_size_dna(self, symbol):
        ...
        if "BTC" in symbol:
            ...
            modulo_preference = (
                500  # Preference for sizes that are multiples of 500 shares
            )
        else:
    ...
    def _generate_institutional_fingerprints(self):
        ...
        for institution in selected_institutions:
            fingerprint = {
                ...
                "day_of_week_preference": random.choice(
    ...
    def analyze_dna_sequence(self, symbol, current_price=None):
        ...
        if current_price and "price_levels" in dna.get("price_level_dna", {}):
            ...
            if strong_levels:
                supports = [
                    level for level in strong_levels if level["price"] < current_price
                ]
                resistances = [
                    level for level in strong_levels if level["price"] > current_price
                ]
                ...
                if supports and not resistances:
                    ...
                    signal["confidence"] = max(
                        level["strength"] for level in resistances
                    )
        ...
        if "timing_dna" in dna:
            ...
            for pattern in timing_dna.get("timestamp_patterns", []):
                if (
                    pattern["type"] == "end_of_hour"
                    and minute >= 60 - pattern["minutes_before"]
                ):
                    ...
                    if (
                        hour == 9
                        and minute >= 30
                        and minute < 30 + pattern["minutes_after_open"]
                    ):
                        if signal["direction"] is None:
