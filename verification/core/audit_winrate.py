...
class WinRateAuditor:
    ...
    def audit_trades(self, file):
        ...
        if "entry_price" in trades.columns and "exit_price" in trades.columns:
            completed_trades = trades_with_pnl[
                trades_with_pnl["entry_price"].notna()
                & trades_with_pnl["exit_price"].notna()
            ].copy()
        ...
        win_rate = (
            len(winning_trades) / len(trades_with_pnl)
            if len(trades_with_pnl) > 0
            else 0
        )
        ...
        for symbol in symbols:
            ...
            symbol_is_realistic = (
                self.min_win_rate <= symbol_win_rate <= self.max_win_rate
            )
            ...
            symbol_stats[symbol] = {
                ...
                "avg_loss": (
                    symbol_trades[symbol_trades["pnl"] < 0]["pnl"].mean()
                    if len(symbol_trades[symbol_trades["pnl"] < 0]) > 0
                    else 0
                ),
            }
        ...
        if "timestamp" in trades.columns:
            ...
            trades["year_week"] = (
                trades["year"].astype(str) + "-" + trades["week"].astype(str)
            )
            ...
            for year_week in trades["year_week"].unique():
                ...
                week_is_realistic = (
                    self.min_win_rate <= week_win_rate <= self.max_win_rate
                )
    ...
    def plot_results(self, output_file="audit_chart.png"):
        ...
        plt.bar(
            ...
            color="g" if self.results["is_realistic"] else "r",
        )
        plt.axhline(
            y=self.min_win_rate,
            color="r",
            linestyle="--",
            label=f"Min: {self.min_win_rate:.0%}",
        )
        plt.axhline(
            y=self.max_win_rate,
            color="r",
            linestyle="--",
            label=f"Max: {self.max_win_rate:.0%}",
        )
        plt.ylim(0, 1)
        ...
        colors = [
            "g" if self.min_win_rate <= wr <= self.max_win_rate else "r"
            for wr in win_rates
        ]
        ...
        if (
            isinstance(self.results["weekly_stats"], dict)
            and "error" not in self.results["weekly_stats"]
        ):
            plt.subplot(2, 1, 2)
            weeks = list(self.results["weekly_stats"].keys())
            weekly_win_rates = [
                self.results["weekly_stats"][w]["win_rate"] for w in weeks
            ]
            weekly_colors = [
                "g" if self.min_win_rate <= wr <= self.max_win_rate else "r"
                for wr in weekly_win_rates
            ]
...
def audit_trades(file, min_win_rate=0.50, max_win_rate=0.80):
    ...
    for symbol, stats in results["symbol_stats"].items():
        print(
            f"  {symbol}: {stats['win_rate']:.2%} win rate over {stats['trades']} trades - "
