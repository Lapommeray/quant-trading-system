...
class MarketStressTest:
    ...
    def _generate_covid_crash_data(self, data_path=None):
        ...
        dates = pd.date_range(
            start="2020-02-01", end="2020-03-23"
        )  # Extended start date
        ...
        pre_crash_prices = np.random.normal(
            starting_price, starting_price * 0.005, pre_crash_days
        )

        decay_factor = np.exp(np.log(ending_price / starting_price) / crash_days)
        crash_prices = starting_price * np.array(
            [decay_factor**i for i in range(crash_days)]
        )
        ...
        volatility[pre_crash_days:] = np.linspace(
            pre_crash_volatility, crash_volatility, crash_days
        )
    ...
    def _generate_fed_panic_data(self, data_path=None):
        ...
        pre_panic_prices = np.random.normal(
            starting_price, starting_price * 0.004, pre_panic_days
        )

        decay_factor = np.exp(np.log(lowest_price / starting_price) / panic_days)
        panic_prices = starting_price * np.array(
            [decay_factor**i for i in range(panic_days)]
        )
        ...
        volatility[pre_panic_days:] = np.linspace(
            pre_panic_volatility, panic_volatility, panic_days
        )
    ...
    def _generate_flash_crash_data(self, data_path=None):
        ...
        timestamps = pd.date_range(
            start="2022-01-01 09:30:00", periods=390, freq="1min"
        )
        ...
        recovery_factor = np.exp(
            np.log((base_price) / (base_price * (1 - crash_pct)))
            / (recovery_idx - crash_idx)
        )
        ...
        for i in range(crash_idx + 1, recovery_idx + 1):
            if i < len(prices):
                ...
                prices[i] = (
                    base_price * (1 - crash_pct) * (recovery_factor**recovery_progress)
                )
        ...
        data.loc[crash_idx, "low"] = base_price * (
            1 - crash_pct - 0.02
        )  # Lower low at crash
        data.loc[crash_idx:recovery_idx, "high"] = (
            data.loc[crash_idx:recovery_idx, "high"] * 1.01
        )  # Higher volatility
        data.loc[crash_idx:recovery_idx, "low"] = (
            data.loc[crash_idx:recovery_idx, "low"] * 0.99
        )  # Higher volatility

        data.loc[crash_idx:recovery_idx, "volume"] = (
            data.loc[crash_idx:recovery_idx, "volume"] * 5
        )
    ...
    def assert_loss_threshold(self):
        ...
        if self.results["worst_drawdown"] >= self.max_drawdown_threshold:
            ...
            print(
                f"✅ All tests passed! Max drawdown: {self.results['worst_drawdown']:.2%} "
                f"(threshold: {self.max_drawdown_threshold:.2%})"
            )
            ...
            class MockTradingSystem:
                def __init__(self, use_advanced_features=True):
                    ...
                    self.max_position_size = (
                        0.03  # Reduced max position size to 3% of portfolio
                    )
                    ...
                    if use_advanced_features:
                        try:
                            ...
                            from core.order_book_reconstruction import (
                            ...
                            print(
                                "✅ Advanced verification modules loaded successfully"
                            )
                ...
                def process_bar(self, bar):
                    ...
                    if len(self.last_highs) > 5:
                        ...
                        self.volatility = (
                            sum(ranges) / len(ranges) / self.last_prices[-1]
                        )

                    if len(self.last_prices) > 10:
                        prev_ranges = []
                        for i in range(
                            len(self.last_highs) - 24, len(self.last_highs) - 14
                        ):
                            if i >= 0:
                                true_range = max(
                                    self.last_highs[i] - self.last_lows[i],
                        ...
                        if prev_ranges:
                            ...
                            self.recent_volatility_change = (
                                self.volatility / prev_volatility
                            ) - 1

                    self.consecutive_down_days = 0
                    if len(self.last_prices) > 3:
                        for i in range(
                            len(self.last_prices) - 3, len(self.last_prices)
                        ):
                    ...
                    if self.volatility > 0.03 or self.recent_volatility_change > 0.5:
                        ...
                        print(
                            f"VOLATILE MARKET DETECTED: Volatility={self.volatility:.4f}, Change={self.recent_volatility_change:.2f}"
                        )
                    elif self.volatility > 0.05 or self.recent_volatility_change > 0.8:
                        self.market_regime = "crisis"
                        print(
                            f"CRISIS REGIME DETECTED: Volatility={self.volatility:.4f}, Change={self.recent_volatility_change:.2f}"
                        )
                        if self.recent_volatility_change > 1.2:
                            self.circuit_breaker_active = True
                            print(
                                "⚠️ CIRCUIT BREAKER ACTIVATED: Extreme volatility spike detected"
                            )
                    elif self.volatility > 0.02 or self.recent_volatility_change > 0.3:
                        self.market_regime = "pre_crisis"
                        print(
                            f"PRE-CRISIS DETECTED: Volatility={self.volatility:.4f}, Change={self.recent_volatility_change:.2f}"
                        )
                    else:
                        self.market_regime = "normal"
                        self.circuit_breaker_active = False
                    ...
                    if self.volatility > 0.01 or self.consecutive_down_days >= 3:
                        ...
                        if self.position:
                            return {
                                "direction": (
                                    "BUY" if self.position == "SHORT" else "SELL"
                                ),
                                "price": bar["close"],
                                "confidence": 1.0,
                    ...
                    if self.use_advanced_features and len(self.last_prices) > 5:
                        try:
                            ...
                            dark_pool_signal = self.dark_pool.analyze_dark_pool_signal(
                                "BTCUSD", bar["close"]
                            )
                            ...
                            gamma_signal = self.gamma_trap.analyze_gamma_hedging(
                                "SPX", bar["close"]
                            )
                            ...
                            sentiment_signal = self.sentiment.analyze_sentiment(
                                "BTCUSD"
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--event",
        type=str,
        choices=["covid_crash", "fed_panic", "flash_crash", "all"],
        default="all",
