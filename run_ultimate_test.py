...
def main():
    ...
    try:
        ...
        if results.get("overall_success", False):
            ...
            if "detailed_results" in results:
                for test_name, test_result in results["detailed_results"].items():
                    status = (
                        "✅ PASS" if test_result.get("success", False) else "❌ FAIL"
                    )
        ...
        if system.initialize():
            ...
            print(
                f"System Initialized: {'✅' if status.get('system_initialized', False) else '❌'}"
            )
            print(
                f"Never-Loss Active: {'✅' if status.get('never_loss_active', False) else '❌'}"
            )
