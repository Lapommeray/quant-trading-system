...
class RealityDisplacementMatrix:
    ...
    def decode(self, symbol, history_window):
        ...
        timelines = self._generate_alternate_timelines(
        ...
        true_direction = self._determine_true_direction(
            closes, timelines, manipulation_result["is_manipulated"]
        )
        ...
        if confidence > self.displacement_threshold:
            self.algo.Debug(
                f"RealityMatrix: {symbol} - Reality displacement detected! Score: {confidence:.2f}, True direction: {true_direction}"
            )
            if manipulation_result["is_manipulated"]:
                self.algo.Debug(
                    f"RealityMatrix: {symbol} - Manipulation pattern: {manipulation_result['pattern']}"
                )
    ...
    def _generate_volume_weighted_timeline(self, closes, volumes):
        ...
        for i in range(window, len(closes)):
            vwap = np.sum(closes[i - window : i] * volumes[i - window : i]) / np.sum(
                volumes[i - window : i]
            )
    ...
    def _generate_volatility_timeline(self, closes, highs, lows):
        ...
        for i in range(window, len(closes)):
            ...
            vol_impact = (
                min((vol_ratio - 1) * 0.1, 0.2)
                if vol_ratio > 1
                else max((vol_ratio - 1) * 0.1, -0.2)
            )
    ...
    def _detect_manipulation(self, closes, volumes, timelines):
        ...
        if len(recent_closes) >= 15:
            for i in range(5, len(recent_closes) - 5):
                pre_move = (recent_closes[i] - recent_closes[i - 5]) / recent_closes[
                    i - 5
                ]
                post_move = (recent_closes[i + 5] - recent_closes[i]) / recent_closes[i]

                if (pre_move > 0.02 and post_move < -0.02) or (
                    pre_move < -0.02 and post_move > 0.02
                ):
        ...
        for timeline in timelines:
            ...
            if len(recent_closes) == len(recent_timeline):
                ...
                timeline_deviations.append(
                    1 - correlation
                )  # Higher value = more deviation

        avg_timeline_deviation = (
            np.mean(timeline_deviations) if timeline_deviations else 0
        )

        max_pattern_score = max(patterns.values())
        manipulation_score = max(
            float(max_pattern_score), float(avg_timeline_deviation)
        )

        is_manipulated = manipulation_score > self.manipulation_threshold
        pattern = (
            max(patterns.items(), key=lambda x: x[1])[0]
