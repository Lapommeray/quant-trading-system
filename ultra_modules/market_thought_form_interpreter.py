...
class MarketThoughtFormInterpreter:
    ...
    def decode(self, symbol, history_window):
        ...
        if intent_score > self.thought_threshold:
            direction_votes = {
                ...
                "NEUTRAL": (volume_intent["direction"] == "NEUTRAL")
                * self.volume_weight
                + (price_intent["direction"] == "NEUTRAL") * self.price_weight,
            }

            collective_intent = max(
            ...
            self.algo.Debug(
                f"ThoughtForm: {symbol} - Collective intent detected! Direction: {collective_intent}, Score: {intent_score:.2f}"
            )
            self.algo.Debug(
                f"ThoughtForm: {symbol} - Patterns: {', '.join(thought_patterns)}"
            )
        else:
            collective_intent = "NEUTRAL"
            thought_patterns = []
            self.algo.Debug(
                f"ThoughtForm: {symbol} - No clear collective intent. Score: {intent_score:.2f}"
            )
    ...
    def _analyze_volume_patterns(self, volumes, prices):
        ...
        if recent_vol_z > 2.0:
            ...
            recent_price_trend = np.polyfit(
                range(len(recent_prices)), recent_prices, 1
            )[0]
    ...
    def _analyze_price_structure(self, opens, highs, lows, closes):
        ...
        for i in range(len(closes) - 1, max(len(closes) - 5, 0), -1):
            if (
                closes[i] > opens[i]  # Current candle is bullish
                and closes[i - 1] < opens[i - 1]  # Previous candle is bearish
                and closes[i] > opens[i - 1]  # Current close above previous open
                ...
                closes[i] < opens[i]  # Current candle is bearish
                and closes[i - 1] > opens[i - 1]  # Previous candle is bullish
                and closes[i] < opens[i - 1]  # Current close below previous open
                and opens[i] > closes[i - 1]
