...
class EmotionDNADecoder:
    ...
    def decode(self, symbol, history_window):
        ...
        fear_score = self._calculate_fear_score(
            body_to_range_ratios, upper_wicks, lower_wicks
        )
        greed_score = self._calculate_greed_score(
            body_to_range_ratios, upper_wicks, lower_wicks
        )

        is_favorable = (fear_score < self.fear_threshold) and (
            greed_score < self.greed_threshold
        )

        self.algo.Debug(
            f"EmotionDNA: {symbol} - Fear: {fear_score:.2f}, Greed: {greed_score:.2f}, Favorable: {is_favorable}"
        )
        return is_favorable
