...
class FractalResonanceGate:
    ...
    def decode(self, symbol, history_window):
        ...
        price_resonance = self._check_price_fibonacci(
        ...
        self.algo.Debug(
            f"FractalResonance: {symbol} - Score: {resonance_score:.2f}, Resonant: {is_resonant}"
        )
        return is_resonant
    ...
    def _check_price_fibonacci(self, highs, lows, closes, swing_highs, swing_lows):
        ...
        max_possible_matches = (
            len(movements) * (len(movements) - 1) / 2 * len(self.fibonacci_levels)
        )
        return min(
            fib_matches / max_possible_matches if max_possible_matches > 0 else 0, 1.0
        )
    ...
    def _check_time_resonance(self, timestamps, swing_highs, swing_lows):
        ...
        all_swings = [
            (idx, ts) for idx, _ in swing_highs + swing_lows for ts in [timestamps[idx]]
        ]
        ...
        max_possible_matches = (
            len(intervals) * (len(intervals) - 1) / 2 * len(self.fibonacci_levels)
        )
        return min(
            time_matches / max_possible_matches if max_possible_matches > 0 else 0, 1.0
        )
