...
class QuantumTremorScanner:
    ...
    def decode(self, symbol, history_window):
        ...
        if tremor_score > self.tremor_threshold:
            direction = self._determine_likely_direction(
                micro_volatility, volume_anomalies, price_microstructures
            )
            self.algo.Debug(
                f"QuantumTremor: {symbol} - Tremors detected! Score: {tremor_score:.2f}, Direction: {direction}"
            )
            return {
            ...
            self.algo.Debug(
                f"QuantumTremor: {symbol} - No significant tremors. Score: {tremor_score:.2f}"
            )
            return {
                "tremors_detected": False,
                "direction": None,
                "confidence": tremor_score,
            }
    ...
    def _calculate_micro_volatility(self, closes):
        ...
        std_baseline = (
            np.std(baseline_changes) if np.std(baseline_changes) > 0 else 1e-9
        )
        ...
        score = min(
            max_z_score / 3.0, 1.0
        )  # Cap at 1.0, 3 sigma is considered significant
    ...
    def _detect_volume_anomalies(self, volumes):
        ...
        recent_avg = (
            rolling_avg[-5] if len(rolling_avg) >= 5 else np.mean(volumes[-15:-5])
        )
    ...
    def _analyze_price_microstructure(self, highs, lows, closes):
        ...
        compression_ratio = (
            np.mean(baseline_ranges) / np.mean(recent_ranges)
            if np.mean(recent_ranges) > 0
            else 1.0
        )
    ...
    def _determine_likely_direction(
        self, micro_volatility, volume_anomalies, price_microstructures
    ):
        ...
        buy_votes = sum(
            weight for direction, weight in directions if direction == "BUY"
        )
        sell_votes = sum(
            weight for direction, weight in directions if direction == "SELL"
        )
