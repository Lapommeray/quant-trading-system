...
class FutureShadowDecoder:
    ...
    def decode(self, symbol, history_window):
        ...
        order_flow_imbalance = self._detect_order_flow_imbalance(
        ...
        if shadow_score > self.shadow_threshold:
            ...
            self.algo.Debug(
                f"FutureShadow: {symbol} - Future intention detected! Direction: {future_direction}, Score: {shadow_score:.2f}, Timeframe: {timeframe} bars"
            )
        else:
            future_direction = "NEUTRAL"
            timeframe = 0
            self.algo.Debug(
                f"FutureShadow: {symbol} - No clear future intention. Score: {shadow_score:.2f}"
            )
    ...
    def _detect_accumulation_distribution(self, closes, volumes):
        ...
        if recent_vol_price > earlier_vol_price * 1.2 and recent_price_trend <= 0:
            direction = "BUY"
            score = (
                min(recent_vol_price / (earlier_vol_price + 1e-9), 2.0) / 2.0
            )  # Normalize to [0,1]
            timeframe = self.future_window
        elif recent_vol_price < earlier_vol_price * 0.8 and recent_price_trend >= 0:
            direction = "SELL"
            score = (
                min(earlier_vol_price / (recent_vol_price + 1e-9), 2.0) / 2.0
            )  # Normalize to [0,1]
            timeframe = self.future_window
        else:
            direction = "NEUTRAL"
    ...
    def _detect_order_flow_imbalance(self, highs, lows, closes, volumes):
        ...
        close_positions = (closes[-20:] - lows[-20:]) / (
            ranges[-20:] + 1e-9
        )  # Avoid div by zero
