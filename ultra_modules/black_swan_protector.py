...
class BlackSwanProtector:
    ...
    def decode(self, symbol, history_window):
        ...
        if risk_score > 0.7:
            if volatility_risk["score"] > max(
                tail_risk["score"], correlation_risk["score"], liquidity_risk["score"]
            ):
                risk_type = "VOLATILITY_EXPLOSION"
            elif tail_risk["score"] > max(
                correlation_risk["score"], liquidity_risk["score"]
            ):
            ...
            self.algo.Debug(
                f"BlackSwan: {symbol} - High risk detected! Score: {risk_score:.2f}, Type: {risk_type}"
            )
    ...
    def _calculate_volatility_risk(self, closes):
        ...
        if len(vol_windows) >= 3:
            vol_acceleration = (vol_windows[-1] - vol_windows[-2]) - (
                vol_windows[-2] - vol_windows[-3]
            )

        score = min(vol_ratio / 3.0, 1.0)  # Cap at 1.0
        if vol_acceleration > 0:
            score = min(
                score + vol_acceleration * 10, 1.0
    ...
    def _calculate_correlation_breakdown(self, closes):
        ...
        for i in range(len(closes) - self.correlation_window * 2):
            ...
            segment2 = closes[
                i + self.correlation_window : i + self.correlation_window * 2
            ]
        ...
        recent_changes = (
            np.mean(correlation_changes[-5:]) if len(correlation_changes) >= 5 else 0
        )
        historical_changes = (
            np.mean(correlation_changes[:-5]) if len(correlation_changes) >= 10 else 0
        )
