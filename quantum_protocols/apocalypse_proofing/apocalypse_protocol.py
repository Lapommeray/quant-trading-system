...
class ApocalypseProtocol:
    ...
    def __init__(self, crash_threshold=0.65, volatility_threshold=3.5):
        ...
        logger.info(
            f"Initialized ApocalypseProtocol with crash_threshold={crash_threshold}"
        )
    ...
    def analyze_crash_risk(self, data: Dict) -> Dict:
        ...
        immunity_level = (
            self._generate_immunity(crash_probability) if crash_risk_detected else 0.0
        )
    ...
    def _detect_volatility_spike(self, prices: List[float]) -> Dict:
        ...
        probability = min(
            0.95,
            (
                max(0, (z_score - 1) / (self.volatility_threshold - 1))
                if z_score > 1
                else 0
            ),
        )
    ...
    def _detect_volume_anomaly(self, volumes: List[float]) -> Dict:
        ...
        probability = min(
            0.95, max(0, (volume_ratio - 1.5) / 3.5) if volume_ratio > 1.5 else 0
        )
    ...
    def _update_crash_indicators(
        self, symbol: str, crash_probability: float, immunity_level: float
    ) -> None:
    ...
    def apply_immunity_field(self, trading_signal: Dict) -> Dict:
        ...
        if original_signal in ["SELL", "STRONG_SELL"]:
            ...
            new_confidence = min(
                0.95, max(original_confidence, self.immunity_level * 0.7)
            )
