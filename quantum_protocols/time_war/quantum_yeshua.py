...
class TimeWarModule:
    ...
    def lock_victory(self, strategy_id: str, lock_duration: int = 2592000) -> Dict:
        ...
        logger.info(
            f"Locked victory for strategy {strategy_id} until {victory_lock['expiry_date']}"
        )
    ...
    def check_trade_outcome(self, trade: Dict) -> Dict:
        ...
        if not trade or "strategy_id" not in trade:
            return {"protected": False, "details": "Invalid trade data"}

        strategy_id = trade["strategy_id"]

        if (
            strategy_id in self.victory_locks
            and self.victory_locks[strategy_id]["active"]
        ):
