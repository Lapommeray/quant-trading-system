...
try:
    ...
    warnings.warn(
        "scikit-learn not available. Kernel cointegration will not be available."
    )
...
try:
    ...
    warnings.warn(
        "statsmodels.tsa.stattools not available. ADF test will not be available."
    )
...
class AdvancedCointegration:
    ...
    def johansen_test(self, prices_matrix: pd.DataFrame) -> Dict[str, Any]:
        ...
        if not STATSMODELS_AVAILABLE:
            self.logger.error(
                "statsmodels not available. Cannot perform Johansen test."
            )
        ...
        recent_prices = (
            prices_matrix.iloc[-self.lookback :]
            if len(prices_matrix) > self.lookback
            else prices_matrix
        )
    ...
    def kernel_cointegration(self, x: pd.Series, y: pd.Series) -> Dict[str, Any]:
        ...
        if not SKLEARN_AVAILABLE or not ADFULLER_AVAILABLE:
            self.logger.error(
                "scikit-learn or statsmodels not available. Cannot perform kernel cointegration."
            )
            return {
    ...
    def calculate_hedge_ratio(
        self, x: pd.Series, y: pd.Series, method="ols"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if method == "ols":
                ...
                return {
                    ...
                    "cointegrated": (
                        adf_result[1] < (1 - self.confidence_level)
                        if adf_result
                        else None
                    ),
                    "timestamp": datetime.now().isoformat(),
