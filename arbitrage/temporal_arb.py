...
class TemporalArbitrageEngine:
    ...
    def execute_arbitrage(self, opportunity_id):
        ...
        try:
            ...
            if len(self.active_arbitrages) >= self.settings["max_active_arbitrages"]:
                self.logger.warning(
                    f"Maximum active arbitrages reached: {self.settings['max_active_arbitrages']}"
                )
                return None
            ...
            if not entry_result:
                self.logger.warning(
                    f"Failed to execute entry for arbitrage: {opportunity_id}"
                )
                return None
    ...
    def close_arbitrage(self, arbitrage_id):
        ...
        try:
            ...
            if not exit_result:
                self.logger.warning(
                    f"Failed to execute exit for arbitrage: {arbitrage_id}"
                )
                return None
            ...
            self.logger.info(
                f"Arbitrage closed: {arbitrage_id}, profit: {results.get('profit', 0):.2%}"
            )
    ...
    def _create_opportunity(self, anomaly):
        ...
        opportunity = {
            ...
            "target_price": (
                current_price * (1.0 + anomaly.get("profit_potential", 0))
                if anomaly.get("direction", "") == "long"
                else current_price * (1.0 - anomaly.get("profit_potential", 0))
            ),
            ...
            "expiry_time": (
                datetime.now() + timedelta(seconds=anomaly.get("duration", 0))
            ).isoformat(),
        }
        ...
        for arbitrage_id, arbitrage in list(self.active_arbitrages.items()):
            ...
            if symbol in self.algorithm.Securities:
                ...
                if direction == "long" and current_price >= target_price:
                    self.logger.info(
                        f"Target reached for long arbitrage: {arbitrage_id}"
                    )
                    self.close_arbitrage(arbitrage_id)

                elif direction == "short" and current_price <= target_price:
                    self.logger.info(
                        f"Target reached for short arbitrage: {arbitrage_id}"
                    )
                    self.close_arbitrage(arbitrage_id)
