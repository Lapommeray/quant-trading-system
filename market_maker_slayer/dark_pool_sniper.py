...
class DarkPoolImpactPredictor:
    ...
    def predict_impact(self, dark_pool_print):
        ...
        if historical_prints:
            avg_size = sum([p["size"] for p in historical_prints]) / len(
                historical_prints
            )
        else:
            avg_size = size
        ...
        side_impact = (
            0.6 if side == "BUY" else 0.7
        )  # Sell side prints often have more impact
    ...
    def snipe_liquidity(self, symbol):
        ...
        if impact > 0.8:  # 80%+ chance of price movement
            return {
                ...
                "expected_move": impact
                * last_print["size"]
                / 1000,  # Scale down for reasonable move size
                "confidence": impact,
