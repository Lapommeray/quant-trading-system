...
class MarketMakerSlayer:
    ...
    def get_performance_stats(self, symbol=None):
        ...
        dark_pool_executions = sum(
            1 for e in executions if e["execution_type"] == "dark_pool"
        )
        flow_executions = sum(1 for e in executions if e["execution_type"] == "flow")
        stop_hunt_executions = sum(
            1 for e in executions if e["execution_type"] == "stop_hunt"
        )
        avg_confidence = sum(e["confidence"] for e in executions) / total_executions
        avg_expected_profit = (
            sum(e["expected_profit"] for e in executions) / total_executions
        )
