    ...
    def _initialize_population(self):
        ...
        if self.algorithm:
            self.algorithm.Debug(
                f"Agent Lab: Initialized population with {self.population_size} agents"
            )
    ...
    def darwin_cycle(self, market_data=None):
        ...
        for agent in self.agents:
            try:
                ...
                if self.algorithm:
                    self.algorithm.Debug(
                        f"Agent Lab: Error evaluating agent {agent.id} - {e}"
                    )
        ...
        top_performers = [
            agent for agent, _ in fitness_results[: int(self.population_size * 0.2)]
        ]
    ...
    def _mutate(self, agent, mutation_rate=0.1, mutation_amount=0.2):
        ...
        for gate in agent.gate_thresholds.keys():
            if random.random() < mutation_rate:
                ...
                agent.gate_thresholds[gate] = max(
                    0.0, min(1.0, agent.gate_thresholds[gate] + mutation)
                )

        for module in agent.module_weights.keys():
            if random.random() < mutation_rate:
                mutation = random.uniform(-mutation_amount, mutation_amount)
                agent.module_weights[module] = max(
                    0.0, min(1.0, agent.module_weights[module] + mutation)
                )

        for tf in agent.timeframe_weights.keys():
            if random.random() < mutation_rate:
                mutation = random.uniform(-mutation_amount, mutation_amount)
                agent.timeframe_weights[tf] = max(
                    0.0, min(1.0, agent.timeframe_weights[tf] + mutation)
                )
...
class QMPAgent:
    ...
    def evaluate_fitness(self, market_data=None):
        ...
        if market_data is None:
            ...
            fitness = (
                threshold_score * 0.4 + balance_score * 0.3 + timeframe_score * 0.3
            )
