...
class MMProfiler:
    ...
    def update_profile(self, mm_name, order_book):
        ...
        for behavior, value in behaviors.items():
            if behavior not in self.mm_profiles[mm_name]["behaviors"]:
                ...
                self.mm_profiles[mm_name]["behaviors"][behavior] = (
                    0.75 * old_value + 0.25 * value
                )
    ...
    def _extract_behaviors(self, order_book):
        ...
        behaviors = {
            ...
            "spoofing_probability": random.random()
            * 0.3,  # Lower probability for spoofing
    ...
    def load_profiles(self, filename):
        ...
        try:
            ...
            for mm, profile in serializable_profiles.items():
                ...
                self.mm_signatures[mm] = profile.get(
                    "signature", self._generate_signature(mm)
                )

            logger.info(
                f"Loaded {len(serializable_profiles)} MM profiles from {filename}"
            )
            return True
...
def main():
    ...
    parser.add_argument(
        "--output-dna", type=str, default=None, help="Output file for DNA profiles"
    )
