...
class MarketMorpher:
    ...
    def reshape_bid_ask(self, symbol, bid_shift, ask_shift):
        ...
        """
        self.logger.info(
            f"Reshaping bid/ask for {symbol}: bid_shift={bid_shift}, ask_shift={ask_shift}"
        )
        ...
        if not broadcast_success:
            self.logger.warning(
                f"Failed to broadcast updated liquidity profile for {symbol}"
            )
    ...
    def _create_liquidity_profile(self, symbol, parameters):
        ...
        try:
            ...
            for i in range(depth_levels):
                ...
                bid_depth.append({"level": i, "price": level_price, "size": level_size})
            ...
            for i in range(depth_levels):
                ...
                ask_depth.append({"level": i, "price": level_price, "size": level_size})
