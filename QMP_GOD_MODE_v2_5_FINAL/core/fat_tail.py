...
class FatTailRiskManager:
    ...
    def kelly_criterion_fat_tail(self, returns):
        ...
        if len(returns) < 10:
            return 0.005  # More conservative than standard Kelly (0.01)
            
        # Calculate standard Kelly first
        standard_kelly = self.kelly_criterion(returns)
        
        # Calculate fat-tail adjusted Kelly
        ...
        kelly_fraction = (-es_95 / sigma) * 0.33  # Use 1/3 Kelly for fat tails
        ...
        kelly_fraction = max(0.001, min(0.15, kelly_fraction))  # More conservative upper limit (0.15 vs 0.2)
        
        if returns.std() > 0.05:  # If daily vol > 5%
            kelly_fraction *= 0.5  # Further reduce position size
            
        kelly_fraction = min(kelly_fraction, standard_kelly * 0.8)
