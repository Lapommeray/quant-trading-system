...
def simulate_crash(
    ...
    base_volatility = {"SPX": 0.015, "BTC": 0.035, "XRP": 0.045}.get(asset, 0.025)
    ...
    for date in dates:
        if date < crash_start:
            ...
            progress = (date - crash_start).days / max(
                1, (crash_end - crash_start).days
            )
            daily_volatility = base_volatility * volatility_factor * (1 + progress)
            drift = (
                -0.02 * crash_intensity * (1 - progress * 0.5)
            )  # Strong downward bias
        else:
            progress = (date - recovery_start).days / max(
                1, (recovery_end - recovery_start).days
            )
            daily_volatility = (
                base_volatility * volatility_factor * (1 + (1 - progress) * 0.5)
            )
    ...
    for i, date in enumerate(dates):
        ...
        base_volume = {"SPX": 4e9, "BTC": 2e10, "XRP": 5e9}.get(asset, 1e9)
    ...
    for _ in range(num_events):
        ...
        if event_type in [
            ...
            sentiment = np.random.choice(
                ["Positive", "Negative", "Neutral"], p=[0.3, 0.5, 0.2]
            )
...
def save_simulated_data(
    ...
    news_events = generate_news_events(
        start_date=start_date, end_date=end_date, num_events=30, seed=42
    )
