...
from market_maker_slayer.order_flow_hunter import (
    OrderFlowHunter,
    OrderBookImbalanceScanner,
    HTFBehaviorDatabase,
)
...
class OrderFlowHunterQC(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.metrics = {"total_trades": 0, "winning_trades": 0, "losing_trades": 0}
    ...
    def GenerateSignals(self):
        ...
        for symbol_name, symbol in self.symbols.items():
            ...
            if result:
                self.Debug(
                    f"Signal for {symbol_name}: {result['expected_hft_action']} | Imbalance: {result['imbalance_ratio']} | Confidence: {result['confidence']}"
                )
    ...
    def OnOrderEvent(self, orderEvent):
        ...
        if orderEvent.Status == OrderStatus.Filled:
            self.Debug(
                f"Order {orderEvent.OrderId} filled: {orderEvent.FillQuantity} @ {orderEvent.FillPrice}"
            )
