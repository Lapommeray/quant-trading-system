...
class InstitutionalTradingQC(QCAlgorithm):
        ...
        for symbol_name, symbol in self.symbols.items():
            ...
            try:
                ...
                if signal["direction"] != "HOLD":
                    self.Debug(
                        f"Signal for {symbol_name}: {signal['direction']} | Confidence: {signal['confidence']}"
                    )
    ...
    def GenerateInstitutionalSignal(
        self, symbol_name, volatility, ml_prediction, regime
    ):
        """
        ...
        if execution_type == "vwap":
            ...
            if symbol_name in self.historical_volumes:
                ...
                schedule = vwap.get_execution_schedule(
                    target_quantity=self.Portfolio.Cash
                    * position_size
            ...
            target_shares = (
                self.Portfolio.Cash * position_size / self.Securities[symbol].Price
            )
