...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("candle_overlays.log")],
)
...
class VoidTraderOverlays:
    ...
    def create_candlestick_chart(
        ...
        try:
            ...
            colors = [
                (
                    self.up_color
                    if ohlc_data["close"][i] >= ohlc_data["open"][i]
            ...
            if self.show_void_levels and void_levels is not None:
                for i, level in enumerate(void_levels):
                    if (
                        "price" in level
                        and "start_time" in level
                        and "end_time" in level
                    ):
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="lines",
                                line=dict(
                                    color=self.void_level_color, width=2, dash="dash"
                                ),
            ...
            if self.show_fibonacci_grid:
                ...
                for i, (level, price) in enumerate(zip(fib_levels, fib_prices)):
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=[
                                ohlc_data["timestamp"].iloc[0],
                                ohlc_data["timestamp"].iloc[-1],
                            ],
                            y=[price, price],
                            mode="lines",
                            line=dict(
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    showgrid=True, gridcolor=self.grid_color, showticklabels=True
                ),
                xaxis2=dict(
                    showgrid=True, gridcolor=self.grid_color, showticklabels=True
                ),
    ...
    def create_quantum_heatmap(
        ...
        try:
            if probability_matrix.shape != (len(price_levels), len(time_points)):
                logger.error(
                    f"Probability matrix shape {probability_matrix.shape} does not match price_levels {len(price_levels)} and time_points {len(time_points)}"
                )
                return None
    ...
    def create_void_trader_dashboard(
        ...
        try:
            ...
            fig = make_subplots(
                ...
                subplot_titles=[title, "Volume"]
                + (["Quantum Probability"] if probability_matrix is not None else []),
            )
    ...
    def generate_ghost_candles(
        ...
        try:
            ...
            time_delta = (
                ohlc_data["timestamp"].iloc[-1] - ohlc_data["timestamp"].iloc[-2]
            )
            ...
            for i in range(num_candles):
                ...
                if price_change >= 0:
                    ...
                    ghost_high = ghost_close + np.random.uniform(
                        0, avg_range * 0.5 * volatility_factor
                    )
                    ghost_low = ghost_open - np.random.uniform(
                        0, avg_range * 0.3 * volatility_factor
                    )
                else:
                    ghost_open = last_close
                    ghost_close = ghost_open + price_change
                    ghost_high = ghost_open + np.random.uniform(
                        0, avg_range * 0.3 * volatility_factor
                    )
                    ghost_low = ghost_close - np.random.uniform(
                        0, avg_range * 0.5 * volatility_factor
                    )
    ...
    def generate_void_levels(
        self, ohlc_data: pd.DataFrame, num_levels: int = 3, lookback_periods: int = 50
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            if len(ohlc_data) < lookback_periods:
                logger.error(
                    f"Not enough data. Need at least {lookback_periods} periods."
                )
                return []
            ...
            for price in void_prices:
                level = {
                    ...
                    + (
                        ...
                        - recent_data["timestamp"].iloc[-2]
                    )
                    * 5,
                    "strength": 1.0
                    - (hist[np.digitize(price, bin_edges) - 1] / hist.max()),
                }
    ...
    def save_chart_to_image(
        self, fig: go.Figure, filepath: str, format: str = "png"
    ) -> bool:
        """
