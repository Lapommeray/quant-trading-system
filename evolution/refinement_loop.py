...
class StrategyEvolver:
    ...
    def _load_current_strategies(self):
        ...
        strategy_files = [
            f for f in os.listdir(self.strategies_dir) if f.endswith(".json")
        ]
        ...
        while len(strategies) < self.population_size:
            strategies.append(
                self._create_random_strategy(f"random_strategy_{len(strategies)}")
            )
    ...
    def _run_evolution(self):
        ...
        for generation in range(self.generations):
            ...
            self.logger.info(
                f"Generation {generation + 1}: Best fitness = {best_fitness:.4f}, Avg fitness = {avg_fitness:.4f}"
            )
    ...
    def _evaluate_strategy(self, strategy):
        ...
        if self.backtester:
            try:
                ...
                self.logger.error(
                    f"Error backtesting strategy {strategy['name']}: {str(e)}"
                )
                return 0.0
