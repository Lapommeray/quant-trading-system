...
try:
    ...
    MT5_AVAILABLE = False
    logger.warning(
        "MetaTrader5 not available (Windows only). Running in simulation mode."
    )
...
class CircuitBreaker:
    ...
    def check_order(
        self, order: OrderRequest, account: AccountInfo
    ) -> Tuple[bool, str]:
        """Check if order should be allowed"""
        if self.tripped:
            if self.trip_time and datetime.now() - self.trip_time > timedelta(
        ...
        recent_orders = sum(
            1 for t in self.order_timestamps if now - t < timedelta(minutes=1)
        )
            ...
            if order.side == OrderSide.BUY:
                ...
                if pos.quantity > 0:
                    new_avg = (
                        ...
                        + result.avg_fill_price * result.filled_quantity
                    ) / new_qty
                else:
...
class DepthFeedIntegration:
    ...
    def _get_order_book(self, symbol: str) -> Optional[Dict]:
        ...
        if self.engine.active_broker == BrokerType.MT5 and MT5_AVAILABLE:
            ...
            if tick:
                ...
                bids = [
                    (mid - spread / 2 - i * 0.0001, 1000 + random.randint(-200, 200))
                    for i in range(10)
                ]
                asks = [
                    (mid + spread / 2 + i * 0.0001, 1000 + random.randint(-200, 200))
    ...
    def get_all_beliefs(self) -> Dict[str, Dict]:
        ...
        return {
            s: self.get_belief_state(s) for s in self.symbols if s in self.market_states
        }
...
def demo():
    ...
    order = OrderRequest(
        symbol="EURUSD", side=OrderSide.BUY, quantity=0.1, order_type=OrderType.MARKET
    )
