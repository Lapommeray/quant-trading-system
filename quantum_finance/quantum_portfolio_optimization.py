...
class QuantumPortfolioOptimizer:
    ...
    def __init__(
        ...
        if risk_aversion <= 0 or entanglement_factor < 0 or crisis_boost <= 0:
            logger.error(
                f"Invalid parameters: risk_aversion={risk_aversion}, "
        ...
        if min_allocation < 0 or max_allocation > 1 or min_allocation >= max_allocation:
            logger.error(
                f"Invalid allocation bounds: min={min_allocation}, max={max_allocation}"
            )
        ...
        logger.info(
            f"Initialized QuantumPortfolioOptimizer with risk_aversion={risk_aversion}, "
            f"entanglement_factor={entanglement_factor}, crisis_boost={crisis_boost}"
        )
    ...
    def _detect_crisis(self, cov_matrix, returns, threshold=None):
        ...
        if is_crisis:
            logger.info(
                f"Crisis detected: score={crisis_score:.4f}, threshold={threshold:.4f}, "
                f"avg_vol={avg_vol:.4f}, avg_corr={avg_corr:.4f}, skew={returns_skew:.4f}"
            )
        else:
            logger.debug(
                f"Normal conditions: score={crisis_score:.4f}, threshold={threshold:.4f}"
            )

        return is_crisis, crisis_score

    def _quantum_adjusted_objective(
        self, weights, expected_returns, cov_matrix, crisis_score
    ):
        """
        Quantum-adjusted objective function for portfolio optimization
        ...
        for i in range(n_assets):
            for j in range(n_assets):
                if i != j:
                    quantum_penalty += (
                        weights[i] * weights[j] * cov_matrix[i, j] * crisis_score
                    )
        ...
        quantum_adjusted_return = (
            portfolio_return - adjusted_entanglement * quantum_penalty
        )
        quantum_adjusted_risk = np.sqrt(portfolio_variance) * (
            1 + 0.2 * crisis_score * concentration
        )
    ...
    def _apply_constraints(self, weights, min_allocation=None, max_allocation=None):
        ...
        if np.any(large_weights):
            ...
            if np.any(non_large):
                weights[non_large] += (
                    excess * weights[non_large] / np.sum(weights[non_large])
                )
    ...
    def optimize_portfolio(
        ...
        if cov_matrix.shape != (n_assets, n_assets):
            logger.error(
                f"Shape mismatch: expected_returns={n_assets}, cov_matrix={cov_matrix.shape}"
            )
            raise ValueError(
                "Covariance matrix shape must match expected returns length"
            )
        ...
        if market_returns is None:
            logger.warning(
                "No market returns provided, using synthetic data for crisis detection"
            )
        ...
        if is_crisis:
            ...
            logger.info(
                f"Crisis boost applied: entanglement_factor={self.entanglement_factor:.4f}"
            )
        ...
        logger.info(
            f"Portfolio optimized: return={portfolio_return:.4f}, risk={portfolio_risk:.4f}, "
    ...
    def optimize_multi_period(
        ...
        if len(expected_returns_series) < periods:
            logger.error(
                f"Insufficient expected returns data: {len(expected_returns_series)} < {periods}"
            )
            raise ValueError(
                "Expected returns series must have at least 'periods' elements"
            )

        if len(cov_matrix_series) < periods:
            logger.error(
                f"Insufficient covariance data: {len(cov_matrix_series)} < {periods}"
            )
            raise ValueError(
                "Covariance matrix series must have at least 'periods' elements"
            )

        if len(market_returns_series) < periods:
            logger.error(
                f"Insufficient market returns data: {len(market_returns_series)} < {periods}"
            )
            raise ValueError(
                "Market returns series must have at least 'periods' elements"
            )
        ...
        for t in range(periods):
            ...
            def objective_with_costs(new_weights):
                ...
                portfolio_variance = np.dot(
                    new_weights.T, np.dot(cov_matrix, new_weights)
                )
                ...
                adjusted_entanglement = self.entanglement_factor * (
                    1 + crisis_score * self.crisis_boost if is_crisis else 1
                )

                net_return = (
                    portfolio_return - cost - adjusted_entanglement * quantum_penalty
                )
            ...
            try:
                ...
                if not result.success:
                    logger.warning(
                        f"Period {t} optimization did not converge: {result.message}"
                    )
            ...
            logger.info(
                f"Period {t} optimized: return={period_return:.4f}, trades={trades:.4f}, "
        ...
        logger.info(
            f"Multi-period optimization completed: periods={periods}, "
            f"cumulative_return={cumulative_return:.4f}"
        )

        return all_weights

    def optimize_crisis_resistant(
        self,
        expected_returns,
        cov_matrix,
        market_returns=None,
        ...
        base_weights = self.optimize_portfolio(
        ...
        for i in range(stress_scenarios):
            stress_factor = (
                1.0 + (i + 1) * (stress_volatility_multiplier - 1.0) / stress_scenarios
            )
            ...
            for j in range(n_assets):
                for k in range(j + 1, n_assets):
                    corr_jk = cov_matrix[j, k] / np.sqrt(
                        cov_matrix[j, j] * cov_matrix[k, k]
                    )

                    new_corr = min(1.0, corr_jk + (1 - corr_jk) * 0.2 * stress_factor)

                    stressed_cov[j, k] = new_corr * np.sqrt(
                        stressed_cov[j, j] * stressed_cov[k, k]
                    )
                    stressed_cov[k, j] = stressed_cov[j, k]

            stress_weights = self.optimize_portfolio(
                expected_returns, stressed_cov, market_returns
            )
            scenario_weights.append(stress_weights)

            logger.info(
                f"Stress scenario {i+1} optimized: stress_factor={stress_factor:.2f}"
            )
        ...
        portfolio_variance = np.dot(
            combined_weights.T, np.dot(cov_matrix, combined_weights)
        )
        portfolio_risk = np.sqrt(portfolio_variance)
        sharpe_ratio = portfolio_return / portfolio_risk if portfolio_risk > 0 else 0
        ...
        logger.info(
            f"Crisis-resistant optimization completed: return={portfolio_return:.4f}, "
            f"risk={portfolio_risk:.4f}, sharpe={sharpe_ratio:.4f}"
        )

        return combined_weights

    def _meyer_baaquie_optimization(
        self, eigenvalues, eigenvectors, expected_returns, confidence_level=0.99
    ):
        """
        ...
        for i in range(n_assets):
            if eigenvalues[i] > 1e-10:  # Avoid division by zero
                quantum_weights += (
                    expected_returns[i] / eigenvalues[i]
                ) * eigenvectors[:, i]
            else:
                quantum_weights += (
                    expected_returns[i] * 100 * eigenvectors[:, i]
                )  # Large weight for zero-risk directions
    ...
    def estimate_federal_outperformance(
        self,
        portfolio_weights,
        expected_returns,
        cov_matrix,
        ...
        fed_returns = np.array(
            [indicator["return"] for indicator in federal_indicators.values()]
        )
        fed_risks = np.array(
            [indicator["risk"] for indicator in federal_indicators.values()]
        )
        fed_sharpes = np.array(
            [indicator["sharpe"] for indicator in federal_indicators.values()]
        )
        ...
        return_outperformance = (
            portfolio_return / fed_mean_return if fed_mean_return > 0 else float("inf")
        )
        risk_reduction = (
            1 - (portfolio_risk / fed_mean_risk) if fed_mean_risk > 0 else 1
        )
        sharpe_outperformance = (
            portfolio_sharpe / fed_mean_sharpe if fed_mean_sharpe > 0 else float("inf")
        )
        ...
        quantum_risk = np.sqrt(
            np.dot(quantum_weights.T, np.dot(cov_matrix, quantum_weights))
        )
        quantum_sharpe = quantum_return / quantum_risk if quantum_risk > 0 else 0

        quantum_return_outperformance = (
            quantum_return / fed_mean_return if fed_mean_return > 0 else float("inf")
        )
        quantum_risk_reduction = (
            1 - (quantum_risk / fed_mean_risk) if fed_mean_risk > 0 else 1
        )
        quantum_sharpe_outperformance = (
            quantum_sharpe / fed_mean_sharpe if fed_mean_sharpe > 0 else float("inf")
        )
        ...
        for i in range(bootstrap_samples):
            ...
            bootstrap_outperformances[i] = (
                quantum_return / bootstrap_fed_mean
        ...
        if len(bootstrap_outperformances) > 0:
            ...
            lower_bound = (
                np.percentile(bootstrap_outperformances, lower_percentile)
                if len(bootstrap_outperformances) > 0
                else 0
            )
            upper_bound = (
                np.percentile(bootstrap_outperformances, upper_percentile)
                if len(bootstrap_outperformances) > 0
                else 0
            )
        else:
    ...
    def get_statistics(self):
        ...
        multi_period_count = sum(
            1 for h in self.history if h.get("type") == "multi_period"
        )
        crisis_resistant_count = sum(
            1 for h in self.history if h.get("type") == "crisis_resistant"
        )
        federal_outperformance_count = sum(
            1 for h in self.history if h.get("type") == "federal_outperformance"
        )

        returns = [
            h.get("portfolio_return", 0)
            for h in self.history
            if "portfolio_return" in h
        ]
        risks = [
            h.get("portfolio_risk", 0) for h in self.history if "portfolio_risk" in h
        ]
        sharpes = [
            h.get("sharpe_ratio", 0) for h in self.history if "sharpe_ratio" in h
        ]
        ...
        crisis_scores = [
            h.get("crisis_score", 0) for h in self.history if "crisis_score" in h
        ]
        avg_crisis_score = np.mean(crisis_scores) if crisis_scores else 0

        # Extract federal outperformance statistics
        outperformances = [
            h.get("result", {}).get("quantum_return_outperformance", 0)
            for h in self.history
            if h.get("type") == "federal_outperformance"
        ]
        ...
        stats = {
            ...
            "max_federal_outperformance": (
                float(np.max(outperformances)) if outperformances else 0
            ),
        }
    ...
    class TestQuantumPortfolioOptimizer(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.qpo = QuantumPortfolioOptimizer(
                risk_aversion=3.0, entanglement_factor=0.2, crisis_boost=2.0
            )
            ...
            self.crisis_returns[80:] = np.random.normal(
                -0.02, 0.03, 20
            )  # Add crisis at the end

        def test_crisis_detection(self):
            """Test crisis detection"""
            is_normal_crisis, normal_score = self.qpo._detect_crisis(
                self.cov_matrix, self.normal_returns
            )

            is_crisis, crisis_score = self.qpo._detect_crisis(
                self.cov_matrix, self.crisis_returns
            )
        ...
        def test_crisis_resistant_optimization(self):
            ...
            standard_risk = np.sqrt(
                np.dot(standard_weights.T, np.dot(self.cov_matrix, standard_weights))
            )
            resistant_risk = np.sqrt(
                np.dot(resistant_weights.T, np.dot(self.cov_matrix, resistant_weights))
            )
        ...
        def test_constraint_application(self):
            ...
            constrained = self.qpo._apply_constraints(
                weights, min_allocation=0.01, max_allocation=0.5
            )
