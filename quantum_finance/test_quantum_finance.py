...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("quantum_finance_test.log"), logging.StreamHandler()],
)
...
def test_quantum_black_scholes():
    ...
    for vol in volatilities:
        ...
        logger.info(
            f"Volatility: {vol:.2f}, Standard Price: ${standard_price:.4f}, Quantum Price: ${quantum_price:.4f}, Premium: {premium:.2f}%"
        )
...
def test_quantum_stochastic_process():
    ...
    for vol_idx in volatility_indices:
        ...
        logger.info(
            f"Volatility Index: {vol_idx:.2f}, Crisis Factor: {params['crisis_factor']:.2f}, Jump Intensity: {params['intensity']:.4f}"
        )

    logger.info("Simulating price paths")

    times, normal_prices = qsp.simulate_price_path(
        s0, mu, sigma, t, dt, volatility_index=0.2, seed=42
    )
    _, crisis_prices = qsp.simulate_price_path(
        s0, mu, sigma, t, dt, volatility_index=0.6, seed=42
    )
...
def test_quantum_portfolio_optimization():
    ...
    normal_weights = qpo.optimize_portfolio(
    ...
    logger.info(
        f"Normal scenario - Return: {normal_return:.4f}, Risk: {normal_risk:.4f}, Sharpe: {normal_sharpe:.4f}"
    )
    logger.info(f"Normal weights: {normal_weights}")

    crisis_weights = qpo.optimize_portfolio(
        expected_returns, cov_matrix, crisis_returns
    )
    ...
    logger.info(
        f"Crisis scenario - Return: {crisis_return:.4f}, Risk: {crisis_risk:.4f}, Sharpe: {crisis_sharpe:.4f}"
    )
...
def test_quantum_risk_measures():
    ...
    crisis_correlation = np.array([[1.0, 0.8, 0.7], [0.8, 1.0, 0.9], [0.7, 0.9, 1.0]])

    for i in range(n_days - 20, n_days):
        crisis_returns[i] = np.random.multivariate_normal(
            [-0.02, -0.02, -0.02], crisis_correlation * 0.03**2, 1
        )[0]
    ...
    logger.info(
        f"Normal scenario - Classical VaR: {normal_classical_var:.6f}, Quantum VaR: {normal_var:.6f}"
    )
    logger.info(
        f"Normal scenario - Classical CVaR: {normal_classical_cvar:.6f}, Quantum CVaR: {normal_cvar:.6f}"
    )
    ...
    logger.info(
        f"Crisis scenario - Classical VaR: {crisis_classical_var:.6f}, Quantum VaR: {crisis_var:.6f}"
    )
    logger.info(
        f"Crisis scenario - Classical CVaR: {crisis_classical_cvar:.6f}, Quantum CVaR: {crisis_cvar:.6f}"
    )
    ...
    logger.info(
        f"Stress test - Stressed VaR: {stress_results['stressed_var']:.6f}, Stressed CVaR: {stress_results['stressed_cvar']:.6f}"
    )
    logger.info(
        f"Stress test - Worst-case loss: {stress_results['worst_case_loss']:.6f}, Probability of extreme loss: {stress_results['prob_extreme_loss']:.2%}"
    )
...
def test_enhanced_modules():
    ...
    logger.info(
        f"Standard Entropy: {standard_entropy:.4f}, Quantum Entropy: {quantum_entropy:.4f}"
    )

    account_size = 10000
    current_price = prices[-1]

    standard_position = es_quantum.position_size(
        standard_entropy, account_size, current_price
    )
    quantum_position = es_quantum.position_size_quantum(
        quantum_entropy, account_size, current_price, returns=returns
    )

    logger.info(
        f"Standard Position Size: {standard_position['position_size']:.2f}, Quantum Position Size: {quantum_position['position_size']:.2f}"
    )
    ...
    bids = {
        prices[-1] * (1 - 0.001 * i): volumes[-1] * (1 - 0.1 * i) for i in range(10)
    }
    asks = {
        prices[-1] * (1 + 0.001 * i): volumes[-1] * (1 - 0.2 * i) for i in range(10)
    }

    standard_signal, ratio = lm_quantum.scan_liquidity(bids, asks)
    _, _, quantum_signal = lm_quantum.scan_liquidity_quantum(
        bids, asks, prices, volatility_index=0.3
    )
...
def test_integration():
    ...
    logger.info(
        f"Market State: {analysis['market_state']}, Direction: {analysis['direction']}, Confidence: {analysis['confidence']:.2f}"
    )
    logger.info(
        f"Quantum Premium: {analysis['quantum_premium']:.4f}, Quantum VaR: {analysis['quantum_var']:.4f}"
    )
    ...
    portfolio = qfi.optimize_portfolio(
        ["BTC", "ETH", "XRP"], expected_returns, cov_matrix, asset_returns
    )

    logger.info(
        f"Expected Return: {portfolio['expected_return']:.4f}, Expected Risk: {portfolio['expected_risk']:.4f}"
    )
    ...
    logger.info(
        f"Stressed VaR: {stress_results['stressed_var']:.6f}, Stressed CVaR: {stress_results['stressed_cvar']:.6f}"
    )
    logger.info(
        f"Worst-case loss: {stress_results['worst_case_loss']:.6f}, Probability of extreme loss: {stress_results['prob_extreme_loss']:.2%}"
    )
    ...
    logger.info(
        f"Trading Signal: {signal['signal']}, Direction: {signal['direction']}, Position Size: {signal['position_size']:.2f}"
    )
