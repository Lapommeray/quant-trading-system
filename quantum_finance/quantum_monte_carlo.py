...
class QuantumMonteCarlo:
    ...
    def __init__(self, num_qubits=5, num_shots=1000, quantum_speedup=True):
        ...
        self.logger.info(
            f"Initialized QuantumMonteCarlo with {num_qubits} qubits, "
    ...
    def _quantum_monte_carlo(self, payoff_func, num_samples):
        ...
        """
        effective_samples = (
            int(np.sqrt(num_samples)) if self.quantum_speedup else num_samples
        )
        ...
        std_error = (
            np.std(samples) / num_samples
            if self.quantum_speedup
            else np.std(samples) / np.sqrt(num_samples)
        )

        return price, std_error

    def price_european_option(
        self, s0, k, r, sigma, t, option_type="call", num_samples=10000
    ):
        """
        ...
        self.logger.info(
            f"Priced {option_type} option using {'quantum' if self.quantum_speedup else 'classical'} "
    ...
    def price_asian_option(
        self, s0, k, r, sigma, t, n_steps, option_type="call", num_samples=10000
    ):
        """
        ...
        self.logger.info(
            f"Priced Asian {option_type} option using {'quantum' if self.quantum_speedup else 'classical'} "
            f"Monte Carlo: price={price:.4f}, std_error={std_error:.6f}"
        )
    ...
    def price_barrier_option(
        ...
        self.logger.info(
            f"Priced {barrier_type} {option_type} option using {'quantum' if self.quantum_speedup else 'classical'} "
            f"Monte Carlo: price={price:.4f}, std_error={std_error:.6f}"
        )
    ...
    def price_basket_option(
        ...
        def payoff_func():
            ...
            for i in range(n_assets):
                s_t[i] = s0_vector[i] * np.exp(
                    (r - 0.5 * sigma_vector[i] ** 2) * t
                    + sigma_vector[i] * np.sqrt(t) * correlated_z[i]
                )
        ...
        self.logger.info(
            f"Priced basket {option_type} option using {'quantum' if self.quantum_speedup else 'classical'} "
            f"Monte Carlo: price={price:.4f}, std_error={std_error:.6f}"
        )
    ...
    def compare_quantum_classical(self, option_func, option_params, num_samples_range):
        ...
        for num_samples in num_samples_range:
            ...
            self.logger.info(
                f"  Quantum: price={q_price:.4f}, error={q_error:.6f}, time={q_time:.4f}s"
            )
            self.logger.info(
                f"  Classical: price={c_price:.4f}, error={c_error:.6f}, time={c_time:.4f}s"
            )
    ...
    def adaptive_crisis_sampling(
        self, payoff_func, num_samples, volatility_index, crisis_threshold=0.3
    ):
        """
        ...
        if volatility_index > crisis_threshold:
            crisis_factor = (
                volatility_index / crisis_threshold
            ) ** 2  # Quadratic scaling for severe crises
            adjusted_samples = int(num_samples * crisis_factor)
            self.logger.info(
                f"QUANTUM CRISIS DETECTED (vol_idx={volatility_index:.2f}), "
        ...
        if self.quantum_speedup:
            ...
            price, std_error = self._classical_monte_carlo(
                payoff_func, adjusted_samples
            )
    ...
    def price_european_option_crisis(
        self,
        s0,
        k,
        r,
        ...
        result = self.adaptive_crisis_sampling(
            payoff_func, num_samples, volatility_index
        )
        ...
        self.logger.info(
            f"Priced {option_type} option using adaptive crisis sampling: "
    ...
    def get_statistics(self):
        ...
        quantum_errors = [
            h.get("std_error", 0) for h in self.history if h.get("method") == "quantum"
        ]
        classical_errors = [
            h.get("std_error", 0)
            for h in self.history
            if h.get("method") == "classical"
        ]

        adaptive_count = sum(
            1 for h in self.history if h.get("type") == "adaptive_crisis_sampling"
        )
        ...
        stats = {
            ...
            "avg_quantum_error": (
                float(np.mean(quantum_errors)) if quantum_errors else 0
            ),
            "avg_classical_error": (
                float(np.mean(classical_errors)) if classical_errors else 0
            ),
