...
class NewsFilter:
    ...
    def __init__(self, news_data_path=None, market_news_path=None, min_wait_minutes=30):
        ...
        if market_news_path and os.path.exists(market_news_path):
            ...
            logger.warning(
                f"Market news categories file not found at {market_news_path}"
            )
    ...
    def load_news_data(self, news_data_path):
        ...
        try:
            with open(news_data_path, "r") as f:
                ...
                logger.info(
                    f"Loaded {len(self.news_events)} news events from {news_data_path}"
                )
    ...
    def load_market_news_categories(self, market_news_path):
        ...
        try:
            with open(market_news_path, "r") as f:
                ...
                category_count = sum(
                    len(subcategories)
                    for category in self.market_news_categories.values()
                    for subcategories in category.values()
                )
                logger.info(
                    f"Loaded {category_count} market news categories from {market_news_path}"
                )
        except Exception as e:
    ...
    def add_news_event(
        self, event_time, event_type, event_impact, event_description, assets=None
    ):
        """
        ...
        if self.last_news_time is None or event_time > datetime.fromisoformat(
            self.last_news_time
        ):
            ...
            for subcategory_name, subcategory in category.items():
                for event_code, event in subcategory.items():
                    ...
                    if release_day == "Varies":
                        ...
                        continue
                    elif "Monthly" in release_day and day_of_month not in [
                        1,
                        15,
                        30,
                        31,
                        ...
                        continue
                    elif "Quarterly" in release_day and current_time.month not in [
                        1,
                        4,
                        7,
                        10,
                        ...
                        release_day not in ["Varies", "Monthly", "Quarterly"]
                        and release_day != day_of_week
                    ):
                        continue

                    return (
                        True,
                        f"{event.get('name', 'Unknown event')} ({event.get('description', '')})",
                    )
        ...
        if self.news_events:
            for event in self.news_events:
                ...
                if 0 <= time_since_event < self.min_wait_minutes:
                    logger.info(
                        f"Not safe to trade: {event['description']} occurred {time_since_event:.1f} minutes ago"
                    )
                    self.safe_to_trade = False
                    return False

        # Check scheduled news events
        is_scheduled, event_description = self.is_scheduled_news_event(
            current_time, asset
        )
        if is_scheduled:
            logger.info(
                f"Not safe to trade: Scheduled news event {event_description} at {current_time}"
            )
            self.safe_to_trade = False
            return False
    ...
    def get_next_safe_time(self, current_time=None, asset=None):
        ...
        for event in self.news_events:
            ...
            if event_time <= current_time and (
                most_recent_time is None or event_time > most_recent_time
            ):
    ...
    def save_news_data(self, news_data_path=None):
        ...
        try:
            with open(news_data_path, "w") as f:
                ...
                logger.info(
                    f"Saved {len(self.news_events)} news events to {news_data_path}"
                )
        except Exception as e:
    ...
    def get_status(self):
        ...
        return {
            ...
            "market_news_categories_count": sum(
                len(subcategories)
                for category in self.market_news_categories.values()
                for subcategories in category.values()
            ),
        ...
        try:
            ...
            if source.lower() == "newsapi":
                ...
                if response.status_code == 200:
                    ...
                    for article in articles:
                        ...
                        if published_at and title:
                            try:
                                ...
                                logger.error(
                                    f"Invalid published_at format: {published_at}"
                                )
                    ...
                    logger.error(
                        f"Error fetching news from NewsAPI: {response.status_code}"
                    )
                    return []
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Error fetching news from CryptoPanic: {response.status_code}"
                    )
                    return []
            else:
