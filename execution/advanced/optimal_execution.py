...
try:
    ...
    warnings.warn(
        "cvxpy not available. OptimalExecution will have limited functionality."
    )
...
class OptimalExecution:
    ...
    def solve_optimal_strategy(
        ...
        try:
            ...
            for t in range(time_horizon):
                constraints.append(
                    remaining_inventory[t + 1] == remaining_inventory[t] - x[t]
                )
            ...
            variance_cost = cp.sum(
                [
                    self.risk_aversion
                    * (volatility**2)
                    * cp.square(remaining_inventory[t])
                    for t in range(time_horizon)
                ]
            )
    ...
    def calculate_implementation_shortfall(
        ...
        weighted_avg_price = (
            np.sum(executed_prices * executed_quantities) / total_quantity
        )
