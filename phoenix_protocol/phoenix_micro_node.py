...
class PhoenixProtocol:
    ...
    def get_phoenix_action(self, current_market_metrics):
        ...
        if self.algorithm:
            self.algorithm.Debug(
                f"Phoenix Protocol: {regime_status} regime, collapse score {collapse_info['score']:.2f}"
            )
            self.algorithm.Debug(
                f"Phoenix Action: {decision['action']} (multiplier: {decision['position_multiplier']:.2f})"
            )
    ...
    def _detect_collapse_pattern(self, metrics):
        ...
        term_structure_factor = max(
            0, min(1, (1.0 - metrics["vix_term_structure"]) * 2)
        )
        ...
        return {"score": score, "match": match, "failure_mode": failure_mode}
    ...
    def _anti_failure_decision(self, regime, collapse_score):
        ...
        return {"action": action, "position_multiplier": multiplier, "notes": notes}
