...
def main() -> int:
    ...
    check(
        "modules discovered",
        len(active) >= 8,
        f"{len(active)} active: {sorted(active)}",
    )
    check(
        "wiring failures",
        not wiring.get("failed"),
        json.dumps(wiring.get("failed", {}))[:200],
    )
    ...
    check(
        "event bus 4 lanes",
        set(lane_stats) >= {"critical", "operational", "adaptive", "evolutionary"},
    ...
    bus.subscribe(
        "WHALE_FLOW", lambda e: events.append({"event_type": e.event_type, **e.payload})
    )
    bus.subscribe(
        "LIQUIDATION_EVENT",
        lambda e: events.append({"event_type": e.event_type, **e.payload}),
    )
    ...
    check(
        "OKX frames parsed",
        feed.feed_status()["stats"]["frames"] == 3,
        json.dumps(feed.feed_status()["stats"]),
    )
    check(
        "DataRing populated (BTCUSDT)",
        len(ring_snapshot) >= 5,
        f"bid={ring_snapshot[-1]['bid']} ask={ring_snapshot[-1]['ask']}",
    )
    whale_events = [e for e in events if e["event_type"] == "WHALE_FLOW"]
    check(
        "whale flow event (0.5 BTC = $54k)",
        len(whale_events) >= 1,
        json.dumps(whale_events[0]) if whale_events else "none",
    )
    check(
        "whale cluster detected",
        bool(whale_events) and any(e.get("cluster") for e in whale_events),
    )
    check(
        "liquidation event", any(e["event_type"] == "LIQUIDATION_EVENT" for e in events)
    )
    ...
    check(
        "SPX ring populated",
        spx_status["last_price"] == 5530.25 and len(spx_ring.latest(2)) >= 1,
    ...
    check(
        "OFI analyze ran",
        res_ofi is not None,
        f"signal={res_ofi.signal} conf={res_ofi.confidence:.2f}",
    )
    check(
        "CVD analyze ran",
        res_cvd is not None,
        f"signal={res_cvd.signal} conf={res_cvd.confidence:.2f}",
    )
    ...
    for i in range(6):
        learning.record_outcome(
            prediction_id=f"smoke_{i}",
            module_name="ofi_detector",
            symbol="BTCUSDT",
            pnl=-0.012 if i % 2 == 0 else 0.004,
            correct=(i % 2 == 1),
            reason="loss" if i % 2 == 0 else "win",
            regime="range",
        )
    ...
    check(
        "outcomes recorded",
        stats.get("outcomes", stats.get("total_outcomes", 0)) >= 6,
    ...
    check(
        "self-code proposal produced",
        result.get("status") in ("validated", "approved", "applied", "pending_approval")
        or result.get("proposal_status")
        in ("VALIDATED", "APPROVED", "APPLIED", "PENDING_APPROVAL"),
        json.dumps({k: v for k, v in result.items() if k != "code"})[:240],
    )
    check(
        "self-code validated",
        result.get("validated", result.get("validation", {}).get("passed", False))
        in (True, "passed", "validated"),
    )
    ...
    p_retail = planner.plan(
        "BTCUSDT", "buy", 0.60, {**base_market, "ofi_z": 0.3, "whale_conf": 0.1}
    )
    check(
        "weak signal aborts (no retail chasing)",
        p_retail.routing.value == "abort",
        p_retail.reason,
    )

    p_flow = planner.plan(
        "BTCUSDT", "buy", 0.72, {**base_market, "ofi_z": 0.9, "whale_conf": 0.3}
    )
    check(
        "flow OK -> maker inside spread",
        p_flow.routing.value == "maker" and p_flow.limit_price < 109234.1,
        f"limit={p_flow.limit_price} reason={p_flow.reason}",
    )

    p_whale = planner.plan(
        "BTCUSDT",
        "buy",
        0.90,
        {**base_market, "ofi_z": 2.4, "whale_conf": 0.92, "cvd_align": True},
    )
    check(
        "whale stack -> immediate taker",
        p_whale.routing.value == "taker",
        p_whale.reason,
    )

    p_recheck = planner.finalize_after_recheck(
        p_flow, {**base_market, "ofi_z": 1.8, "whale_conf": 0.6, "urgency": 0.7}, 210
    )
    check(
        "recheck escalates with flow",
        p_recheck.routing.value == "taker",
        p_recheck.reason,
    )

    p_dead = planner.finalize_after_recheck(
        p_flow, {**base_market, "ofi_z": -0.2, "whale_conf": 0.0, "urgency": 0.1}, 260
    )
    check(
        "dead flow after timeout aborts", p_dead.routing.value == "abort", p_dead.reason
    )
