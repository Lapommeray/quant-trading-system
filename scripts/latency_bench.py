...
def bench_symbol(symbol="BTCUSDT", n=100):
    ...
    # warmup pushes
    for i in range(200):
        ring.push(time.time(), 67000+i*0.01, 67000.5+i*0.01, 1.2+ (i%5)*0.1, 0.8, 67000.2+i*0.01, 0.1+ (i%3)*0.2, 1 if i%2==0 else -1)
    ...
    for _ in range(n):
        # simulate new tick
        ring.push(time.time(), 67000+np.random.randn()*0.5, 67000.5+np.random.randn()*0.5, 1.2, 0.8, 67000.2, 0.3, 1)
    ...
    for k,v in latencies.items():
        arr = np.array(v)
        print(f"{symbol} {k}: p50 {np.percentile(arr,50):.2f}ms p95 {np.percentile(arr,95):.2f}ms avg {np.mean(arr):.2f}ms max {np.max(arr):.2f}ms")
