...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("deploy_mt5.log"), logging.StreamHandler()],
)
...
class MT5Deployer:
    ...
    def initialize(self) -> bool:
        ...
        self.engine = MT5LiveEngine(
            risk_limits=risk_limits, simulation_mode=self.config.paper_trading
        )
    ...
    def _process_symbol(self, symbol: str):
        ...
        order = Order(
            symbol=symbol,
            order_type=(
                OrderType.MARKET_BUY if direction == "BUY" else OrderType.MARKET_SELL
            ),
            volume=volume,
            stop_loss=stop_loss,
        ...
        logger.info(
            f"Executing {direction} order for {symbol} (confidence: {confidence:.2f})"
        )
