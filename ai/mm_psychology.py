...
class MMPsychology:
    ...
    def __init__(self, sensitivity=0.8, memory_length=50, emotion_threshold=0.7):
        ...
        logger.info(
            f"Initialized MMPsychology with sensitivity: {sensitivity}, "
    ...
    def _extract_features(self, market_data, order_flow=None):
        ...
        if "prices" in market_data and len(market_data["prices"]) > 1:
            ...
            features["acceleration"] = (
                np.diff(returns).mean() if len(returns) > 1 else 0
            )
        else:
        ...
        if order_flow:
            if "cancellations" in order_flow and "placements" in order_flow:
                cancel_rate = order_flow["cancellations"] / max(
                    1, order_flow["placements"]
                )
            ...
            if "modifications" in order_flow:
                features["modification_rate"] = order_flow["modifications"] / max(
                    1, order_flow["placements"]
                )
            else:
    ...
    def _analyze_emotional_state(self, features):
        ...
        greed_score = max(0, features["recent_trend"]) * max(
            0, features["volume_trend"]
        )
        ...
        confidence_score = max(0, features["recent_trend"]) * (
            1 - min(1, features["volatility"] * 10)
        )
        ...
        for emotion in self.emotional_states:
            self.emotional_states[emotion] = (
                0.8 * self.emotional_states[emotion] + 0.2 * emotional_state[emotion]
            )
    ...
    def analyze_sleep_cycle(self, mm_identifier=None, market_data=None):
                    ...
                    if "sleep_cycles" in mm_dna:
                        for cycle in mm_dna["sleep_cycles"]:
                            if cycle["period"] == current_session:
                                ...
                                sleep_cycle["aggression_factor"] = cycle[
                                    "aggression_factor"
                                ]
                                sleep_cycle["preferred_patterns"] = cycle[
                                    "preferred_patterns"
                                ]
                                break
        ...
        logger.info(
            f"MM Sleep Cycle: {current_session} (Activity: {sleep_cycle['activity_level']:.2f}, "
    ...
    def _predict_decision_bias(self, emotional_state, mm_identifier=None):
        ...
        decision_bias["contrarian"] = (
            0.8
            * adjusted_emotional_state["confidence"]
        ...
        decision_bias["aggressive_positioning"] = (
            0.7
            * adjusted_emotional_state["greed"]
        ...
        if mm_identifier and mm_identifier in self.mm_profiles:
            ...
            for bias in decision_bias:
                if bias in profile["bias_tendency"]:
                    decision_bias[bias] = (
                        0.7 * decision_bias[bias] + 0.3 * profile["bias_tendency"][bias]
                    )
    ...
    def _update_mm_profile(self, mm_identifier, emotional_state, decision_bias):
        ...
        profile["emotional_history"].append(
            {"timestamp": datetime.now(), "emotional_state": emotional_state}
        )

        if len(profile["emotional_history"]) > self.memory_length:
            profile["emotional_history"] = profile["emotional_history"][
                -self.memory_length :
            ]

        for bias, value in decision_bias.items():
            profile["bias_tendency"][bias] = (
                0.9 * profile["bias_tendency"][bias] + 0.1 * value
            )
        ...
        if len(self.memory) >= 2:
            ...
            volatility_change = (
                recent_features[1]["volatility"] - recent_features[0]["volatility"]
            )
            trend_change = (
                recent_features[1]["recent_trend"] - recent_features[0]["recent_trend"]
            )
            ...
            if volatility_change != 0:
                ...
                profile["volatility_response"] = (
                    0.9 * profile["volatility_response"] + 0.1 * volatility_response
                )

            if trend_change != 0:
                trend_response = sum(emotional_change.values()) / trend_change
                profile["trend_response"] = (
                    0.9 * profile["trend_response"] + 0.1 * trend_response
                )
    ...
    def _log_emotional_shifts(self, emotional_state, mm_identifier=None):
        ...
        for emotion, value in emotional_state.items():
            if (
                value >= self.emotion_threshold
                and value > self.emotional_states[emotion] * 1.5
            ):
    ...
    def _generate_market_data(self, market_scenario):
        ...
        """
        market_data = {"prices": [], "volumes": [], "sentiment": 0.0}
    ...
    def _generate_order_flow(self, mm_type, market_scenario):
        ...
        """
        order_flow = {"placements": 100, "cancellations": 0, "modifications": 0}
    ...
    def _plot_mm_simulation(self, market_data, analysis, mm_type, market_scenario):
        ...
        try:
            ...
            plt.title(
                f"Market Maker Psychology Simulation: {mm_type.capitalize()} MM in {market_scenario.capitalize()} Market"
            )
            plt.ylabel("Price")
            plt.grid(True)
            ...
            plt.subplot(3, 1, 2)
            plt.bar(
                range(len(market_data["volumes"])), market_data["volumes"], alpha=0.7
            )
            plt.ylabel("Volume")
            plt.grid(True)
            ...
            plt.bar(
                emotional_state.keys(),
                emotional_state.values(),
                color="blue",
                alpha=0.7,
            )
            plt.axhline(
                y=self.emotion_threshold,
                color="r",
                linestyle="--",
            ...
            bias_text = "\n".join(
                [f"{bias}: {value:.2f}" for bias, value in decision_bias.items()]
            )
            ...
            logger.info(
                f"Saved plot to output/mm_psychology_{mm_type}_{market_scenario}.png"
            )
...
def main():
    ...
    parser.add_argument(
        "--simulate", action="store_true", help="Run market maker simulation"
    )
    ...
    parser.add_argument(
        "--sensitivity", type=float, default=0.8, help="Detection sensitivity (0.0-1.0)"
    )
    ...
    if args.simulate:
        result = mm_psychology.simulate_mm_behavior(
            mm_type=args.mm_type, market_scenario=args.market_scenario
        )
        ...
        print(
            f"  - Dominant Emotion: {dominant_emotion[0]} ({dominant_emotion[1]:.2f})"
        )
        print(f"  - Dominant Bias: {dominant_bias[0]} ({dominant_bias[1]:.2f})")

        if dominant_bias[0] == "risk_aversion" and dominant_bias[1] > 0.7:
            print(
                "  - Consider counter-trend positions as MM likely to reduce liquidity"
            )
