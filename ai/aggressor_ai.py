...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("aggressor_ai.log"), logging.StreamHandler()],
)
...
class AggressorAI:
    ...
    def analyze_market(self, data: Dict) -> Dict:
        ...
        logger.info(
            f"Generated attack signal: {attack_signal['attack_signal']} with confidence {attack_signal['confidence']}"
        )
    ...
    def _identify_price_clusters(self, orders: List) -> List[Dict]:
        ...
        current_cluster = {"price": orders[0][0], "volume": orders[0][1], "orders": 1}
        ...
        for i in range(1, len(orders)):
            ...
            if (
                abs(price - current_cluster["price"]) / current_cluster["price"]
                < price_tolerance
            ):
                current_cluster["volume"] += volume
                current_cluster["orders"] += 1
            else:
                current_cluster["strength"] = (
                    current_cluster["volume"] * current_cluster["orders"]
                )
