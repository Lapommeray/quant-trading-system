...
class ComplianceCheck:
    ...
    def pre_trade_check(self, symbol, order_size=None, price=None):
        ...
        if order_size is not None and price is not None:
            ...
            if position_pct > self.max_position_pct:
                result["issues"].append(
                    f"Position size exceeds {self.max_position_pct*100}% of portfolio"
                )
                result["limit_order_size"] = (
        ...
        if result["issues"]:
            self.algo.Debug(
                f"ComplianceCheck: Issues detected for {symbol_str}: {', '.join(result['issues'])}"
            )
    ...
    def post_trade_check(self, symbol, order_size, price):
        ...
        result = {"compliant": True, "issues": [], "timestamp": current_time}
        ...
        if result["issues"]:
            self.algo.Debug(
                f"ComplianceCheck: Post-trade issues detected for {symbol_str}: {', '.join(result['issues'])}"
            )
    ...
    def _log_compliance_result(self, log_entry):
        ...
        try:
            ...
            self.algo.Debug(
                f"ComplianceCheck: Failed to log compliance result: {str(e)}"
            )
    ...
    def _load_sec_insiders(self):
        ...
        """
        return set(["RESTRICTED1", "RESTRICTED2", "RESTRICTED3"])
