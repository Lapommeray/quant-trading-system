...
def check_module(mod_name: str):
    ...
    for attr_name in dir(mod):
        try:
            ...
            pass
    if not candidates:
        return {"module":mod_name,"status":"no_trading_module","error":"No class with module_name"}
    results = []
...
def main():
    ...
    for mod_name in CORE_MODULES:
        ...
        if res["status"] != "ok":
            ...
            for cl in res["classes"]:
                if cl["ok"]:
                    print(f"[OK] {mod_name}.{cl['class']} ({cl['module_name']}) has all autonomous methods")
                else:
    ...
    try:
        ...
        print(f"Discovered {len(wired['active'])} active modules: {wired['active'][:20]}")
        expected = ["ofi_detector","whale_flow_detector","mm_intent_detector","cvd_detector","funding_detector","volume_profile","cross_asset_leader"]
        for exp in expected:
    ...
    try:
        ...
        print(f"[OK] DataRing push+latest works len={len(ticks)} latency {ring.latency_ms():.1f}ms")
    except Exception as e:
    ...
    try:
        ...
        frame = {"arg":{"channel":"trades","instId":"BTC-USDT-SWAP"},"data":[{"px":"67000","sz":"0.5","side":"buy","ts":"1710000000000"}]}
    ...
    try:
        ...
        print(f"[OK] RealEnhancedIndicator signal {sig.get('signal')} conf {sig.get('confidence')} latency {sig.get('latency_ms')}ms reason {sig.get('reason','')[:80]}")
    except Exception as e:
    ...
    try:
        out = subprocess.check_output(["grep","-R","np.random","core","--include=*.py"], text=True)
        ...
        filtered = [l for l in lines if "template" not in l and "real_enhanced" not in l and "mock" not in l.lower()][:20]
        if filtered:
            print(f"[WARN] Found np.random in core (should be 0 in hot path):\n" + "\n".join(filtered))
        else:
    ...
    try:
        ...
        # warm push 100
        for i in range(100):
            ring.push(time.time(), 67000+i*0.01, 67000.5+i*0.01, 1.2, 0.8, 67000.2, 0.5, 1 if i%2==0 else -1)
        det = OFIDetector()
        times = []
        for _ in range(20):
            s = time.time()
        ...
        import numpy as np
        print(f"[OK] OFI latency p50 {np.percentile(times,50):.1f}ms p95 {np.percentile(times,95):.1f}ms avg {np.mean(times):.1f}ms")
