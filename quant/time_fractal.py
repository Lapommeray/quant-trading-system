...
class TimeFractal:
    ...
    def find_similar_patterns(self, prices, lookback_window=30, num_patterns=3):
        ...
        current_normalized = (current_pattern - np.mean(current_pattern)) / np.std(
            current_pattern
        )
        ...
        for i in range(len(prices) - 2 * lookback_window):
            ...
            historical_normalized = (
                historical_pattern - np.mean(historical_pattern)
            ) / np.std(historical_pattern)

            correlation = np.corrcoef(current_normalized, historical_normalized)[0, 1]

            distance = np.sqrt(
                np.sum((current_normalized - historical_normalized) ** 2)
            )
            ...
            similarities.append(
                {
                    ...
                    "future_returns": float(
                        prices[i + lookback_window] / prices[i + lookback_window - 1]
                        - 1
                    ),
                }
            )
