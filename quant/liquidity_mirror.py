...
class LiquidityMirror:
    ...
    def _detect_iceberg_orders(self, orders):
        ...
        round_number_count = sum(
            1 for size in sizes if size % 100 == 0 or size % 1000 == 0
        )
    ...
    def analyze_order_book_depth(self, bids, asks, price_levels=20):
        ...
        spread = ask_prices[0] - bid_prices[0] if bid_prices and ask_prices else 0
        mid_price = (
            (bid_prices[0] + ask_prices[0]) / 2 if bid_prices and ask_prices else 0
        )
                ...
                support_levels.append(
                    {
                        ...
                        "distance_from_mid": (
                            abs(price - mid_price) / mid_price if mid_price > 0 else 0
                        ),
                    }
                )
        ...
        return {
            ...
            "bid_ask_imbalance": (
                (sum(bid_sizes) - sum(ask_sizes)) / (sum(bid_sizes) + sum(ask_sizes))
                if (sum(bid_sizes) + sum(ask_sizes)) > 0
                else 0
            ),
        }
