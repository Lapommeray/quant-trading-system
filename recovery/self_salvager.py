...
class StrategySurgeon:
    ...
    def __init__(self, algorithm, strategy_generator=None, quarantine_dir=None):
        ...
        default_quarantine_dir = (
            Path(__file__).resolve().parents[1] / "strategies" / "quarantine"
        )
    ...
    def _identify_weak_strategies(self):
        ...
        for name, performance in self.strategy_performance.items():
            ...
            if performance.get("sharpe_ratio", 0) < self.sharpe_threshold:
                is_weak = True
                flaws.append(
                    f"Low Sharpe ratio: {performance.get('sharpe_ratio', 0):.2f}"
                )
            ...
            if performance.get("profit_factor", 0) < 1.0:
                is_weak = True
                flaws.append(
                    f"Profit factor below 1.0: {performance.get('profit_factor', 0):.2f}"
                )
    ...
    def _fix_strategy(self, strategy):
        ...
        if not self.strategy_generator:
            self.logger.warning(
                f"No strategy generator available, strategy {name} removed"
            )
        ...
        try:
            ...
            return {"name": f"{name}_fixed", "path": new_path, "original": name}
    ...
    def update_strategy_performance(self, strategy_name, performance_data):
        ...
        if perf["trade_count"] > 0:
            avg_win = (
                perf["total_profit"] / perf["win_count"] if perf["win_count"] > 0 else 0
            )
            avg_loss = (
                abs(perf["total_loss"]) / perf["loss_count"]
                if perf["loss_count"] > 0
                else 0
            )
            perf["expectancy"] = (perf["win_rate"] * avg_win) - (
                (1 - perf["win_rate"]) * avg_loss
            )
    ...
    def set_thresholds(
        self, weakness_threshold=None, sharpe_threshold=None, win_rate_threshold=None
    ):
        """
