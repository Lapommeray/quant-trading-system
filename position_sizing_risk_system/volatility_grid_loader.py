...
class VolatilityGridLoader:
    ...
    def _update_vix_data(self):
        ...
        self.vix_data["term_structure_ratio"] = (
            vix3m / current_vix if current_vix > 0 else 1.0
        )
    ...
    def _update_grid_data(self):
        ...
        base_multiplier = self.position_multipliers[
            min(threshold_idx, len(self.position_multipliers) - 1)
        ]
        ...
        self.grid_data = {
            "vix_threshold": (
                self.vix_thresholds[min(threshold_idx, len(self.vix_thresholds) - 1)]
                if threshold_idx < len(self.vix_thresholds)
                else "50+"
            ),
    ...
    def calculate_position_size(
        self, base_size, asset_volatility=None, asset_correlation=None
    ):
        """
