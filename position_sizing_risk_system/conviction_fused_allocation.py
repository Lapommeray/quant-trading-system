...
from quantum_wealth_matrix.fed_modules.liquidity_arbitrage_decoder import (
    FedLiquidityArbitrageDecoder,
)
from quantum_wealth_matrix.fed_modules.repo_market_shadow_monitor import (
    RepoMarketShadowMonitor,
)
from quantum_wealth_matrix.fed_modules.interest_rate_pulse_integrator import (
    InterestRatePulseIntegrator,
)
from quantum_wealth_matrix.fed_modules.federal_docket_forecast_ai import (
    FederalDocketForecastAI,
)
from quantum_wealth_matrix.fed_modules.treasury_auction_imbalance_scanner import (
    TreasuryAuctionImbalanceScanner,
)
from quantum_wealth_matrix.fed_modules.overnight_reserve_drift_tracker import (
    OvernightReserveDriftTracker,
)
...
class ConvictionFusedAllocation:
    ...
    def __init__(self, algorithm):
        ...
        self.conviction_weights = {"fed": 0.6, "technical": 0.4}
        ...
        self.conviction_scores = {"fed": 0.0, "technical": 0.0, "combined": 0.0}
        ...
        for module_name, weight in self.fed_weights.items():
            ...
            if module is not None:
                ...
                if "combined_score" in module_result:
                    ...
                    "imbalance_score" in module_result
                    and "distortion_score" in module_result
                ):
                    module_score = (
                        module_result["imbalance_score"]
                        + module_result["distortion_score"]
                    ) / 2
                elif (
                    "volatility_score" in module_result
                    and "market_impact_score" in module_result
                ):
                    module_score = (
                        module_result["volatility_score"]
                        + module_result["market_impact_score"]
                    ) / 2
                elif (
                    "inversion_score" in module_result
                    and "pulse_score" in module_result
                ):
                    module_score = (
                        module_result["inversion_score"] + module_result["pulse_score"]
                    ) / 2
                elif (
                    "drift_score" in module_result and "leakage_score" in module_result
                ):
                    module_score = (
                        module_result["drift_score"] + module_result["leakage_score"]
                    ) / 2
                else:
                    module_score = 0.5
