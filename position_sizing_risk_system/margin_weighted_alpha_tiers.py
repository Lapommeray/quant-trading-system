...
class MarginWeightedAlphaTiers:
    ...
    def _assign_strategy_tiers(self):
        ...
        for strategy_name, performance in self.strategy_performance.items():
            ...
            self.strategy_tiers[strategy_name] = {
                ...
                "allocation_multiplier": self.alpha_tiers[assigned_tier][
                    "allocation_multiplier"
                ],
                "margin_requirement_discount": self.alpha_tiers[assigned_tier][
                    "margin_requirement_discount"
                ],
            }
    ...
    def _calculate_allocations(self):
        ...
        for strategy_name, tier_data in self.strategy_tiers.items():
            ...
            base_allocation = (
                tier_data["allocation_multiplier"] / total_points
                if total_points > 0
                else 0.0
            )
            ...
            effective_margin = margin_requirement * (
                1.0 - tier_data["margin_requirement_discount"]
            )

            margin_adjusted_allocation = (
                base_allocation / effective_margin if effective_margin > 0 else 0.0
            )
        ...
        total_margin_adjusted = sum(
            data["margin_adjusted_allocation"] for data in self.allocation_data.values()
        )
        ...
        if total_margin_adjusted > 0:
            for strategy_name in self.allocation_data:
                self.allocation_data[strategy_name]["normalized_allocation"] = (
                    self.allocation_data[strategy_name]["margin_adjusted_allocation"]
                    / total_margin_adjusted
                )
        else:
            equal_allocation = (
                1.0 / len(self.allocation_data)
                if len(self.allocation_data) > 0
                else 0.0
            )
            for strategy_name in self.allocation_data:
                self.allocation_data[strategy_name][
