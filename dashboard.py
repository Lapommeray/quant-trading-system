...
st.set_page_config(
    page_title="QMP Overrider Trading Dashboard", page_icon="📊", layout="wide"
)
...
if not signal_data.empty:
    if "timestamp" in signal_data.columns:
        days_range = st.sidebar.slider(
            "Days to display", min_value=1, max_value=90, value=30
        )
        ...
        if detailed_data:
            ...
            for entry in detailed_data:
                if "environment_state" in entry and entry["environment_state"]:
                    ...
                    env_data = {"timestamp": timestamp, "symbol": symbol}
            ...
            if env_states:
                ...
                if "market_mode" in env_df.columns:
                    market_mode_counts = (
                        env_df["market_mode"].value_counts().reset_index()
                    )
                ...
                for entry in detailed_data:
                    if (
                        "oversoul_enabled_modules" in entry
                        and entry["oversoul_enabled_modules"]
                    ):
                        for module, is_active in entry[
                            "oversoul_enabled_modules"
                        ].items():
                ...
                if module_activations:
                    ...
                    module_status = (
                        module_df.groupby("module")["active"]
                        .value_counts()
                        .unstack()
                        .fillna(0)
                    )
                    ...
                    module_status_pct = (
                        module_status.div(module_status.sum(axis=1), axis=0) * 100
                    )
                    ...
                    fig = px.bar(
                        ...
                        y=(
                            ["Enabled", "Disabled"]
                            if "Enabled" in module_status_pct.columns
                            and "Disabled" in module_status_pct.columns
                            else (
                                ["Enabled"]
                                if "Enabled" in module_status_pct.columns
                                else ["Disabled"]
    ...
    with tabs[3]:
        ...
        if detailed_data:
                    ...
                    predictive_data.append(
                        {
                            "timestamp": entry.get("timestamp", ""),
                            "symbol": entry.get("symbol", ""),
                            "forecast_direction": entry["predictive_overlay"].get(
                                "forecast_direction", "neutral"
                            ),
                            "forecast_confidence": entry["predictive_overlay"].get(
                                "forecast_confidence", 0.0
                            ),
                            "ghost_candles_count": len(
                                entry["predictive_overlay"].get("ghost_candles", [])
                            ),
                            "timelines_count": len(
                                entry["predictive_overlay"].get("timelines", [])
                            ),
                            "future_zones_count": len(
                                entry["predictive_overlay"].get("future_zones", [])
                            ),
                            "convergence_zones_count": len(
                                entry["predictive_overlay"].get("convergence_zones", [])
                            ),
                        }
                    )
            ...
            if predictive_data:
                ...
                direction_counts = (
                    pred_df["forecast_direction"].value_counts().reset_index()
                )
                ...
                for entry in reversed(detailed_data):
                    if (
                        "predictive_overlay" in entry
                        and entry["predictive_overlay"]
                        and entry["predictive_overlay"].get("ghost_candles", [])
                    ):
                        recent_entry = entry
                        break
                ...
                if recent_entry:
                    ...
                    if ghost_candles:
                        ...
                        st.info(
                            "Ghost candles represent projected future price movements with varying confidence levels. The more transparent the candle, the lower the confidence."
                        )
                ...
                for entry in reversed(detailed_data):
                    if (
                        "predictive_overlay" in entry
                        and entry["predictive_overlay"]
                        and entry["predictive_overlay"].get("timelines", [])
                    ):
                        recent_entry = entry
                        break

                if recent_entry and recent_entry["predictive_overlay"].get(
                    "timelines", []
                ):
                    ...
                    if recent_entry["predictive_overlay"].get("convergence_zones", []):
                        for i, zone in enumerate(
                            [{"time_index": 3}, {"time_index": 7}]
                        ):
                            zone_time = base_time + timedelta(
                                minutes=5 * zone["time_index"]
                            )
                    ...
                    st.info(
                        "The Timeline Warp Plot shows multiple possible future price paths. Convergence zones (highlighted areas) indicate where multiple timelines converge, suggesting high-probability price levels."
                    )
                ...
                for entry in reversed(detailed_data):
                    if (
                        "predictive_overlay" in entry
                        and entry["predictive_overlay"]
                        and entry["predictive_overlay"].get("future_zones", [])
                    ):
                        recent_entry = entry
                        break
                ...
                if recent_entry and recent_entry["predictive_overlay"].get(
                    ...
                    st.info(
                        "Future Zone Sensory visualization shows high-probability price zones based on gate consensus and alignment patterns. Wider zones indicate higher uncertainty."
                    )
            else:
