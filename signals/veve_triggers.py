...
class VeveTriggers:
    ...
    def erzulie_freda_cycle(self, prices, period=14):
        ...
        if rsi < love_level:
            ...
            message = (
                "Erzulie Freda rejects with passion. Mean reversion downward likely."
            )
        else:
            strength = 0
            signal = None
    ...
    def baron_samedi_death_zone(self, high_prices, low_prices, close_prices, period=14):
        ...
        """
        if (
            len(high_prices) < period + 10
            or len(low_prices) < period + 10
            or len(close_prices) < period + 10
        ):
            return {"signal": None, "strength": 0, "message": "Insufficient data"}
    ...
    def analyze_market(self, df):
        ...
        if samedi_signal["signal"]:
            combined_strength += (
                samedi_signal["strength"] * 1.5
            )  # Baron Samedi has higher weight
            active_signals.append(samedi_signal["signal"])

        if active_signals:
            combined_strength = min(
                1.0, combined_strength / (len(active_signals) + 0.5)
            )
