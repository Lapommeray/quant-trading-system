...
class LiquidityDrainKillSwitch:
    ...
    def _check_kill_switch_conditions(self, current_time):
        ...
        fed_balance_sheet_change_1d = self.liquidity_metrics[
            "fed_balance_sheet_change_1d"
        ]
        ...
        if reverse_repo > self.critical_threshold:
            self._activate_kill_switch(
                current_time,
                "CRITICAL",
                f"Reverse repo > ${self.critical_threshold}B (${reverse_repo}B)",
            )
            return
        ...
        if reverse_repo > self.danger_threshold:
            self._activate_kill_switch(
                current_time,
                "DANGER",
                f"Reverse repo > ${self.danger_threshold}B (${reverse_repo}B)",
            )
            return
        ...
        if reverse_repo > self.warning_threshold:
            if self.kill_switch_status["warning_level"] != "WARNING":
                self._activate_kill_switch(
                    current_time,
                    "WARNING",
                    f"Reverse repo > ${self.warning_threshold}B (${reverse_repo}B)",
                )
            return
        ...
        if reverse_repo_change_1d > 50.0:  # $50 billion increase in 1 day
            self._activate_kill_switch(
                current_time,
                "DANGER",
                f"Reverse repo increased by ${reverse_repo_change_1d}B in 1 day",
            )
            return
        ...
        if reverse_repo_change_5d > 150.0:  # $150 billion increase in 5 days
            self._activate_kill_switch(
                current_time,
                "DANGER",
                f"Reverse repo increased by ${reverse_repo_change_5d}B in 5 days",
            )
            return

        if (
            reverse_repo > self.warning_threshold
            and reverse_repo_change_1d > 20.0
            and fed_balance_sheet_change_1d < -20.0
            and excess_reserves_change_1d < -20.0
        ):
            self._activate_kill_switch(
                current_time, "DANGER", "Combined liquidity drain detected"
            )
            return
    ...
    def _activate_kill_switch(self, current_time, warning_level, reason):
        ...
        """
        if self.kill_switch_status["active"] and self._warning_level_value(
            self.kill_switch_status["warning_level"]
        ...
        self.kill_switch_history.append(
            {
                ...
                "action": (
                    "ACTIVATE" if self.kill_switch_status["active"] else "WARNING"
                ),
    ...
    def _warning_level_value(self, warning_level):
        ...
        - Numeric value
        """
        warning_levels = {"NORMAL": 0, "WARNING": 1, "DANGER": 2, "CRITICAL": 3}
