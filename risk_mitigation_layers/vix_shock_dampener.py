...
class VIXShockDampener:
    ...
    def __init__(self, algorithm):
        ...
        self.current_risk_reduction = self.risk_reduction_levels[
    ...
    def _determine_risk_level(self):
        ...
        if (
            vix > self.vix_extreme_threshold
            and yield_10y2y < self.deep_inversion_threshold
        ):
        ...
        if new_risk_level != self.current_risk_level:
            ...
            self.logger.info(
                f"Risk level changed from {self.risk_level_history[-1]['from_level']} to {self.current_risk_level}"
            )
