...
class FlashCrashDeflectionLogic:
    ...
    def __init__(self, algorithm):
        ...
        self.update_frequency = timedelta(
            minutes=1
        )  # More frequent updates for crash detection
    ...
    def _detect_flash_crash(self, current_time):
        ...
        for symbol, data in self.market_data.items():
            change_5m = (
                (data["current_price"] / data["price_5m_ago"]) - 1.0
                if data["price_5m_ago"] > 0
                else 0.0
            )
            change_15m = (
                (data["current_price"] / data["price_15m_ago"]) - 1.0
                if data["price_15m_ago"] > 0
                else 0.0
            )
            change_60m = (
                (data["current_price"] / data["price_60m_ago"]) - 1.0
                if data["price_60m_ago"] > 0
                else 0.0
            )
        ...
        if len(crash_symbols) >= 2:  # At least 2 symbols crashing
            ...
            self.crash_detection["pre_crash_prices"] = {
                symbol: crash["reference_price"]
                for symbol, crash in crash_symbols.items()
            }
            self.crash_detection["crash_magnitude"] = {
                symbol: crash["magnitude"] for symbol, crash in crash_symbols.items()
            }
            self.crash_detection["recovery_prices"] = {}

            self.logger.warning(
                f"Flash crash detected: {crash_type} crash in {len(crash_symbols)} symbols"
            )
    ...
    def _check_crash_recovery(self, current_time):
        ...
        """
        crash_duration = (
            current_time - self.crash_detection["crash_time"]
        ).total_seconds() / 60.0
    ...
    def _place_deflection_orders(self, crash_symbols):
        ...
        for symbol, crash in crash_symbols.items():
            ...
            for i, level in enumerate(levels):
                ...
                self.logger.info(
                    f"Placed deflection order: {symbol} BUY LIMIT {order_size} @ {limit_price} (Level {i+1})"
                )
    ...
    def _cancel_deflection_orders(self):
        ...
        for order_id, order in self.active_orders.items():
            if order["status"] == "ACTIVE":
                ...
                self.logger.info(
                    f"Cancelled deflection order: {order['symbol']} {order['side']} {order['type']} {order['size']} @ {order['price']}"
                )
    ...
    def _manage_deflection_orders(self, current_time):
        ...
        for order_id, order in self.active_orders.items():
            ...
            if symbol in self.market_data:
                ...
                if current_price <= limit_price:
                    ...
                    self.logger.info(
                        f"Deflection order filled: {symbol} {order['side']} {order['type']} {order['size']} @ {current_price}"
                    )
        ...
        for order_id in filled_orders:
            ...
            for other_id, other_order in self.active_orders.items():
                if (
                    other_id != order_id
                    and other_order["symbol"] == order["symbol"]
                    and other_order["level"] > order["level"]
                    and other_order["status"] == "ACTIVE"
                ):
                    other_order["status"] = "CANCELLED"

                    self.logger.info(
                        f"Cancelled deflection order: {other_order['symbol']} {other_order['side']} {other_order['type']} {other_order['size']} @ {other_order['price']}"
                    )
