...
class YieldCurveStressForecast:
    ...
    def __init__(self, algorithm):
        ...
        self.update_frequency = timedelta(
            hours=6
        )  # Less frequent updates for yield curve data
    ...
    def _update_yield_curve_data_internal(self):
        ...
        if len(self.yield_curve_data["historical"]) > 30:
            self.yield_curve_data["historical"] = self.yield_curve_data["historical"][
                -30:
            ]
    ...
    def _calculate_spreads(self):
        ...
        """
        self.yield_curve_data["10y2y_spread"] = (
            self.yield_curve_data["10y"] - self.yield_curve_data["2y"]
        )

        self.yield_curve_data["30y5y_spread"] = (
            self.yield_curve_data["30y"] - self.yield_curve_data["5y"]
        )
    ...
    def _calculate_stress_level(self):
        ...
        if (
            spread_10y2y <= self.inversion_threshold
            and spread_30y5y <= self.inversion_threshold
        ):
        ...
        if new_stress_level != self.current_stress_level:
            ...
            self.logger.info(
                f"Stress level changed from {self.stress_history[-1]['from_level']} to {self.current_stress_level}"
            )
    ...
    def _generate_stress_forecast(self):
        ...
        if len(self.yield_curve_data["historical"]) >= 5:
            recent_spreads = [
                data["10y2y_spread"]
                for data in self.yield_curve_data["historical"][-5:]
            ]
