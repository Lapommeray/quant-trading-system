...
class FederalDocketForecastAI:
    ...
    def __init__(self, algorithm):
        ...
        self.update_frequency = timedelta(
            hours=4
        )  # Less frequent updates for docket data
        ...
        self.docket_scores = {"pacer": 0.0, "fomc": 0.0, "sec": 0.0}
    ...
    def update(self, current_time):
        ...
        if current_time - self.last_update < self.update_frequency:
            return {
                ...
                "combined_score": (self.volatility_score + self.market_impact_score)
                / 2,
    ...
    def _calculate_docket_scores(self):
        ...
        if self.pacer_data:
            ...
            self.docket_scores["pacer"] = (
                high_profile_score * 0.6
                + filing_volume_score * 0.2
                + hearing_volume_score * 0.2
            )

        if self.fomc_data:
            next_meeting_date = datetime.strptime(
                self.fomc_data["next_meeting"], "%Y-%m-%d"
            )
            days_until_meeting = max(0, (next_meeting_date - datetime.now()).days)
            meeting_proximity_score = max(
                0.0, 1.0 - (days_until_meeting / 60.0)
            )  # Higher score as meeting approaches

            minutes_date = datetime.strptime(
                self.fomc_data["minutes_release"], "%Y-%m-%d"
            )
            days_until_minutes = max(0, (minutes_date - datetime.now()).days)
            minutes_proximity_score = max(
                0.0, 1.0 - (days_until_minutes / 30.0)
            )  # Higher score as minutes release approaches

            speaker_score = sum(
                event["importance_score"] for event in self.fomc_data["speaker_events"]
            ) / max(1, len(self.fomc_data["speaker_events"]))

            dot_plot_factor = 1.2 if self.fomc_data["dot_plot_update"] else 1.0

            sentiment_factor = 1.0 + abs(
                self.fomc_data["statement_sentiment"] - 0.5
            )  # Higher factor for more extreme sentiment

            self.docket_scores["fomc"] = min(
                1.0,
                (
                    (
                        meeting_proximity_score * 0.4
                        + minutes_proximity_score * 0.2
                        + speaker_score * 0.4
                    )
                    * dot_plot_factor
                    * sentiment_factor
        ...
        if self.sec_data:
            enforcement_score = sum(
                action["importance_score"]
                for action in self.sec_data["recent_enforcement_actions"]
            ) / max(1, len(self.sec_data["recent_enforcement_actions"]))
            rule_change_score = sum(
                rule["importance_score"]
                for rule in self.sec_data["upcoming_rule_changes"]
            ) / max(1, len(self.sec_data["upcoming_rule_changes"]))
            comment_period_score = min(
                1.0, self.sec_data["comment_periods_ending"] / 5.0
            )
            speech_score = min(1.0, self.sec_data["commissioner_speeches"] / 10.0)

            self.docket_scores["sec"] = (
                enforcement_score * 0.3
                + rule_change_score * 0.4
                + comment_period_score * 0.2
                + speech_score * 0.1
            )
                ...
                self.upcoming_events.append(
                    {
                        ...
                        + (
                            " (with dot plot)"
                            if self.fomc_data.get("dot_plot_update", False)
                            else ""
                        ),
