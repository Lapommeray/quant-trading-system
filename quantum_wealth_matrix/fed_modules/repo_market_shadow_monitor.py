...
class RepoMarketShadowMonitor:
    ...
    def _calculate_anomaly_score(self):
        ...
        volume_change = (
            self.triparty_repo_data["total_volume"]
            - self.triparty_repo_data["previous_day"]["total_volume"]
        ) / self.triparty_repo_data["previous_day"]["total_volume"]

        treasury_change = (
            self.triparty_repo_data["treasury_collateral"]
            - self.triparty_repo_data["previous_day"]["treasury_collateral"]
        ) / self.triparty_repo_data["previous_day"]["treasury_collateral"]

        agency_change = (
            self.triparty_repo_data["agency_collateral"]
            - self.triparty_repo_data["previous_day"]["agency_collateral"]
        ) / self.triparty_repo_data["previous_day"]["agency_collateral"]

        other_change = (
            self.triparty_repo_data["other_collateral"]
            - self.triparty_repo_data["previous_day"]["other_collateral"]
        ) / self.triparty_repo_data["previous_day"]["other_collateral"]
    ...
    def _calculate_substitution_score(self):
        ...
        substitution_change = (
            self.collateral_data["substitution_rate"]
            - self.collateral_data["previous_day"]["substitution_rate"]
        ) / self.collateral_data["previous_day"]["substitution_rate"]

        downgrade_change = (
            self.collateral_data["downgrade_rate"]
            - self.collateral_data["previous_day"]["downgrade_rate"]
        ) / self.collateral_data["previous_day"]["downgrade_rate"]

        haircut_change = (
            self.collateral_data["haircut_change"]
            - self.collateral_data["previous_day"]["haircut_change"]
        ) / self.collateral_data["previous_day"]["haircut_change"]

        substitution_score = min(
            1.0,
            max(
                0.0,
                (
                    abs(substitution_change) * 4
                    + abs(downgrade_change) * 3
                    + abs(haircut_change) * 3
                )
                / 10,
            ),
        )
