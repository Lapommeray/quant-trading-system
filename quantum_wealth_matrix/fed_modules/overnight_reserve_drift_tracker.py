...
class OvernightReserveDriftTracker:
    ...
    def __init__(self, algorithm):
        ...
        self.update_frequency = timedelta(
            hours=12
        )  # Less frequent updates for balance sheet data
        ...
        self.reserve_trends = {"1d": 0.0, "1w": 0.0, "1m": 0.0, "3m": 0.0}

        self.balance_sheet_trends = {"1d": 0.0, "1w": 0.0, "1m": 0.0, "3m": 0.0}
    ...
    def _calculate_reserve_trends(self):
        ...
        """
        if (
            not self.reserve_data
            or "current" not in self.reserve_data
            or "historical" not in self.reserve_data
        ):
            return
        ...
        if "1d_ago" in historical:
            self.reserve_trends["1d"] = (
                current["total_reserves"] - historical["1d_ago"]["total_reserves"]
            ) / historical["1d_ago"]["total_reserves"]

        if "1w_ago" in historical:
            self.reserve_trends["1w"] = (
                current["total_reserves"] - historical["1w_ago"]["total_reserves"]
            ) / historical["1w_ago"]["total_reserves"]

        if "1m_ago" in historical:
            self.reserve_trends["1m"] = (
                current["total_reserves"] - historical["1m_ago"]["total_reserves"]
            ) / historical["1m_ago"]["total_reserves"]

        if "3m_ago" in historical:
            self.reserve_trends["3m"] = (
                current["total_reserves"] - historical["3m_ago"]["total_reserves"]
            ) / historical["3m_ago"]["total_reserves"]
    ...
    def _calculate_balance_sheet_trends(self):
        ...
        """
        if (
            not self.balance_sheet_data
            or "current" not in self.balance_sheet_data
            or "historical" not in self.balance_sheet_data
        ):
            return
        ...
        if "1d_ago" in historical:
            self.balance_sheet_trends["1d"] = (
                current["total_assets"] - historical["1d_ago"]["total_assets"]
            ) / historical["1d_ago"]["total_assets"]

        if "1w_ago" in historical:
            self.balance_sheet_trends["1w"] = (
                current["total_assets"] - historical["1w_ago"]["total_assets"]
            ) / historical["1w_ago"]["total_assets"]

        if "1m_ago" in historical:
            self.balance_sheet_trends["1m"] = (
                current["total_assets"] - historical["1m_ago"]["total_assets"]
            ) / historical["1m_ago"]["total_assets"]

        if "3m_ago" in historical:
            self.balance_sheet_trends["3m"] = (
                current["total_assets"] - historical["3m_ago"]["total_assets"]
            ) / historical["3m_ago"]["total_assets"]
    ...
    def _calculate_leakage_score(self):
        ...
        """
        if (
            not self.balance_sheet_trends
            or not self.reserve_data
            or "current" not in self.reserve_data
        ):
            return 0.0
        ...
        reverse_repo_ratio = (
            self.reserve_data["current"]["reverse_repo"]
            / self.reserve_data["current"]["total_reserves"]
        )

        leakage_score = min(
            1.0,
            max(0.0, (0.5 - weighted_trend * 10.0) * 0.7 + reverse_repo_ratio * 0.3),
        )
    ...
    def _generate_signal(self):
        ...
        rapid_balance_sheet_change = (
            abs(self.balance_sheet_trends["1d"]) > 0.005
        )  # 0.5% daily change
