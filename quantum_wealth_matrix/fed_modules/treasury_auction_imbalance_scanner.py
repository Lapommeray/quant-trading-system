...
class TreasuryAuctionImbalanceScanner:
    ...
    def __init__(self, algorithm):
        ...
        self.update_frequency = timedelta(
            hours=6
        )  # Less frequent updates for auction data
    ...
    def _calculate_imbalance_score(self):
        ...
        """
        if (
            not self.auction_data
            or "recent_auctions" not in self.auction_data
            or not self.auction_data["recent_auctions"]
        ):
            return 0.0

        avg_bid_to_cover = sum(
            auction["bid_to_cover"] for auction in self.auction_data["recent_auctions"]
        ) / len(self.auction_data["recent_auctions"])

        avg_tail = sum(
            auction["tail"] for auction in self.auction_data["recent_auctions"]
        ) / len(self.auction_data["recent_auctions"])

        avg_primary_dealer_take = sum(
            auction["primary_dealers"]
            for auction in self.auction_data["recent_auctions"]
        ) / len(self.auction_data["recent_auctions"])

        bid_to_cover_factor = max(
            0.0, 1.0 - (avg_bid_to_cover - 2.0) / 1.0
        )  # 2.0 is threshold, 3.0 is excellent
        tail_factor = min(1.0, avg_tail * 500.0)  # 0.002 (2 bps) is high tail
        primary_dealer_factor = min(
            1.0, avg_primary_dealer_take * 2.0
        )  # 0.5 (50%) is high take

        imbalance_score = (
            bid_to_cover_factor * 0.4 + tail_factor * 0.4 + primary_dealer_factor * 0.2
        )
    ...
    def _calculate_failure_score(self):
        ...
        failure_severity = sum(
            failure["severity"]
            for failure in self.primary_dealer_data.get("dealer_failures", [])
        ) / max(1, failure_count)
        dealer_stress = self.primary_dealer_data.get("dealer_stress_index", 0.0)

        failure_score = min(
            1.0,
            (failure_count / 5.0) * 0.3 + failure_severity * 0.3 + dealer_stress * 0.4,
        )
