...
class InterestRatePulseIntegrator:
    ...
    def _calculate_spreads(self):
        ...
        self.spreads["3m10y"] = (
            self.yield_curve_data["10y"] - self.yield_curve_data["3m"]
        )
        self.spreads["5s30s"] = (
        ...
        self.spreads["10s30s"] = (
            self.yield_curve_data["30y"] - self.yield_curve_data["10y"]
        )
    ...
    def _calculate_inversion_score(self):
        ...
        for spread in self.inversion_durations:
            if self.inversions[spread]:
                ...
                duration_effect = (
                    2.0 / (1.0 + np.exp(-0.05 * self.inversion_durations[spread])) - 1.0
                )
        ...
        inversion_score = min(
            ...
            max(
                0.0,
                (
                    inversion_count / len(self.inversions) * 0.5
                    + weighted_duration * 0.5
                ),
            ),
        )
    ...
    def _calculate_pulse_score(self):
        ...
        raw_pulse_score = (
            0.5
            - (
                next_meeting_change * 5.0
                + six_month_change * 3.0
                + twelve_month_change * 2.0
            )
            / 10.0
        )
