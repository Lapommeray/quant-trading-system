...
class FedLiquidityArbitrageDecoder:
    ...
    def _calculate_imbalance_score(self):
        ...
        amount_change = (
            self.on_rrp_data["total_amount"]
            - self.on_rrp_data["previous_day"]["total_amount"]
        ) / self.on_rrp_data["previous_day"]["total_amount"]

        participant_change = (
            self.on_rrp_data["participants"]
            - self.on_rrp_data["previous_day"]["participants"]
        ) / self.on_rrp_data["previous_day"]["participants"]

        imbalance_score = min(
            1.0, max(0.0, (abs(amount_change) * 5 + abs(participant_change) * 3) / 8)
        )
    ...
    def _calculate_distortion_score(self):
        ...
        volume_change = (
            self.fedwire_data["total_volume"]
            - self.fedwire_data["previous_day"]["total_volume"]
        ) / self.fedwire_data["previous_day"]["total_volume"]

        transaction_change = (
            self.fedwire_data["transaction_count"]
            - self.fedwire_data["previous_day"]["transaction_count"]
        ) / self.fedwire_data["previous_day"]["transaction_count"]

        value_change = (
            self.fedwire_data["average_value"]
            - self.fedwire_data["previous_day"]["average_value"]
        ) / self.fedwire_data["previous_day"]["average_value"]
