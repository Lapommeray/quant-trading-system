...
class WebSocketStreams:
    ...
    def __init__(self, exchange: str):
        ...
        if self.exchange not in self.ws_endpoints:
            raise ValueError(
                f"Exchange {self.exchange} not supported for WebSocket streams"
            )
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        ...
        """
        logger.info(
            f"WebSocket connection closed for {self.exchange}: {close_msg} ({close_status_code})"
        )
    ...
    def stop(self):
        ...
        for stream_name, ws in list(self.connections.items()):
            try:
                ...
                logger.error(
                    f"Error closing WebSocket connection for {stream_name}: {e}"
                )
    ...
    def subscribe(
        self, stream_name: str, callback: Callable[[Dict[str, Any]], None]
    ) -> bool:
        """
        ...
        try:
            if self.exchange == "binance":
                ...
                logger.error(
                    f"WebSocket subscription not implemented for {self.exchange}"
                )
                return False
    ...
    def unsubscribe(self, stream_name: str) -> bool:
        ...
        if stream_name in self.connections:
            try:
                ...
                logger.error(
                    f"Error unsubscribing from {stream_name} on {self.exchange}: {e}"
                )
                return False
        else:
    ...
    def is_subscribed(self, stream_name: str) -> bool:
        ...
        """
        return (
            stream_name in self.connections
            and self.connections[stream_name].sock
            and self.connections[stream_name].sock.connected
        )
