...
class DataVerifier:
    ...
    def verify_ohlcv_data(
        self, data: List[List[float]], symbol: str, exchange: str, timeframe: str
    ) -> Tuple[bool, str]:
        """
        ...
        for i in range(len(data) - 1):
            ...
            price_change_pct = (
                abs(next_open - current_close) / current_close if current_close else 0
            )
            if price_change_pct > 0.5:  # 50% change between candles is suspicious
                reason = (
                    f"Unrealistic price gap: {price_change_pct:.2%} between candles"
                )
                self._handle_verification_failure(reason, symbol, exchange)
                return False, reason
        ...
        self._record_verification(
            True, "Data verified successfully", symbol, exchange, "ohlcv", timeframe
        )
        return True, "Data verified successfully"

    def verify_ticker_data(
        self, ticker: Dict[str, Any], symbol: str, exchange: str
    ) -> Tuple[bool, str]:
        """
        ...
        self._record_verification(
            True, "Ticker verified successfully", symbol, exchange, "ticker"
        )
        return True, "Ticker verified successfully"

    def verify_order_book_data(
        self, order_book: Dict[str, Any], symbol: str, exchange: str
    ) -> Tuple[bool, str]:
        """
        ...
        self._record_verification(
            True, "Order book verified successfully", symbol, exchange, "order_book"
        )
        return True, "Order book verified successfully"

    def verify_trade_data(
        self, trades: List[Dict[str, Any]], symbol: str, exchange: str
    ) -> Tuple[bool, str]:
        """
        ...
        self._record_verification(
            True, "Trade data verified successfully", symbol, exchange, "trades"
        )
        return True, "Trade data verified successfully"

    def verify_exchange_info(
        self, exchange_info: Dict[str, Any], exchange: str
    ) -> Tuple[bool, str]:
        """
        ...
        if "has" in exchange_info:
            ...
            if missing_capabilities:
                reason = (
                    f"Exchange missing required capabilities: {missing_capabilities}"
                )
                logger.warning(f"Exchange limitation: {reason} for {exchange}")

        self._record_verification(
            True,
            "Exchange info verified successfully",
            "N/A",
            exchange,
            "exchange_info",
        )
    ...
    def verify_all_data_sources(self, data_sources: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for source_name, source_data in data_sources.items():
            ...
            if source_type == "ohlcv":
                ...
                is_authentic, reason = self.verify_order_book_data(
    ...
    def _handle_verification_failure(
        self, reason: str, symbol: str, exchange: str
    ) -> None:
        """
    ...
    def run_nuclear_verification(self) -> bool:
        ...
        failure_count = sum(
            1 for record in self.verification_history if not record["success"]
        )

        if failure_count > 0:
            logger.error(
                f"Nuclear verification failed: {failure_count} verification failures detected"
            )
            return False
