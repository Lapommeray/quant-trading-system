...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("live_data_test.log")],
)
...
def test_api_vault():
    ...
    vault.set_credentials(
        "test_exchange", "test_api_key", "test_secret", {"additional": "param"}
    )
...
def test_exchange_connector():
    ...
    try:
        ...
        if connected:
            ...
            logger.info(
                f"BTC/USDT order book: {len(order_book['bids'])} bids, {len(order_book['asks'])} asks"
            )
    ...
    try:
        ...
        if connected:
            ...
            is_authentic, reason = verifier.verify_order_book_data(
                order_book, "BTC/USDT", "binance"
            )
            logger.info(
                f"Order book verification result: {is_authentic}, reason: {reason}"
            )

            trades = exchange.fetch_trades("BTC/USDT", limit=10)
            is_authentic, reason = verifier.verify_trade_data(
                trades, "BTC/USDT", "binance"
            )
        ...
        if any(connection_results.values()):
            ...
            if subscribed:
                ...
                if received_message[0]:
                    ...
                    logger.warning(
                        f"No WebSocket message received via router within {timeout} seconds"
                    )
